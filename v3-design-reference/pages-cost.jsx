...
/* ----- 원가율 게이지 바 컴포넌트 ----- */
function CostGauge({ rate, max = 50, warn = 35 }) {
  const pct = Math.min((rate / max) * 100, 100);
  const warnPct = (warn / max) * 100;
  const color = rate >= warn ? "var(--negative)" : rate >= 30 ? "var(--warn)" : "var(--positive)";
  const cls = rate >= warn ? "danger" : rate >= 30 ? "caution" : "safe";
  return (
    <div className="cost-gauge-wrap">
      <div className="cost-gauge-track" style={{borderRadius:4, overflow:"visible"}}>
        <div className="cost-gauge-fill" style={{width:`${pct}%`, background:color, borderRadius:4}} />
        <div className="cost-gauge-mark" style={{left:`${warnPct}%`}} data-label={`${warn}%`} />
      </div>
      <div className="cost-gauge-labels">
        <span>0%</span><span style={{color}}>{rate.toFixed(1)}%</span><span>{max}%</span>
      </div>
    </div>
  );
}
  ...
  return (
    <main className="main">
      <PageHeader
        ...
        actions={<>
          <button className="btn">onClick={()=>showToast("전체 양식 다운로드 완료", "ok")}<Icon.download style={{width:14, height:14}}/>전체 양식 다운로드</button>
        </>}
      />
    ...
    <main className="main">
      ...
      <div className="card table-card">
        <table className="data-table stagger-rows">
          <thead>
            <tr>
              <th onClick={()=>toggleSort("name")} className="sortable sort-th">메뉴명 <SortIco active={sortKey==="name"} dir={sortDir}/></th>
              <th style={{width:100}}>엣지</th>
              <th style={{width:80}}>규격</th>
              <th style={{width:140, textAlign:"right"}} onClick={()=>toggleSort("sale")} className="sortable sort-th">판매가 <SortIco active={sortKey==="sale"} dir={sortDir}/></th>
              <th style={{width:140, textAlign:"right"}} onClick={()=>toggleSort("cost")} className="sortable sort-th">원가 <SortIco active={sortKey==="cost"} dir={sortDir}/></th>
              <th style={{width:160, textAlign:"right"}} onClick={()=>toggleSort("rate")} className="sortable sort-th">원가율 <SortIco active={sortKey==="rate"} dir={sortDir}/></th>
    ...
    <main className="main">
      <PageHeader
        ...
        actions={<>
          ...
          <button className="btn primary">onClick={()=>showToast("파일을 선택해주세요 (데모)", "info")}<Icon.upload style={{width:14, height:14}}/>세부 원가 업로드</button>
        </>}
      />
  ...
  return (
    <main className="main">
      <PageHeader
        ...
        actions={<button className="btn primary">onClick={()=>showToast("파일을 선택해주세요 (데모)", "info")}<Icon.upload style={{width:14, height:14}}/>업로드</button>}
      />
  ...
  return (
    <main className="main">
      <PageHeader
        ...
        actions={<>
          <button className="btn">onClick={()=>showToast("양식 다운로드 완료", "ok")}<Icon.download style={{width:14, height:14}}/>양식 다운로드</button>
          <button className="btn primary">onClick={()=>showToast("판매가 파일을 업로드해주세요 (데모)", "info")}<Icon.upload style={{width:14, height:14}}/>판매가 업로드</button>
        </>}
      />
  ...
  return (
    <main className="main">
      <PageHeader
        ...
        actions={<button className="btn">onClick={()=>showToast("CSV 파일이 저장됐어요", "ok")}<Icon.download style={{width:14, height:14}}/>CSV 내보내기</button>}
      />
