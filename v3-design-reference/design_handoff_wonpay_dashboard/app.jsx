/* global React, ReactDOM, Sidebar, TopBar, Dashboard, Icon, CommandPalette, Pages, TweaksPanel, useTweaks, TweakSection, TweakRadio, TweakColor, TweakSlider, TweakToggle, fmtKRW */
    ...
    <div className="scrim" onClick={onClose}>
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        <div className="actions">
          ...
          <button className="btn primary block">저장하기</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="scrim" onClick={onClose}>
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        <div className="actions">
          <button className="btn ghost block" onClick={onClose}>취소</button>
          <button className="btn primary block">파일 선택</button>
        </div>
      </div>
    </div>
  );
...
function App() {
  ...
  return (
    <div className={"app " + (mobileNav ? "nav-open" : "")}>
      <Sidebar active={active} onNav={(id)=>{ setActive(id); setMobileNav(false); }} />
      ...
      <div>
        ...
        />
        <div key={active} style={{animation:"slide-up 280ms cubic-bezier(0.2,0.8,0.2,1) both"}}>
        {(() => {
          if (active === "home") {
            return <Dashboard
              onOpenUpload={()=>setModal("upload")}
              onOpenNote={()=>setActive("menu-dev-note-write")}
            />;
          }
          ...
          if (active === "cost")                   return <Pages.CostLandingPage onNav={setActive} />;
          ...
          if (active === "menu-dev-note-list")     return <Pages.MenuDevNoteListPage onWrite={()=>setActive("menu-dev-note-write")} />;
          if (active === "menu-dev-note-write")    return <Pages.MenuDevNoteWritePage onCancel={()=>setActive("menu-dev-note-list")} />;

          /* ----- 보고서센터 ----- */
          if (active === "report")                      return <Pages.ReportLandingPage onNav={setActive} />;
