...
- **잔여 대상** (우선순위 순):
  1. `app/note/sample/page.jsx` (~422줄) — 데이터 로드/검색·정렬 상태/샘플 액션 handler를 hook으로 분리할지 재평가
  2. `app/note/_NoteContent.jsx` (~186줄) — 상단 헤더/필터/프리셋/다이얼로그 callback wiring이 더 커질 때 controller hook 분리 재평가
  3. `app/nutrition/allergen/useAllergenPageData.js` (~142줄) — 상세 행 상태/총계 계산이 더 커질 때 detail/summary hook 분리 재평가
  4. `app/nutrition/allergen/allergenPageDataUtils.js` (~127줄) — 검색/정렬/CSV 정책이 더 늘어날 때 filter/order/export helper 분리 재평가
  ...
  7. `components/report/sales/SalesRankTable.jsx` (~80줄) — 순위표 row 변형/전월 비교 표시 요구가 늘어날 때 row type별 추가 분리
  8. `app/ingredient/manage/IngredientManualCostFields.jsx` (~71줄) — 수동 단가 정책이 더 늘어날 때 보관/과세/가격 입력을 추가 분리 재평가
  9. `app/nutrition/allergen/useAllergenOrderState.js` (~52줄) — 순서 저장 정책이 더 늘어날 때 저장 helper 분리 재평가
  10. `app/ingredient/list/page.jsx` — 현재 redirect route 5줄 수준, 별도 분해 불필요
