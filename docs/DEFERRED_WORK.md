...
- **잔여 대상** (우선순위 순):
  ...
  3. `app/note/sample/_SampleFilterControls.jsx` (~192줄) — 필터 UI 옵션이 더 늘어날 때 카테고리·별점·검색 섹션 하위 분리 재평가
  4. `app/note/_NoteContent.jsx` (~186줄) — 상단 헤더/필터/프리셋/다이얼로그 callback wiring이 더 커질 때 controller hook 분리 재평가
  5. `app/note/sample/_SampleRecordsView.jsx` (~179줄) — 카드/테이블 상호작용이 더 늘어날 때 grid/list view 하위 분리 재평가
  6. `app/nutrition/allergen/useAllergenPageData.js` (~142줄) — 상세 행 상태/총계 계산이 더 커질 때 detail/summary hook 분리 재평가
  7. `app/nutrition/allergen/allergenPageDataUtils.js` (~127줄) — 검색/정렬/CSV 정책이 더 늘어날 때 filter/order/export helper 분리 재평가
  8. `app/note/_NoteListBody.jsx` (~113줄) — 목록 본문 상호작용이 더 늘어날 때 context/detail state hook 분리 재평가
  ...
  12. `app/nutrition/allergen/useAllergenOrderState.js` (~52줄) — 순서 저장 정책이 더 늘어날 때 저장 helper 분리 재평가
  13. `app/ingredient/list/page.jsx` — 현재 redirect route 5줄 수준, 별도 분해 불필요
