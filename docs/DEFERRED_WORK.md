...
- **잔여 대상** (우선순위 순):
  ...
  2. `app/nutrition/allergen/useAllergenPageData.js` (~142줄) — 상세 행 상태/총계 계산이 더 커질 때 detail/summary hook 분리 재평가
  3. `app/nutrition/allergen/allergenPageDataUtils.js` (~127줄) — 검색/정렬/CSV 정책이 더 늘어날 때 filter/order/export helper 분리 재평가
  ...
  8. `app/nutrition/allergen/useAllergenOrderState.js` (~52줄) — 순서 저장 정책이 더 늘어날 때 저장 helper 분리 재평가
  9. `app/ingredient/list/page.jsx` — 현재 redirect route 5줄 수준, 별도 분해 불필요
