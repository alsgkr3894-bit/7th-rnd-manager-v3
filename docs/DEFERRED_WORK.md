...
- **잔여 대상** (우선순위 순):
  1. `app/note/sample/useSamplePageController.js` (~170줄) — 샘플기록 props 그룹이 더 늘어날 때 actions/filter/records/dialog builder 추가 분리 재평가
  2. `app/note/sample/useSamplePageState.js` (~160줄) — 상태/URL/검색히스토리 wiring이 더 늘어날 때 state hook 추가 분리 재평가
  3. `lib/note/content-props.js` (~167줄) — 노트 목록 props 그룹이 더 늘어날 때 dialogs/header/filter/body builder 추가 분리 재평가
  ...
  23. `hooks/useNoteContentController.js` (~40줄) — 현재 shell hook 수준, 별도 분해 불필요
  24. `app/note/_NoteContent.jsx` (~39줄) — 현재 조립 컴포넌트 수준, 별도 분해 불필요
  25. `app/ingredient/list/page.jsx` — 현재 redirect route 5줄 수준, 별도 분해 불필요
