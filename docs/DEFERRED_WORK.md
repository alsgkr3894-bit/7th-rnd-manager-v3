...
- **2026-06-18 추가 완료**: 입력부 구조 분리 완료. `MenuRecipeSection.jsx`는 조립·focus 이동 중심으로 축소했고, DB load/save/요약 계산은 `useMenuRecipeEditor.js`, 자동완성 키보드 처리는 `useRecipeIngredientSearch.js`, row hydrate/save helper는 `recipeComponentRows.js`로 분리했다.
- **2026-06-29 추가 완료**: 공통원가 묶음 상세 접힘 목록, 구성품 행 복사, 단가 없음 필터/보정 이동, 저장 전 누락 항목 확인 모달, 원산지/알레르기 영향 미리보기, 이슈 탭 빠른 액션(`바로 수정`, `레시피 섹션으로 이동`, `단가 보정으로 이동`) 구현.
- **2026-06-29 추가 완료**: 빈 구성품 행 저장 제외, 직접 레시피 수량 0 이하 방어, 공통원가 음수 차감 유지.
- 최근 사용 / 같은 카테고리 자주 쓰는 식자재 우선 추천 — 사용자 요청으로 제외.
- **현재 상태**: 사용자 요청으로 제외한 최근 사용/자주 쓰는 식자재 추천 외 UX 잔여 후보는 완료.
...
- **P5 대량 데이터 성능**: `perf-large-dataset.test.mjs` — 1K/5K 행 식자재·메뉴마스터 필터 응답시간(30ms/100ms 기준), selector memoization 구조 검증.
- **P1 운영 QA 실행 기록**: 자동 테스트로만 검증 가능한 항목 완료. 수동 QA(Excel 열람·PDF 검수·대용량 복원 리허설)는 별도 일정에서 진행 예정.
...
**보류 항목** → 메뉴마스터 레시피 UX 잔여 후보 · 식자재 데이터 정리 도구 보류 섹션에 등록.
