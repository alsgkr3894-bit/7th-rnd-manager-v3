...
- **잔여 대상** (우선순위 순):
  ...
  3. `hooks/useNoteContentController.js` (~174줄) — 노트 목록 callback 정책이 더 늘어날 때 dialogs/header/body props helper 분리 재평가
  ...
  22. `app/note/_NoteContent.jsx` (~39줄) — 현재 조립 컴포넌트 수준, 별도 분해 불필요
  23. `app/ingredient/list/page.jsx` — 현재 redirect route 5줄 수준, 별도 분해 불필요
