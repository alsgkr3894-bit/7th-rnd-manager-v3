...
Jest 단위 테스트 276개 파일(lib 250, hooks 20, scripts 6), QA 명령 4종(qa:smoke 22라우트, qa:runtime 전라우트 65개, qa:prod 프로덕션빌드, qa:workflow 업무흐름 E2E 16시나리오) + 문서 수치 검증(audit:docs). 커버리지 수집 비활성화, playwright 기반 브라우저 QA 분리 운용.
