...
- 검증:
  - `npm run test:ci` 269 suites / 1438 tests 통과
  - `npm run lint` 통과
...
- 검증:
  ...
  - `npm run qa:workflow`
- UX 보완 포인트 상태:
  - 저장 전 빈 row 제외, 직접 레시피 수량 0 이하 방어, productCode 없는 수동 식자재 단가 key 정책 테스트 완료.
  - 저장 후 `onSaved` reload 반영은 workflow QA의 레시피 UI 저장/재진입과 원가 반영 시나리오로 확인 완료.
  - 구성품 행 복사, 단가 없음 필터/보정 이동, 원산지/알레르기 영향 미리보기, 공통원가 상세 접힘 목록, 이슈 탭 빠른 액션 완료.
  - 최근 사용 식자재 추천은 사용자 요청으로 제외.
