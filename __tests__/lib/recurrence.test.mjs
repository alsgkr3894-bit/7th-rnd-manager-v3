...
describe('none (단일 발생)', () => {
  test('범위 내: 해당 날짜만 반환', () => {
    expect(expandOccurrences(sched('2026-06-15'), '2026-06-01', '2026-06-30'))
      .toEqual(['2026-06-15']);
  });

  test('범위 밖(이전): 빈 배열', () => {
    expect(expandOccurrences(sched('2026-05-20'), '2026-06-01', '2026-06-30'))
      .toEqual([]);
  });

  test('범위 밖(이후): 빈 배열', () => {
    expect(expandOccurrences(sched('2026-07-01'), '2026-06-01', '2026-06-30'))
      .toEqual([]);
  });

  test('범위 경계(시작): 포함', () => {
    expect(expandOccurrences(sched('2026-06-01'), '2026-06-01', '2026-06-30'))
      .toEqual(['2026-06-01']);
  });

  test('범위 경계(끝): 포함', () => {
    expect(expandOccurrences(sched('2026-06-30'), '2026-06-01', '2026-06-30'))
      .toEqual(['2026-06-30']);
  });

  test('date 없음: 빈 배열', () => {
    expect(expandOccurrences({ date: '', repeatType: 'none', repeatUntil: null }, '2026-06-01', '2026-06-30'))
      .toEqual([]);
  });
});
...
describe('daily (매일 반복)', () => {
  ...
  test('base가 범위 이후: 빈 배열', () => {
    expect(expandOccurrences(sched('2026-07-01', 'daily'), '2026-06-01', '2026-06-30'))
      .toEqual([]);
  });

  test('repeatUntil 이 rangeEnd 보다 이른 경우: repeatUntil 까지만', () => {
    const res = expandOccurrences(sched('2026-06-01', 'daily', '2026-06-05'), '2026-06-01', '2026-06-30');
...
describe('weekly (매주 반복)', () => {
  ...
  test('repeatUntil 적용', () => {
    const res = expandOccurrences(sched('2026-06-01', 'weekly', '2026-06-15'), '2026-06-01', '2026-06-30');
...
describe('monthly (매월 반복)', () => {
  ...
  test('repeatUntil 이 범위 중간: 그 달까지만', () => {
    const res = expandOccurrences(sched('2026-01-10', 'monthly', '2026-04-10'), '2026-01-01', '2026-06-30');
