...
describe('recordPriceChange — NO-OP 가드', () => {
  ...
  test('newPrice == null 이면 put 호출 없음', async () => {
    await recordPriceChange({
      ingredientId: 1, productCode: 'A', ingredientName: '테스트',
      oldPrice: 1000, newPrice: null, source: 'edit',
    });
  ...
  test('oldPrice === newPrice 이면 put 호출 없음', async () => {
    await recordPriceChange({
      ingredientId: 1, productCode: 'A', ingredientName: '테스트',
      oldPrice: 2000, newPrice: 2000, source: 'edit',
    });
    expect(mockPut).not.toHaveBeenCalled();
  });
  ...
  test('hasStore가 false면 put 호출 없음', async () => {
    ...
    await recordPriceChange({
      ingredientId: 1, productCode: 'A', ingredientName: '테스트',
      oldPrice: 1000, newPrice: 2000, source: 'edit',
    });
    expect(mockPut).not.toHaveBeenCalled();
  });
  ...
  test('가격이 다르면 put 호출됨', async () => {
    ...
    await recordPriceChange({
      ingredientId: 5, productCode: 'B', ingredientName: '소금',
      oldPrice: 300, newPrice: 400, source: 'register',
    });
  ...
  test('oldPrice가 null인 신규 가격 등록도 기록됨', async () => {
    ...
    await recordPriceChange({
      ingredientId: 7, productCode: 'C', ingredientName: '버터',
      oldPrice: null, newPrice: 5000, source: 'bulk',
    });
    expect(mockPut).toHaveBeenCalledTimes(1);
  });
  ...
  test('put이 throw해도 호출자로 예외를 전파하지 않음', async () => {
    ...
    await expect(
      recordPriceChange({
        ingredientId: 1, productCode: 'X', ingredientName: '치즈',
        oldPrice: 100, newPrice: 200, source: 'edit',
      }),
