...
describe('note list structure', () => {
  test('노트 목록 page는 필터와 목록 렌더링을 전용 컴포넌트에 위임한다', () => {
    ...
    const photoLightboxSource = readFileSync(resolve('app/note/_NotePhotoLightbox.jsx'), 'utf8');
    const photoCarouselSource = readFileSync(resolve('components/note/PhotoCarousel.jsx'), 'utf8');
    ...
    expect(ideaGroupCardSource).toContain('onDragStart={event => event.preventDefault()}');
    ...
    expect(ideaGroupsSource).toContain('periodLabel: periodLabel(notes)');
    expect(ideaGroupsSource).toContain('title: latestTitle');
    expect(cardSource).toContain('noteDisplayTitle(note)');
    expect(cardSource).toContain('function notePhotos');
    expect(cardSource).toContain("import { PhotoCarousel } from '@/components/note/PhotoCarousel'");
    expect(cardSource).toContain("import { NotePhotoLightbox } from './_NotePhotoLightbox'");
    expect(cardSource).toContain('const [previewPhoto, setPreviewPhoto] = useState(null)');
    expect(cardSource).toContain('setPreviewPhoto(photo)');
    expect(cardSource).toContain('onMouseDown={event => event.stopPropagation()}');
    expect(cardSource).toContain('collectLatestRoundNotePhotos([{ photos }], 99)');
    expect(cardSource).toContain('const snippets = [');
    expect(cardSource).toContain('gridTemplateColumns: photos.length ?');
    expect(cardSource).toContain('photos={photos}');
    expect(photoCarouselSource).toContain('export function PhotoCarousel');
    expect(photoCarouselSource).toContain('setIndex(value => (value + delta + count) % count)');
    expect(photoCarouselSource).toContain("objectFit: 'contain'");
    expect(photoCarouselSource).toContain('Icon.chevLeft');
    expect(photoCarouselSource).toContain('Icon.chevRight');
    expect(photoLightboxSource).toContain('export function NotePhotoLightbox');
    expect(photoLightboxSource).toContain("import { createPortal } from 'react-dom'");
    expect(photoLightboxSource).toContain('document.body');
    expect(photoLightboxSource).toContain("String(title || photo.caption || '사진 보기')");
    expect(photoLightboxSource).not.toContain('photo.name || title');
    expect(photoLightboxSource).toContain('aria-label="사진 크게 보기"');
    expect(photoLightboxSource).toContain("event.key === 'Escape'");
