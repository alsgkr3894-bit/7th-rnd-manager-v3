...
import { buildPaginationPages, normalizePaginationState } from '../../lib/ui/pagination.js';
...
describe('pagination ui helpers', () => {
  ...
  test('전체 페이지가 없거나 1페이지뿐이면 표시 범위는 0으로 둔다', () => {
    expect(normalizePaginationState({ page: 1, totalPages: 1, total: 7, pageSize: 10 }).start).toBe(
      0
    );
    expect(normalizePaginationState({ page: 1, totalPages: 0, total: 0, pageSize: 10 }).end).toBe(
      0
    );
  });
