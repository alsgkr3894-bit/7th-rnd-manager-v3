...
describe('nutrition set calc structure', () => {
  ...
  test('하프앤하프·세트박스 모두 1인용 피자를 자동 피자 후보에서 제외한다', () => {
    ...
    expect(tabSource.replace(/\s+/g, ' ')).toContain('masterByCode, pizzaMenus, safeEdgeMap');
  ...
  test('구성품은 별도 이름 입력 없이 메뉴 검색으로만 추가하고, 부분 수량(qty/baseQty)으로 영양값 비율을 반영한다', () => {
    ...
    const setCalcSource = readFileSync(resolve('lib/nutrition/values/set-calc.js'), 'utf8');
    expect(setCalcSource).toContain('function slotRatio(slot)');
    expect(setCalcSource.replace(/\s+/g, ' ')).toContain('slotMin += Math.min(...kcals) * ratio;');
    expect(setCalcSource.replace(/\s+/g, ' ')).toContain('slotMax += Math.max(...kcals) * ratio;');
  });
