      ...
      >
        ...
        <div
          style={{
            ...
            flex: 1,
            display: 'grid',
            gridTemplateColumns: photos.length ? '1fr 1fr' : '1fr',
          }}
        >
          {photos.length > 0 && (
            <div
                    ...
                    {photos.map((p, i) => (
                      <button
                        ...
                        style={{
                          ...
                          overflow: 'hidden',
                          border:
                            i === currentPhotoIdx ? '2px solid #fff' : '2px solid transparent',
                          padding: 0,
                          cursor: 'pointer',
                          background: '#222',
                        }}
          ...
          <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
            {description && <Section title="테스트 내용 / 조건">{description}</Section>}
            {result && <Section title="평가 / 결과">{result}</Section>}
            {improvements && <Section title="개선사항">{improvements}</Section>}
            {nextAction && <Section title="다음 액션">{nextAction}</Section>}
            {tags.length > 0 && (
              <div>
                <div
                  style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', marginBottom: 6 }}
                >
                  태그
                </div>
