...
export function CompareModal({ samples, onClose }) {
  ...
  const fields = [
    ...
    { label: '단가',     get: (s) => {
      ...
      return price != null ? `${price.toLocaleString('ko-KR')}원${s.priceTaxType === 'excl' ? '(별도)' : ''}` : '';
    } },
      ...
      >
        ...
        <div style={{ overflowY:'auto', flex:1, padding:'16px 20px' }}>
          {safeSamples.length === 0 ? (
            ...
            <>
              <div style={{ display:'grid', gridTemplateColumns:`repeat(${columnCount}, 1fr)`, gap:12, marginBottom:16 }}>
                {safeSamples.map((s, idx) => {
                  const title = asDisplayText(s.title, '제목 없음');
                  const photo = getSamplePhoto(s);

                  return (
                    <div key={asDisplayText(s.id, `sample-${idx}`)} className="compare-col" style={{ textAlign:'center' }}>
                      {photo ? (
                        <img
                          src={photo}
                          alt=""
              ...
              {fields.map(({ label, key, render, get }) => (
                <div key={label} style={{
                  display:'grid', gridTemplateColumns:`160px repeat(${columnCount}, 1fr)`,
                  ...
                  {safeSamples.map((s, idx) => {
                    ...
                    const rating = key === 'rating'
                      ? clampInteger(val, { min: 0, max: 5, fallback: 0 })
                      : 0;
                    const isHighlight = key === 'rating' && rating === maxRating && maxRating > 0;
                    return (
                      <div key={asDisplayText(s.id, `sample-${idx}`)}
                      ...
                      >
                        {render ? render(rating) : (asDisplayText(val) || <span style={{ color:'var(--text-3)' }}>-</span>)}
                      </div>
                    );
                  })}
                </div>
