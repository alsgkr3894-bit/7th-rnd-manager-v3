...
export function DayPanel({
  ...
  const future =
    safeDateKey && safeToday && !isPast(safeDateKey, safeToday) && !isToday(safeDateKey, safeToday);
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 14 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 3 }}>
            {safeDateKey && safeToday && isToday(safeDateKey, safeToday) ? (
              ...
              >{`테스트 ${safeNotes.length}건 · 일정 ${safeSchedules.length}건`}</span>
            )}
          </div>
        </div>
          ...
          {sortedSched.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {sortedSched.map((s, i) => {
                ...
                return (
                  <button
                    key={asDisplayText(s.id, `schedule-${i}`)}
                ...
                return (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      {noteType && (
                        <span style={{ fontSize: 10, color: 'var(--text-4)' }}>{noteType}</span>
                      )}
                    </div>
                    <div
                      style={{
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {safeSamples.map((s, i) => {
              ...
              return (
                <button
                  key={sampleId || `sample-${i}`}
                  onClick={() => {
                    if (sampleId) push(`/note/sample/${sampleId}`);
                  }}
                  style={{
                    display: 'flex',
                    flexDirection: 'column',
