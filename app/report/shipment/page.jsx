    ...
    <table className="paper-table">
      ...
      <tbody>
        {items.map((p, i) => {
          ...
          return (
            <tr
              key={p.productCode || p.productName}
              style={p.isManaged ? { background: 'var(--warn-soft)' } : undefined}
            >
                ...
                >
                  <div
                    style={{
                      width: `${pct}%`,
                      height: '100%',
                      background: p.isManaged ? '#D97706' : 'var(--accent)',
                      borderRadius: 2,
                      opacity: 0.6,
                    }}
                  />
...
export default function Page() {
    ...
    initDB()
      .then(async () => {
        try {
          ...
          // 선택 월의 모든 파일 행 합산
          const targetRows = (
            await Promise.all(targetMonth.files.map(f => getShipmentRowsByFileId(f.id)))
          ).flat();
          if (ignore) return;
          ...
          for (const rows of monthlyRows) {
            ...
            exclusiveData.push(
              a.filter(x => x.productType === 'exclusive').reduce((s, x) => s + x.totalQuantity, 0)
            );
            genericData.push(
              a.filter(x => x.productType !== 'exclusive').reduce((s, x) => s + x.totalQuantity, 0)
            );
  ...
  const notShipped = regProducts
    ...
    .sort((a, b) =>
      (a.normalizedProductName || a.productName || '').localeCompare(
        b.normalizedProductName || b.productName || '',
        'ko'
      )
    );
          ...
          {opts.chart && (
            <div className="paper-section">
              <div className="paper-section-title">
                월별 출고량 추이 (최근 {seriesLabels.length || 7}개월)
              </div>
