...
const STATUS_COLOR = { '인상': 'var(--negative)', '인하': 'var(--positive)', '신규': 'var(--accent)', '삭제': 'var(--text-4)' };
  ...
  useEffect(() => {
    ...
    initDB().then(async () => {
      try {
        ...
        if (files.length < 2) { setIsLoading(false); return; }
        ...
        if (periodMode === 'custom') {
          ...
          if (!latest || !base || latest.id === base.id) { setIsLoading(false); return; }
        } else {
          [latest, base] = sorted;
        }
        ...
        for (const c of filtered) {
          ...
          const entry = catMap.get(cat) || { cat, total: 0, up: 0, down: 0, newItem: 0, del: 0, sum: 0, count: 0 };
          entry.total++;
          const pct = Math.abs((c.changeRate || 0) * 100);
          if (c.changeStatus === '인상') { entry.up++;      entry.sum += pct; entry.count++; }
          if (c.changeStatus === '인하') { entry.down++;    entry.sum += pct; entry.count++; }
    ...
    <ReportBuilderShell
      ...
      options={<>
        <OptGroup label="대상 기간">
          ...
          {periodMode === 'custom' && (
            <div className="opt-period-row" style={{marginTop:8}}>
              ...
              <input type="date" className="input" value={customTo} min={customFrom}
        ...
        <OptGroup label="포함 섹션">
          ...
          <Check label="원가 영향 메뉴 수"   value={opts.costImpact} onChange={v=>upd('costImpact',v)}/>
        </OptGroup>
      ...
      preview={<>
        ...
        <div className="paper-stat-row">
          <div className="paper-stat">
            ...
            <div className="paper-stat-val num" style={{color: rising > 0 ? 'var(--negative)' : undefined}}>
              {rising}
            </div>
          </div>
          <div className="paper-stat">
            <div className="paper-stat-label">인하</div>
            <div className="paper-stat-val num" style={{color: falling > 0 ? 'var(--positive)' : undefined}}>
              {falling}
            </div>
          </div>
          <div className="paper-stat">
            <div className="paper-stat-label">신규</div>
            <div className="paper-stat-val num" style={{color: newItem > 0 ? 'var(--accent)' : undefined}}>
              {newItem}
            </div>
          </div>
          <div className="paper-stat">
              ...
              <tbody>
                {catSummary.map(c => (
                  <tr key={c.cat}>
                    ...
                    <td className="num right" style={{color:'var(--negative)'}}>{c.up || '—'}</td>
                    <td className="num right" style={{color:'var(--positive)'}}>{c.down || '—'}</td>
                    <td className="num right" style={{color:'var(--accent)'}}>{c.newItem || '—'}</td>
                    <td className="num right muted">{c.del || '—'}</td>
                    <td className="num right">{c.count > 0 ? `${(c.sum/c.count).toFixed(1)}%` : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {changes.length > 0 ? cats.map(cat => (
                  ...
                  return (
                    <tr key={`${c.productCode || c.productName}-${ci}`}>
                      ...
                      <td className="num right" style={{fontWeight:700}}>
                        {c.latestPrice != null ? fmtKRW(c.latestPrice) : (c.changeStatus === '신규' ? '신규' : '—')}
                      </td>
                      <td className="num right" style={{color: STATUS_COLOR[c.changeStatus]}}>
                        {isSpecial ? '—' : `${c.latestPrice - c.basePrice > 0 ? '+' : ''}${fmtKRW(c.latestPrice - c.basePrice)}`}
                      </td>
                      <td className="num right" style={{fontWeight:700, color: STATUS_COLOR[c.changeStatus]}}>
                        {isSpecial
          ...
          <div style={{height:60,display:'grid',placeItems:'center',color:'var(--text-4)',fontSize:13}}>
            {dateRange === '—' ? '가격 파일 2개 이상 업로드 후 비교할 수 있어요' : `임계값 ±${threshold}% 이상 변동 품목 없음`}
          </div>
        )}
