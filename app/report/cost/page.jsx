        ...
        try {
          ...
          for (const catLabel of CAT_KEYS) {
            ...
            const catPrices = prices.filter(p =>
              catLabel === '엣지' ? matchEdge(p.category) : p.category === catLabel
            );
            updated[meta.id] = {
              ...updated[meta.id],
      ...
      options={
        <>
          ...
          <OptGroup label="포함 섹션">
            ...
            />
            <Check
              label="카테고리별 메뉴 상세 (5개씩)"
              value={opts.perCategory}
              onChange={v => updOpt('perCategory', v)}
            />
            <Check
              label="판매가 컬럼 표시"
              value={opts.salePrice}
              onChange={v => updOpt('salePrice', v)}
            />
            <Check
                          ...
                          <div key={c.id} className="cost-bar-row">
                            ...
                            <div className="cost-bar-track">
                              <div
                                ...
                                style={{ width: `${Math.min(w, 100)}%`, background: c.color }}
                              ></div>
                              <div
