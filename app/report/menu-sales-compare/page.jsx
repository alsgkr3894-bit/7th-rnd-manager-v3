...
export default function Page() {
  ...
  const periodB = mode === 'custom' ? { year: yearB, month: monthB } : deriveCompareB(periodA, mode);
  ...
  useEffect(() => {
    ...
    initDB().then(async () => {
      try {
        ...
        if (rows.length === 0) { setIsLoading(false); return; }
  ...
  const compareRows = compareResult?.rows?.filter(r => !r.aIsZero && !r.bIsZero)
    .sort((a, b) => Math.abs(b.pct ?? 0) - Math.abs(a.pct ?? 0)).slice(0, 6) || [];

  const periodALabel = `${yearA}.${String(monthA).padStart(2,'0')}`;
  const periodBLabel = `${periodB.year}.${String(periodB.month).padStart(2,'0')}`;
  const reportMeta = { period: `${periodALabel} vs ${periodBLabel}`, name: `판매량 비교 보고서 — ${periodALabel} vs ${periodBLabel}`, pages: 7, options: { mode, scope, yearA, monthA, yearB, monthB, opts } };
    ...
    <ReportBuilderShell
      ...
      options={<>
        ...
        <OptGroup label="기간 A (기준)">
          <div className="opt-period-row">
            <select className="period-select num" value={yearA} onChange={e=>setYearA(parseInt(e.target.value))}>
              {[2024,2025,2026].map(y => <option key={y} value={y}>{y}년</option>)}
            </select>
            <select className="period-select num" value={monthA} onChange={e=>setMonthA(parseInt(e.target.value))}>
        ...
        {mode === 'custom' && (
          <OptGroup label="기간 B (비교)">
            <div className="opt-period-row">
              <select className="period-select num" value={yearB} onChange={e=>setYearB(parseInt(e.target.value))}>
                {[2024,2025,2026].map(y => <option key={y} value={y}>{y}년</option>)}
              </select>
              <select className="period-select num" value={monthB} onChange={e=>setMonthB(parseInt(e.target.value))}>
                {Array.from({length:12},(_,i)=>i+1).map(m => <option key={m} value={m}>{m}월</option>)}
              </select>
            </div>
          </OptGroup>
      ...
      preview={<>
        <div className="paper-head">
          ...
          <h2 className="paper-title">판매량 비교 보고서 — {periodALabel} vs {periodBLabel}</h2>
          <div className="paper-meta">
            <span>비교 모드: {mode === "mom" ? "전월 대비" : mode === "yoy" ? "전년 동월" : "사용자 지정"}</span>
            <span>·</span>
            <span>대상: {scope === "all" ? "전체" : scope === "pizza" ? "피자" : "사이드"}</span>
            <span>·</span>
            <span className="mono">생성일 {new Date().toLocaleDateString('ko-KR').slice(0,-1).replace(/\. /g,'.')} · {getProfile().name}</span>
          </div>
        </div>
        ...
        {opts.summary && (
          <div className="paper-stat-row">
            <div className="paper-stat">
              ...
              <div className="paper-stat-val num">{compareResult ? fmtShort(compareResult.totalA) : '—'}</div>
            </div>
            <div className="paper-stat">
              <div className="paper-stat-label">총 판매량 (B)</div>
              <div className="paper-stat-val num">{compareResult ? fmtShort(compareResult.totalB) : '—'}</div>
            </div>
            <div className="paper-stat">
              <div className="paper-stat-label">증감</div>
              <div className="paper-stat-val num" style={{color: (compareResult?.totalPct ?? 0) >= 0 ? 'var(--positive)' : 'var(--negative)'}}>
                {compareResult?.totalPct != null ? `${compareResult.totalPct >= 0 ? '+' : ''}${compareResult.totalPct.toFixed(1)}%` : '—'}
              </div>
            </div>
          </div>
        )}
        ...
        {opts.chart && (
          <div className="paper-section">
            ...
            <div style={{padding: "8px 0"}}>
              {series.length > 0 ? (
                <AreaChart series={series} labels={[]} colors={["#7C3AED","#3182F6"]} height={180} formatY={fmtShort}/>
              ) : (
                ...
                <tbody>
                  {compareRows.map(r => (
                    <tr key={r.name}>
                      ...
                      <td className="num right" style={{fontWeight:700}}>{fmtShort(r.b)}</td>
                      <td className="num right" style={{color: (r.pct??0)>=0 ? "var(--positive)" : "var(--negative)", fontWeight:700}}>
          ...
          <div className="paper-section">
            ...
            <div className="winners-grid">
              <div className="winner-col">
                ...
                {(compareResult.topRise || []).filter(r => (r.pct ?? 0) >= 10).map(r => (
                  <div className="winner-row" key={r.name}>
                    <span>{r.name}</span>
                    <b className="num" style={{color:"var(--positive)"}}>+{(r.pct??0).toFixed(1)}%</b>
                  </div>
                ))}
                {(compareResult.topRise || []).filter(r => (r.pct ?? 0) >= 10).length === 0 && (
                  <div className="muted" style={{fontSize:12}}>해당 없음</div>
                )}
              </div>
              <div className="winner-col">
                <div className="winner-h" style={{color:"var(--negative)"}}>▼ Losers</div>
                {(compareResult.topFall || []).filter(r => (r.pct ?? 0) <= -10).map(r => (
                  <div className="winner-row" key={r.name}>
                    <span>{r.name}</span>
                    <b className="num" style={{color:"var(--negative)"}}>{(r.pct??0).toFixed(1)}%</b>
                  </div>
                ))}
