...
export default function Page() {
  ...
  // 숨김 목록 복원 (마운트 1회)
  useEffect(() => {
    try { const raw = localStorage.getItem(KEYS.INGREDIENT_USAGE_HIDDEN); if (raw) setHidden(new Set(JSON.parse(raw))); } catch {}
  }, []);
  function toggleHidden(k) {
    setHidden(prev => {
      const next = new Set(prev);
      if (next.has(k)) next.delete(k); else next.add(k);
      try { localStorage.setItem(KEYS.INGREDIENT_USAGE_HIDDEN, JSON.stringify([...next])); } catch {}
      return next;
    });
  }
  ...
  const sorted = useMemo(() => {
    const arr = [...usageRows];
    if (sortKey === 'count') {
      arr.sort((a, b) => (sortDir === 'asc' ? a.count - b.count : b.count - a.count) || a.name.localeCompare(b.name, 'ko'));
    } else {
      arr.sort((a, b) => { const c = a.name.localeCompare(b.name, 'ko'); return sortDir === 'asc' ? c : -c; });
    }
    return arr;
  }, [usageRows, sortKey, sortDir]);

  const nonHidden = useMemo(() => usageRows.filter(r => !hidden.has(keyOf(r))), [usageRows, hidden]);
  ...
  const displayRows = useMemo(() => {
    ...
    arr = showHidden ? arr.filter(r => hidden.has(keyOf(r))) : arr.filter(r => !hidden.has(keyOf(r)));
    return arr;
        ...
        <>
          ...
          <div className="stat-row">
            <div className="stat-card">
              ...
              <div className="stat-value">{nonHidden.length}<span className="unit">개</span></div>
            </div>
            <div className="stat-card" style={{ cursor: 'pointer', outline: onlyOne ? '2px solid var(--warn)' : undefined }}
              onClick={() => setOnlyOne(v => !v)}>
              <div className="stat-label">1개 메뉴만 사용 ⚠</div>
              <div className="stat-value" style={{ color: oneCount > 0 ? 'var(--warn)' : undefined }}>{oneCount}<span className="unit">개</span></div>
              <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 6 }}>{onlyOne ? '필터 해제' : '클릭하여 보기'}</div>
            </div>
            <div className="stat-card">
              <div className="stat-label">미사용</div>
              <div className="stat-value" style={{ color: 'var(--text-3)' }}>{allMeta.length - totalUsedCount}<span className="unit">개</span></div>
            </div>
            <div className="stat-card" style={{ cursor: hiddenCount ? 'pointer' : 'default', outline: showHidden ? '2px solid var(--accent)' : undefined }}
              onClick={() => hiddenCount && setShowHidden(v => !v)}>
              <div className="stat-label">숨김</div>
              <div className="stat-value" style={{ color: hiddenCount > 0 ? 'var(--accent)' : undefined }}>{hiddenCount}<span className="unit">개</span></div>
              {hiddenCount > 0 && <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 6 }}>{showHidden ? '숨김 보기 끄기' : '숨김만 보기'}</div>}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', margin: '10px 0 8px' }}>
            <div style={{ display: 'flex', gap: 4 }}>
              {USAGE_CATS.map(c => (
                <button key={c} className={'chip' + (usageCat === c ? ' active' : '')} onClick={() => setUsageCat(c)}>{c}</button>
              ))}
            </div>
            <div style={{ marginLeft: 'auto', display: 'flex', gap: 6, alignItems: 'center' }}>
              <button className="btn sm" onClick={() => setExpanded(new Set(displayRows.map(keyOf)))}>모두 펼치기</button>
              <button className="btn sm" onClick={() => setExpanded(new Set())}>모두 접기</button>
              <button className="btn sm" onClick={() => printUsageReport(displayRows, usageCat)}>PDF 출력</button>
            </div>
          </div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 8 }}>
            ...
            {showHidden && <span style={{ marginLeft: 8, color: 'var(--accent)' }}>· 숨김 항목만 표시 중</span>}
            {onlyOne && <span style={{ marginLeft: 8, color: 'var(--warn)' }}>· 1개 사용만 표시 중</span>}
          </div>
          ...
          <div className="card table-card">
            {displayRows.length === 0 ? (
              <div style={{ padding: '40px 0', textAlign: 'center', color: 'var(--text-3)', fontSize: 13 }}>
                {showHidden ? '숨긴 재료가 없습니다.' : '등록된 레시피가 없거나 해당 조건에 맞는 재료가 없어요.'}
              </div>
            ) : (
              <table className="data-table">
                <thead>
                  <tr>
                    ...
                    <SortableTh sortKey="name" active={sortKey} dir={sortDir} onClick={handleSort}>식자재명</SortableTh>
                    <SortableTh sortKey="count" active={sortKey} dir={sortDir} onClick={handleSort} width={96}>사용 메뉴수</SortableTh>
                    <th>메뉴 목록</th>
                ...
                <tbody>
                  {displayRows.map((r, idx) => {
                    ...
                    const showTier = byCount && (idx === 0 || tierOf(displayRows[idx - 1].count) !== tier);
                    ...
                    return (
                      ...
                      {showTier && (
                        <tr>
                          <td colSpan={5} style={{ padding: '7px 12px', background: 'var(--surface-2)',
                            fontSize: 11, fontWeight: 700, color: 'var(--text-3)', borderTop: '1px solid var(--divider)' }}>
                            {TIER_LABEL[tier]}
                          </td>
                        </tr>
                      )}
                      <tr style={isHidden ? { opacity: .55 } : undefined}>
                          ...
                          <div style={{ fontWeight: 600, fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
                            ...
                            {r.scope && (
                              <span style={{ fontSize: 10, fontWeight: 700, padding: '1px 6px', borderRadius: 8,
                                color: sc?.color || 'var(--text-3)', background: sc?.bg || 'var(--surface-2)', whiteSpace: 'nowrap' }}>
                                {r.scope}
                              </span>
                            )}
                          </div>
                          {r.code && <div style={{ fontSize: 11, color: 'var(--text-4)', fontFamily: 'monospace' }}>{r.code}</div>}
                        </td>
                        <td style={{ textAlign: 'center' }}>
                          <span style={{ display: 'inline-block', minWidth: 32, padding: '2px 8px', borderRadius: 99, fontWeight: 700, fontSize: 13,
                            background: r.count >= 8 ? '#DBEAFE' : r.count >= 4 ? '#D1FAE5' : r.count === 1 ? 'var(--warn-soft)' : 'var(--surface-2)',
                            color:      r.count >= 8 ? '#1D4ED8' : r.count >= 4 ? '#065F46' : r.count === 1 ? 'var(--warn)' : 'var(--text-2)' }}>
                            {r.count}
                          </span>
                        </td>
                        <td>
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, alignItems: 'center' }}>
                            {visible.map(m => {
                              ...
                              return (
                                <span key={m.menuName} style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 99,
                                  background: cs.bg, color: cs.color, whiteSpace: 'nowrap' }}>{m.menuName}</span>
                              );
                            })}
                            {!open && more > 0 && (
                              <button onClick={() => toggle(k)} style={{ fontSize: 11, color: 'var(--accent)', border: 0, background: 'none', cursor: 'pointer', padding: '2px 4px', fontWeight: 600 }}>
                                +{more}개 더보기
                              </button>
                            )}
                            {open && r.menus.length > SHOW && (
                              <button onClick={() => toggle(k)} style={{ fontSize: 11, color: 'var(--text-3)', border: 0, background: 'none', cursor: 'pointer', padding: '2px 4px' }}>접기</button>
                            )}
                          </div>
                        </td>
                        <td style={{ textAlign: 'center' }}>
                          <button className="btn sm" style={{ fontSize: 10, padding: '2px 6px', color: isHidden ? 'var(--accent)' : 'var(--text-3)' }}
                            title={isHidden ? '표시' : '숨김(목록·출력 제외)'} onClick={() => toggleHidden(k)}>
                            {isHidden ? '표시' : '숨김'}
                          </button>
                        </td>
                      </tr>
            ...
            <div style={{ padding: '8px 16px', fontSize: 11, color: 'var(--text-3)', borderTop: '1px solid var(--divider)' }}>
              {displayRows.length}개 표시 · {usageCat !== '전체' ? `${usageCat} 필터 중` : '전체 카테고리'}{hiddenCount > 0 ? ` · 숨김 ${hiddenCount}개` : ''}
            </div>
          </div>
        </>
      )}
