...
import {
  getSetRecipeMap, upsertSetRecipe, setTotalCost,
...
export default function Page() {
  ...
  const load = useCallback(async () => {
    ...
    setMenus(allMenus.filter(m => m.category === '세트박스'));
    setRecipeMap(recMap);
  }, []);
  ...
  async function handleSave(data) {
    try {
      const result = await upsertSetRecipe(data);
      const msg = result.mode === 'insert' ? '레시피 등록 완료' :
                  !data.id ? '기존 레시피 덮어씌움' : '레시피 수정 완료';
      showToast(msg, 'ok');
      setTarget(null);
      const newMap = await getSetRecipeMap();
      setRecipeMap(newMap);
    } catch (err) {
  ...
  const summaryRows = useMemo(() => menus.map(m => {
    ...
    const cost = setTotalCost(recipe);
  ...
  const sub = loading
    ? '로딩 중…'
    : menus.length === 0
      ? '메뉴 판매가에 등록된 세트박스가 없습니다'
      : `세트박스 ${menus.length}개 · 레시피 ${stats.withRecipe}건 작성됨`;

  if (dbError) return (
    <main className="main">
      <PageHeader breadcrumb={['원가계산', '세트박스']} title="세트박스 원가" sub="로드 실패"/>
          ...
          {!loading && menus.length === 0 && (
            <div className="card" style={{minHeight:200, display:'grid', placeItems:'center'}}>
              <div style={{textAlign:'center', color:'var(--text-3)'}}>
                ...
                <div style={{fontWeight:600, marginBottom:4}}>세트박스 메뉴가 없습니다</div>
