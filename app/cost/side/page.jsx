...
import {
  getSideRecipeMap, upsertSideRecipe, sideTotalCost,
...
export default function Page() {
  ...
  const load = useCallback(async () => {
    ...
    setMenus(allMenus.filter(m => m.category === '사이드'));
    setRecipeMap(recMap);
  }, []);
  ...
  async function handleSave(data) {
    try {
      const result = await upsertSideRecipe(data);
      const msg = result.mode === 'insert' ? '레시피 등록 완료' :
                  !data.id ? '기존 레시피 덮어씌움' : '레시피 수정 완료';
      showToast(msg, 'ok');
      setTarget(null);
      const newMap = await getSideRecipeMap();
      setRecipeMap(newMap);
    } catch (err) {
  ...
  const summaryRows = useMemo(() => menus.map(m => {
    ...
    const cost = sideTotalCost(recipe);
  ...
  const sub = loading
    ? '로딩 중…'
    : menus.length === 0
      ? '메뉴 판매가에 등록된 사이드 메뉴가 없습니다'
      : `사이드 ${menus.length}개 · 레시피 ${stats.withRecipe}건 작성됨`;

  if (dbError) return (
    <main className="main">
      <PageHeader breadcrumb={['원가계산', '사이드']} title="사이드 메뉴 원가" sub="로드 실패"/>
