...
import { getAllEdges as getAllCostEdges, edgeTotalCost } from '@/lib/cost/edge-dough';
...
export const MENU_RECIPE_SUMMARY_STATUS = {
  ...
  // 엣지(카테고리 '엣지')는 menu_recipes 레시피가 아니라 cost_edge_dough(엣지 관리)의
  // 사이즈별 구성품 원가를 그대로 보여준다 — summarizeMenuEdge 참고.
  EDGE: 'edge',
};

/**
 * 카테고리 '엣지' 메뉴마스터 행의 원가 요약. cost_edge_dough(엣지 관리)에서 같은 패밀리의
 * L/R 원가를 가져와 보여준다 — 레시피 미지원(UNSUPPORTED)이던 것을 실제 원가로 바꾼다.
 * 석쇠(costEdgeType null)는 별도 엣지 원가가 없는 기본 도우라 0원으로 READY 취급.
 *
 * @param {{ category?: string, menuName?: string, edgeKey?: string, price?: number }} menu
 * @param {object[]} edges - getAllEdges() (cost_edge_dough)
 * @param {Map} [unitPriceMap] - 있으면 저장된 단가 대신 최신 제때 단가로 다시 계산
 * @returns {object|null} EDGE 상태 요약, 엣지 카테고리가 아니면 null
 */
export function summarizeMenuEdge(menu, edges, unitPriceMap) {
  ...
  if (family.costEdgeType) {
    for (const edge of asObjectArray(edges)) {
      if (edge.edgeType !== family.costEdgeType || !edge.size) continue;
      sizeCosts[edge.size] = Math.round(edgeTotalCost(edge, unitPriceMap));
    }
  }
  // 치즈크러스트·골드스윗은 L/R 행이 따로 있어(edge-size-split.js) 그 행의 사이즈 원가만 쓴다.
  // 사이즈가 없는 행(석쇠·씬바사삭)은 종전처럼 L(유일한 사이즈)을 대표값으로 쓴다.
  const size = menu?.size === 'L' || menu?.size === 'R' ? menu.size : null;
  const totalCost = (size ? sizeCosts[size] : sizeCosts.L) ?? (family.costEdgeType ? null : 0);
  const sellingPrice = asFiniteNumber(menu?.price, null);
  const costRate = totalCost != null ? calcMarginRate(totalCost, sellingPrice) : null;

  return {
    kind: 'edge',
    status: MENU_RECIPE_SUMMARY_STATUS.EDGE,
    hasRecipe: totalCost != null,
    edgeFamilyKey: family.key,
    size,
    sizeCosts,
...
function countMissingRecipeComponentInputs(components, unitPriceMap, options = {}) {
  const requirePositiveQuantity = options.requirePositiveQuantity === true;
  ...
  for (const component of components) {
    ...
    if (quantity == null || (requirePositiveQuantity && quantity <= 0)) {
...
export function summarizeMenuRecipe(menu, recipe, unitPriceMap = new Map(), options = {}) {
  ...
  if (!kind) {
    const edgeSummary = summarizeMenuEdge(menu, options.edges, unitPriceMap);
    if (edgeSummary) return edgeSummary;
    return {
      kind: null,
  ...
  const costableDirectComponents = directComponents.map(component => {
    const quantity = asFiniteNumber(component?.quantity, null);
    return quantity != null && quantity > 0 ? component : { ...component, quantity: null };
  });
  const components = [...costableDirectComponents, ...commonComponents];
  ...
  const directCost = Math.round(effectiveComponentsRawCost(costableDirectComponents, unitPriceMap));
...
export async function loadMenuRecipeSummaryMap(menus) {
  const [recipeMaps, unitPriceMap, recipeGroups, edges] = await Promise.all([
    loadMenuRecipeMaps(),
    loadLatestUnitPriceMap(),
    getAllRecipeGroups(),
    getAllCostEdges(),
  ]);
  return buildMenuRecipeSummaryMap(menus, recipeMaps, unitPriceMap, { recipeGroups, edges });
}
