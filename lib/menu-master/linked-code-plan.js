 ...
 */
// identity: { updated: Record<storeName, count> } — menuCode는 그대로인데 menuName/category만
// 바뀌어 nutrition_menu_ref 등에 새로고침한 건수(collectIdentitySyncRows 결과). 둘 다 없을 수도,
// 코드 변경과 동시에 이름도 바뀌어 둘 다 있을 수도 있다.
export function formatMenuCodeCascadeSummary(cascade, identity = null) {
  const identityParts = Object.entries(identity?.updated || {})
    .filter(([, count]) => count > 0)
    .map(([storeName, count]) => `${LINKED_STORE_LABELS[storeName] || storeName} ${count}건`);
  const identitySuffix = identityParts.length ? ` · 메뉴명 동기화: ${identityParts.join('·')}` : '';

  if (!cascade) return identityParts.length ? `저장 완료${identitySuffix}` : '저장 완료';
  ...
  if (skipped?.nutrition === 'sibling') {
    return `${base} · 영양성분은 다른 규격이 같은 코드를 써서 유지${identitySuffix}`;
  }
  return `${base}${identitySuffix}`;
}
