...
function rowDisplayNames(row) {
  ...
  // L/R 행("치즈크러스트 L", "패밀리박스 R")은 판매량 화면에 사이즈 없는 이름으로 나오므로
  // 사이즈 접미사를 뗀 이름도 같은 메뉴로 본다 — 사이즈 표기가 행의 size와 일치할 때만.
  const size = String(row?.size || '')
    .trim()
    .toUpperCase();
  if ((size === 'L' || size === 'R') && normalized.toUpperCase().endsWith(` ${size}`)) {
    const base = normalized.slice(0, -2).trim();
...
export function buildDiscontinuedMenuNameSet(menuMasterRows) {
  const discontinued = collectNormalizedMenuNames(
    menuMasterRows,
    row => row?.status === 'discontinued'
  );
  // 같은 이름으로 아직 팔리는 행이 있으면(예: L만 단종, R은 판매 중) 그 이름은 단종이 아니다.
  const stillActive = collectNormalizedMenuNames(
    menuMasterRows,
    row => row?.status !== 'discontinued'
  );
  for (const name of stillActive) discontinued.delete(name);
  return discontinued;
}
