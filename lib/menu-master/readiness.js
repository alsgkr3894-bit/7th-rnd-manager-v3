...
import { buildNutritionOutputCoverage } from './readiness-coverage';
import { buildNutritionLinkedMenuCodeSetFrom, isMenuNutritionLinked } from './readiness-nutrition';
import { asFiniteNumber, asObjectArray } from '@/lib/ui/prop-guards';

export { buildNutritionLinkedMenuCodeSet, isMenuNutritionLinked } from './readiness-nutrition';
...
/** 판매가 */
function checkPrice(menuCode, priceMap, isEdge) {
  ...
  // 엣지(석쇠·씬바사삭)는 추가 요금 0원이 정상이다 — 다른 카테고리와 같은 기준으로
  // "0 이하는 확인 필요"로 잡으면 항상 경고가 뜬다.
  if (isEdge && price === 0) return { status: 'ok', detail: '추가 요금 없음' };
...
function checkRecipe(menuCode, recipeSummaryMap) {
  ...
  // 엣지는 식자재 레시피가 아니라 공통원가관리(엣지 관리)의 사이즈별 구성품 원가를 그대로
  // 쓴다 — 안내 문구도 "레시피 구성품 없음"이 아니라 실제로 고칠 수 있는 화면을 가리켜야 한다.
  if (status === MENU_RECIPE_SUMMARY_STATUS.EDGE && !hasRecipe)
    return { status: 'missing', detail: '엣지 관리(공통 원가 관리)에 원가 구성이 없습니다' };
...
export async function buildMenuReadinessMap(menus, recipeSummaryMap = new Map()) {
  ...
  // 목록 화면의 "영양 연동" 칩(isMenuNutritionLinked)과 같은 기준 — raw_values 코드
  // 정확 일치만 보던 좁은 판정 탓에 토핑명·엣지 패밀리로만 연동된 메뉴(추가토핑 전체,
  // 치즈크러스트·골드스윗 등)가 출시 준비 탭에서 "미작성"으로 잘못 표시되던 문제 수정.
  const nutritionLinkedSet = buildNutritionLinkedMenuCodeSetFrom({
    rawValues,
    toppings,
    nutritionEdges,
  });
  ...
  for (const menu of menus) {
    ...
    const price = checkPrice(mc, priceMap, Boolean(edgeFamily));
    const recipe = checkRecipe(mc, recipeSummaryMap);
    const costRate = checkCostRate(recipe, price);
    const nutrition = isMenuNutritionLinked(menu, nutritionLinkedSet)
      ? edgeFamily
        ? { status: 'ok', detail: '엣지 기준 영양정보 연동' }
        : { status: 'ok' }
      : { status: 'missing', detail: '영양성분 값 미입력' };
    // OPT-EDGE 행은 메뉴마스터의 카탈로그 항목일 뿐, 원산지·알레르기는 실제 피자 메뉴
    // 행에서 엣지가 반영돼 함께 출력된다 — 엣지 코드로는 원산지/알레르기 행이 생길 수
    // 없어 '누락'으로 잡히는 것을 막는다.
    const origin = edgeFamily
      ? { status: 'unknown', detail: '피자 행에서 함께 출력' }
      : checkOrigin(mc, originReadyCodes);
    const allergen = edgeFamily
      ? { status: 'unknown', detail: '피자 행에서 함께 출력' }
      : checkAllergen(mc, allergenReadyCodes);
