...
import {
  checkNutrition,
  computeEdgeNutritionLinkedFamilyKeys,
  buildNutritionLinkedMenuCodeSet,
  isMenuNutritionLinked,
} from './readiness-nutrition';
...
export async function buildMenuReadinessMap(menus, recipeSummaryMap = new Map()) {
  ...
  const edgeNutritionLinkedFamilyKeys = computeEdgeNutritionLinkedFamilyKeys(
    rawValues,
    nutritionEdges
  );
  ...
  for (const menu of menus) {
    ...
    const nutritionByCode = checkNutrition(mc, rawValueMenuCodes);
    const nutrition =
      nutritionByCode.status !== 'ok' &&
      edgeFamily &&
      edgeNutritionLinkedFamilyKeys.has(edgeFamily.key)
