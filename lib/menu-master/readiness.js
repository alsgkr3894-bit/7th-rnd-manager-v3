 ...
 *
 * 추가토핑(menu_master의 T-* 메뉴)은 실제 출력 시 nutrition_topping_master를
 * 이름으로 매칭한다(lib/nutrition/label/menu-preview.js와 같은 규칙) — 코드
...
export async function buildNutritionLinkedMenuCodeSet() {
  ...
  const codes = asObjectArray(rawValues)
    .map(r => r.menuCode)
    .filter(Boolean);
  const toppingNames = asObjectArray(toppings)
    .map(t => t.toppingName)
    .filter(Boolean);
  return new Set([...codes, ...toppingNames]);
}

export function isMenuNutritionLinked(menu, linkedSet) {
  if (checkNutrition(menu?.menuCode, linkedSet).status === 'ok') return true;
  return Boolean(menu?.menuName && linkedSet.has(menu.menuName));
}
