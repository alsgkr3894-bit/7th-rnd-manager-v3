...
import { resolveMenuEdgeFamily } from './edge-family.js';
...
/**
 * 엣지(석쇠·씬바사삭)는 추가 요금 0원이 정상이라, 다른 카테고리처럼 "0 이하는 누락"으로
 * 잡으면 항상 오탐이 뜬다 — readiness.js checkPrice와 같은 예외를 여기서도 반영한다.
 */
function isMissingSellingPrice(menu, price, readiness) {
  if (price == null) return true;
  if (price <= 0) {
    if (price === 0 && resolveMenuEdgeFamily(menu))
      return isMissingReadinessDim(readiness, 'price');
    return true;
  }
  return isMissingReadinessDim(readiness, 'price');
}
...
export function buildMenuDataQualityReport(
  ...
  safeMenus.forEach((menu, index) => {
    ...
    if (isMissingSellingPrice(menu, price, readiness)) {
      addIssue(
        issues,
        QUALITY_KINDS.MISSING_SELLING_PRICE,
        menu,
