 ...
 * 수 있도록 분리했다.
 */
export function buildNutritionLinkedMenuCodeSetFrom({ rawValues, toppings, nutritionEdges }) {
  ...
  return new Set([...codes, ...toppingNameKeys, ...edgeFamilyKeys]);
}

export async function buildNutritionLinkedMenuCodeSet() {
  const [rawValues, toppings, nutritionEdges] = await Promise.all([
  ...
  return buildNutritionLinkedMenuCodeSetFrom({ rawValues, toppings, nutritionEdges });
}
