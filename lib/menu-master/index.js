...
export async function syncMenuMasterFromPrices(priceRows, options = {}) {
  ...
  // priceRows에서 사라진(코드가 더 이상 없는) 기존 마스터는 판매가가 없어졌다는 뜻이므로
  // 옛 price를 그대로 두면 원가율/마진 계산이 존재하지 않는 가격을 기준으로 틀어진다.
  for (const [code, prev] of masterByCode) {
    if (deduped.has(code)) continue;
    if (optionalFinitePrice(prev.price) == null) continue;
    priceUpdated++;
    toUpsert.push({ ...prev, price: null, updatedAt: now });
  }
