...
function buildRecord(data) {
  ...
  // outputMenuCode: 같은 메뉴의 재료 변형(매장별 면 종류 등)일 때 출력(영양성분표·알레르기·
  // 원산지)에서 합쳐질 대표 메뉴 코드 — lib/menu-master/output-alias.js. 자기 자신은 무효.
  if (data.outputMenuCode !== undefined) {
    const target = String(data.outputMenuCode || '').trim();
