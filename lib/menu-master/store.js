...
import { deleteMenuRecipeByMenuCode } from '@/lib/menu-recipes/store';
...
export async function deleteMenuMaster(id) {
  ...
  if (!menuCode) return { cascadeErrors };
  ...
  if (hasStore('menu_recipes')) {
    await deleteMenuRecipeByMenuCode(menuCode);
  }

  // cascade: 구형 원가 레시피 (cost_recipes)
  ...
  try {
    ...
    await nutritionStore.deleteMenuRefsByMenuCode(menuCode);
  } catch (err) {
    cascadeErrors.push({
      step: 'nutritionMenuRefs',
      message: err?.message || String(err),
    });
    console.warn('[menu-master/store] deleteMenuMaster cascade 일부 실패', cascadeErrors);
  }
