...
/**
 * 1인피자 코드 정규화 — menu_master + cost_selling_prices + menu_recipes 대상.
 * @returns {Promise<{ changed: number }>}
 */
export async function normalizePersonalPizzaCodes() {
  const stores = ['menu_master', 'cost_selling_prices', 'menu_recipes'];
  let changed = 0;
...
function codeDerivedPatch(row, { includeSubCategory = true } = {}) {
  const parsed = parseCategoryFromCode(row?.menuCode);
  const patch = {};
  if (parsed.category && row.category !== parsed.category) {
    patch.category = parsed.category;
  }
  if (includeSubCategory && parsed.subCategory && row.subCategory !== parsed.subCategory) {
    patch.subCategory = parsed.subCategory;
  }
  return patch;
}

async function normalizeCodeDerivedCategoryStore(storeName, options = {}) {
  if (!hasStore(storeName)) return { updated: 0 };
  const all = await getAll(storeName);
  const toPut = [];
  for (const row of all) {
    const patch = codeDerivedPatch(row, options);
    if (Object.keys(patch).length === 0) continue;
    toPut.push({ ...row, ...patch });
  }

  if (toPut.length === 0) return { updated: 0 };

  await runTransaction([storeName], 'readwrite', tx => {
    const store = tx.objectStore(storeName);
    for (const row of toPut) store.put(row);
  });
  return { updated: toPut.length };
}

export async function normalizeMenuCodeCategories() {
  const menuMaster = await normalizeCodeDerivedCategoryStore('menu_master', {
    includeSubCategory: true,
  });
  const sellingPrices = await normalizeCodeDerivedCategoryStore('cost_selling_prices', {
    includeSubCategory: false,
  });
  return { changed: menuMaster.updated + sellingPrices.updated };
}
