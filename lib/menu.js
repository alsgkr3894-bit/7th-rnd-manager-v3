 ...
 *
 * 8개 섹션(개발 메모 / RND / 보고서센터 / 상품 관리 / 메뉴 / 원가·영양 / 판매 관리 / 시스템)으로 그룹화.
...
import { filterRoleVisibleGroups, isRoleItemVisible } from '@/lib/navigation/role-visibility';
    ...
    groups: [
      {
        ...
        iconKey: 'beaker',
        children: [
          { id: 'rnd-journal', label: '연구일지', href: '/note/journal' },
          { id: 'rnd-market', label: '시장조사', href: '/note/market' },
          {
            id: 'rnd-corporate-card',
            label: '법인카드 내역서',
            href: '/rnd/corporate-card',
            requiresEdit: true,
          },
          {
            id: 'rnd-login-info',
    ...
    groups: [
      {
        ...
        children: [
          ...
          // requiresEdit 없음 — 이 화면의 주 사용자가 viewer(LAN 조회 PC)다.
          { id: 'settings-sync', label: '서버 데이터 불러오기', href: '/settings/sync' },
          {
            id: 'settings-restore',
            label: '데이터 복원',
...
export function isNavItemVisibleForRole(item, canEdit = false) {
  return isRoleItemVisible(item, canEdit);
}

export function filterNavSectionsForRole(sections = NAV_SECTIONS, canEdit = false) {
  return (Array.isArray(sections) ? sections : [])
    .map(section => {
      const groups = filterRoleVisibleGroups(section?.groups, canEdit, 'children');
