...
const MOBILE_PRIMARY_ROUTE_ORDER = Object.freeze([
  '/',
  '/note',
  '/report',
  '/cost/margin',
...
export const ROUTE_CLASSIFICATIONS = Object.freeze([
  ...
  { route: '/cost/topping', kind: ROUTE_KIND.STATIC_DETAIL, runtime: [MAIN] },
  {
    route: '/cost/margin',
    kind: ROUTE_KIND.SIDEBAR,
    markers: [ROUTE_MARKER.MOBILE_PRIMARY],
  ...
  {
    route: '/cost/ingredient-price',
    kind: ROUTE_KIND.REDIRECT,
    target: '/ingredient/manage?view=price',
    runtime: [MAIN],
  },
  ...
  { route: '/note/write', kind: ROUTE_KIND.INTERNAL_FLOW, runtime: [MAIN, CHINA4_DIRECT] },
  { route: '/note/[id]', kind: ROUTE_KIND.DYNAMIC_DETAIL },
  { route: '/note/board', kind: ROUTE_KIND.INTERNAL_FLOW, runtime: [MAIN] },
  ...
  { route: '/note/sample', kind: ROUTE_KIND.INTERNAL_FLOW, runtime: [MAIN, CHINA4_DIRECT] },
  { route: '/note/sample/write', kind: ROUTE_KIND.INTERNAL_FLOW, runtime: [MAIN] },
  { route: '/note/sample/[id]', kind: ROUTE_KIND.DYNAMIC_DETAIL },
  { route: '/note/market', kind: ROUTE_KIND.SIDEBAR, runtime: [MAIN] },

  { route: '/rnd/corporate-card', kind: ROUTE_KIND.SIDEBAR, runtime: [MAIN] },
  { route: '/rnd/login-info', kind: ROUTE_KIND.SIDEBAR, runtime: [MAIN] },
  ...
  { route: '/settings/sync', kind: ROUTE_KIND.SIDEBAR, runtime: [MAIN, CHINA4] },
  { route: '/settings/sample-recovery', kind: ROUTE_KIND.INTERNAL_FLOW, runtime: [MAIN] },
]);
...
export function routesForMarker(marker) {
  const routes = ROUTE_CLASSIFICATIONS.filter(item => item.markers?.includes(marker)).map(
    item => item.route
  );

  if (marker !== ROUTE_MARKER.MOBILE_PRIMARY) return routes;

  const order = new Map(MOBILE_PRIMARY_ROUTE_ORDER.map((route, index) => [route, index]));
  return routes
    .slice()
    .sort(
      (a, b) =>
