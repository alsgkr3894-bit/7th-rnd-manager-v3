...
function sampleSortKey(sample) {
  // testDate(날짜만 'YYYY-MM-DD')와 updatedAt/createdAt(전체 ISO)이 섞이면 같은 날 기록의
  // 정렬이 뒤틀리므로 날짜 부분(앞 10자)만으로 비교한다.
  return cleanText(sample?.testDate || sample?.updatedAt || sample?.createdAt).slice(0, 10);
}
