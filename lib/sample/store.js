...
export async function updateSample(id, data) {
  ...
  logWork('SAMPLE_SAVE', data.title || data.menuName || '샘플 수정', { ref: id }).catch(e => console.warn('[sample/store] logWork 실패', e));
}
...
function normalizeSampleNames(arr, legacyMenuName) {
  if (Array.isArray(arr)) {
    const names = arr.map(s => (s || '').trim()).filter(Boolean);
    if (names.length) return names;
  }
  const mn = (legacyMenuName || '').trim();
  return mn ? [mn] : [];
}
...
export function sampleNamesOf(rec) {
  if (Array.isArray(rec?.sampleNames) && rec.sampleNames.length) {
    return rec.sampleNames.map(s => (s || '').trim()).filter(Boolean);
  }
  const mn = (rec?.menuName || '').trim();
  return mn ? [mn] : [];
}
