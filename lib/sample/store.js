...
export async function addSample(data) {
  await initSharedDB();
  await assertActiveAdmin('샘플 기록 추가');
...
export async function updateSample(id, data) {
  await initSharedDB();
  await assertActiveAdmin('샘플 기록 수정');
  if (!hasStore(STORE)) throw new Error(`${STORE} store를 찾을 수 없습니다`);
...
export async function deleteSample(id) {
  await initSharedDB();
  await assertActiveAdmin('샘플 기록 삭제');
  if (!hasStore(STORE)) throw new Error(`${STORE} store를 찾을 수 없습니다`);
...
function buildRecord(data, existing = {}) {
  ...
  return {
    brand: (asDisplayText(data.brand || existing.brand || activeBrandId()) || 'main').trim(),
