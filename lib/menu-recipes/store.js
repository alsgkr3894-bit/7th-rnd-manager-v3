 ...
 * 카테고리별 detail store와 구형 cost_recipes는 v22부터 schema에서 제거한다.
 */
...
export function normalizeSelectedRecipeGroupIds(groupIds) {
  return [
    ...new Set((Array.isArray(groupIds) ? groupIds : []).map(value => text(value)).filter(Boolean)),
  ];
}
...
export function buildMenuRecipeRecord(data, existing = null) {
  ...
  return {
    ...
    selectedRecipeGroupIds: normalizeSelectedRecipeGroupIds(
      data?.selectedRecipeGroupIds ?? existing?.selectedRecipeGroupIds
    ),
...
export async function upsertMenuRecipe(data) {
  await assertActiveAdmin('메뉴 레시피 저장');
...
export async function deleteMenuRecipe(id) {
  await assertActiveAdmin('메뉴 레시피 삭제');
  if (!hasStore(MENU_RECIPES_STORE)) throw new Error(`${MENU_RECIPES_STORE} store 없음`);
  await runTransaction([MENU_RECIPES_STORE], 'readwrite', tx => {
...
export async function resetAllMenuRecipes() {
  await assertActiveAdmin('레시피 전체 초기화');
