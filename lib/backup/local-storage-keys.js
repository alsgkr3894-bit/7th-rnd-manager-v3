...
export function pickLocalStorageForScopes(map, scopes = []) {
  ...
  for (const [key, value] of Object.entries(map)) {
    if (
      (allowed.has(key) || (allowAccountKeys && isActiveAccountStorageKey(key))) &&
      typeof value === 'string'
    ) {
      picked[key] = value;
    }
...
function collectDynamicAccountKeys(out) {
  ...
  for (let i = 0; i < length; i += 1) {
    ...
    if (!isActiveAccountStorageKey(key)) continue;
    try {
...
export function collectLocalStorage(keys = PERSISTENT_LS_KEYS) {
  ...
  if (allowAccountKeys) collectDynamicAccountKeys(out);
  return out;
}
...
export function restoreLocalStorage(map, keys = PERSISTENT_LS_KEYS, options = {}) {
  ...
  for (const [k, v] of Object.entries(map)) {
    if (
      !(allow.has(k) || (allowAccountKeys && isActiveAccountStorageKey(k))) ||
      typeof v !== 'string'
    ) {
      continue;
    }
