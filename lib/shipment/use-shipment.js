...
export function useJetteShipment() {
  ...
  async function handleFile(file, period) {
    ...
    try {
      ...
      const fileBuffer = ext === '.csv' ? null : await file.arrayBuffer();
      const fileText   = ext === '.csv' ? await file.text() : null;
      const parsed = ext === '.csv'
        ? readCsvFile(fileText)
        : await readExcelFile(fileBuffer);

      // 파일 해시 계산 (중복 업로드 차단용)
      const rawBytes = fileBuffer ?? new TextEncoder().encode(fileText);
