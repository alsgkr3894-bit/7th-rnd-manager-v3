...
const REQUIRED_LABELS = { productCode: '제품코드', productName: '제품명', quantity: '배송수량', amount: '합계' };
...
export function parseShipmentRows(headers, rows) {
  const headerMap = {};
  headers.forEach(h => { headerMap[normH(h)] = h; });
  ...
  rows.forEach((row, idx) => {
    ...
    if (!qtyRes.ok) {
      failed.push({ rowIndex: idx + 2, productCode, productName, reason: '배송수량 인식 실패', raw: String(row[colMap.quantity] ?? '') });
      return;
    }
    if (!amtRes.ok) {
      failed.push({ rowIndex: idx + 2, productCode, productName, reason: '합계 인식 실패', raw: String(row[colMap.amount] ?? '') });
      return;
    }
    ...
    const priceWithTax = taxType === '과세' ? Math.round(unitPrice * 1.1) : unitPrice;
    ...
    success.push({
      ...
      taxType,
      quantity:    qtyRes.val,
      unitPrice, vatAmount, priceWithTax,
      amount:      amtRes.val,
...
export function filterTargetRows(rows, managedProducts) {
  ...
  const byNorm = new Map(managedProducts.filter(p => p.normalizedProductName || p.productName).map(p => [
    p.normalizedProductName || normalizeProductName(p.productName),
    p,
  ]));
