...
import { normalizeManagedProductRecord } from '../jette/product-types.js';
 ...
 * @returns {Array} aggRows — productType: 'exclusive' | 'generic', isManaged: boolean
 */

export function aggregateShipmentRows(rows, managedProducts = [], priceLookup = null) {
  const safeRows = asObjectArray(rows);
  const safeManagedProducts = asObjectArray(managedProducts).map(normalizeManagedProductRecord);
  ...
  for (const [, g] of groups) {
    ...
    const normalizedManagedProduct = normalizeManagedProductRecord(mp);
    const productType = normalizedManagedProduct.productType;
    // 관리품목 체크 — 신규 isManaged 우선, 구 isSevenManaged fallback
    const isManaged =
      normalizedManagedProduct.isManaged ||
      (productType === 'generic' && Boolean(mp?.isSevenManaged));
