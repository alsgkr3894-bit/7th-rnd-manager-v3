...
export function aggregateShipmentRows(rows, managedProducts = [], priceLookup = null) {
  ...
  const byNorm = new Map(
    safeManagedProducts
      ...
      .map(p => [
        asDisplayText(p.normalizedProductName) || normalizeProductName(asDisplayText(p.productName)),
        p,
      ])
  );
  ...
  for (const [, g] of groups) {
    ...
    const priceWithTax = safePriceLookup && g.productCode
  ...
  result.sort(
    (a, b) =>
      (asFiniteNumber(b.totalAmount, 0) ?? 0) - (asFiniteNumber(a.totalAmount, 0) ?? 0)
  );
  return result;
}
