...
export async function readSpreadsheetFile(file) {
  ...
  throw new Error('지원하지 않는 파일 형식입니다. (.csv, .xlsx, .xls만 허용)');
}
 ...
 */
export function readSpreadsheetFromBuffer(buffer, fileName) {
  const name = String(fileName || '').toLowerCase();
  const dotIndex = name.lastIndexOf('.');
  const ext = dotIndex >= 0 ? name.slice(dotIndex) : '';
  if (ext === '.csv') return readCsvFile(new TextDecoder('utf-8').decode(buffer));
