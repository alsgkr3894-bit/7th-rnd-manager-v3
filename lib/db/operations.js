...
export async function deleteFileWithLog(fileStore, rowStore, fileId) {
  const rowIds = hasStore(rowStore)
    ...
    : [];
  // upload_log는 linkedFileId 인덱스가 있으므로 getByIndex 사용 (getAll 스캔 불필요)
  const logIds = hasStore('upload_log')
    ? (await getByIndex('upload_log', 'linkedFileId', fileId)).map(l => l.id)
    : [];
...
export async function exportSelected(storeNames, meta = {}) {
  ...
  const validNames = storeNames.filter(name => {
 ...
 *
 * 전략: clearStore는 순차 실행(트랜잭션 충돌 방지), bulkPut은 5개씩 병렬 실행(성능).
 *
...
export async function importAll(data) {
  ...
  // 2단계: clearStore 순차 실행 (IndexedDB 트랜잭션 충돌 방지)
  ...
  // 3단계: bulkPut 5개씩 병렬 실행 (clear 실패 store는 건너뜀)
  const failedClears = new Set(errors.map(e => e.store));
  const putEntries = entries.filter(([name]) => !failedClears.has(name));
  const CHUNK = 5;
  for (let i = 0; i < putEntries.length; i += CHUNK) {
    const chunk = putEntries.slice(i, i + CHUNK);
    await Promise.all(chunk.map(async ([storeName, rows]) => {
      try {
        if (Array.isArray(rows) && rows.length > 0) {
          const validRows = rows.filter(r => r !== null && typeof r === 'object');
          if (validRows.length !== rows.length) {
            console.warn('[importAll]', storeName, 'skipped', rows.length - validRows.length, 'invalid rows');
          }
          await bulkPut(storeName, validRows);
        }
        imported++;
      } catch (err) {
        errors.push({ store: storeName, error: err?.message || String(err) });
