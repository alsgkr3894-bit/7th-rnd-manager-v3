  ...
  for (const [storeName, rows] of Object.entries(data.stores)) {
    ...
    try {
      ...
      if (Array.isArray(rows) && rows.length > 0) {
        const validRows = (Array.isArray(rows) ? rows : []).filter(r => r !== null && typeof r === 'object');
        if (validRows.length !== rows.length) console.warn('[importAll]', storeName, 'skipped', rows.length - validRows.length, 'invalid rows');
