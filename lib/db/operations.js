...
/**
 * store를 비우고 새 레코드로 교체 — clear + put을 단일 트랜잭션에서 수행한다.
 *
 * 단일 트랜잭션이므로 put 중 하나라도 실패하면 clear까지 롤백되어
 * "비워졌지만 채워지지 않은" 상태(데이터 손실)가 발생하지 않는다.
 * 복원(importAll)처럼 원자성이 중요한 경로에서 사용.
 *
 * @param {string} storeName
 * @param {object[]} rows
 * @returns {Promise<number>} 저장된 레코드 수
 */
export function replaceStore(storeName, rows) {
...
export async function importAll(data) {
  ...
  // 2단계: store별 clear+put을 단일 트랜잭션으로 교체 (원자성)
  // 트랜잭션 충돌 방지를 위해 순차 실행한다.
