...
/** @private DB 핸들을 지정해 store 전체 조회 */
...
/** @private main DB가 열려 있지 않으면 오픈 (공유 store 접근 전 호출) */
...
/** @private 공유 store → main DB, 나머지 → 활성 브랜드 DB */
function dbForStore(name) {
  return SHARED_STORE_NAMES.has(name) ? getNamed(dbNameFor('main')) : _getDB();
}
...
/**
 * store를 비우고 새 레코드로 교체 — 단일 트랜잭션 (완전 원자성).
 * IndexedDB는 대기 중인 요청이 있는 한 트랜잭션을 유지하므로 수만 건도 원자적.
 * 실패 시 tx.onabort로 clear까지 롤백. 복원 직전 autoBackup이 추가 보호.
 *
 * 공유 store(notes 모듈)는 main DB, 나머지는 활성 브랜드 DB에 기록.
 *
 * @param {string} storeName
 * @param {object[]} rows
 * @returns {Promise<number>} 저장된 레코드 수
...
export async function replaceStore(storeName, rows) {
  const valid = Array.isArray(rows) ? rows.filter(r => r !== null && typeof r === 'object') : [];
  if (SHARED_STORE_NAMES.has(storeName)) await ensureMainDB();
  const db = dbForStore(storeName);
  return new Promise((resolve, reject) => {
...
export async function exportSelected(storeNames, meta = {}, options = {}) {
  // 공유 store가 포함된 경우 main DB를 미리 오픈
  ...
  const activeStoreNames = new Set(Array.from(activeDb.objectStoreNames));
...
export async function importAll(data, options = {}) {
  ...
  // 영양성분 출력 설정(localStorage) 복원 — nutrition 스토어가 포함된 경우에만
  const hasNutritionStores = Object.keys(data.stores).some(n => n.startsWith('nutrition_'));
  if (data.localStorage && hasNutritionStores) {
    try {
