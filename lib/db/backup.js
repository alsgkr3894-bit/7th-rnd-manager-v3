...
// 스키마에서 제거된 구버전 store — 복원 시 조용히 건너뛰지 않고 사용자에게 알린다.
const LEGACY_REMOVED_STORES = new Set([
  'cost_recipes',
  'cost_pizza_detail',
...
// 실패로 보지 않는 정보성 skip 마커 (의도된 건너뜀).
const INFO_SKIP_STORES = new Set(['__shared_skipped__', '__legacy_skipped__']);
...
/**
 * store에 레코드를 써 넣는다 — 단일 트랜잭션.
 *
 * @param {IDBDatabase} db
 * @param {Array<[string, object[]]>} entries [storeName, rows] 쌍
 * @param {{ clear?: boolean, syncToServer?: boolean, brandId?: string,
 *           onStoreQueued?: (storeName: string) => void }} [options]
 *   - `clear`(기본 true): 쓰기 전에 store를 비운다. 서버에서 여러 페이지를 이어 받을 때는
 *     첫 페이지만 true로 주고 이후엔 false로 이어 붙인다.
 *   - `syncToServer`(기본 true): false면 이 쓰기를 서버로 되밀지 않는다. 서버에서 내려받은
 *     데이터를 기록할 때 반드시 false여야 푸시 피드백 루프가 생기지 않는다.
 */
export function writeStoresInDbTransaction(db, entries, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    tx.oncomplete = () => {
      // 서버 동기화 태그는 반드시 "쓰기가 실제로 들어간" 브랜드(options.brandId)로 붙여야 한다.
      // 그렇지 않으면 활성 브랜드로 잘못 태깅돼 다른 브랜드의 서버 데이터를 clear+덮어쓴다.
      queueCapturedStoreSync(captures, { includeClientId: false, brandId: options.brandId });
      resolve(counts);
    };
 ...
 * writeStoresInDbTransaction의 clear:true 래퍼(복원 경로가 쓰는 기존 이름/시그니처 유지).
 */
export function replaceStoresInDbTransaction(db, entries, options = {}) {
  return writeStoresInDbTransaction(db, entries, { ...options, clear: true });
}
...
export async function replaceStoreForBrand(storeName, rows, brandId) {
  ...
  const brandIdForSync = SHARED_STORE_NAMES.has(storeName) ? undefined : brandId;
  const counts = await replaceStoresInDbTransaction(db, [[storeName, rows]], {
    brandId: brandIdForSync,
  });
...
async function replaceStoreGroupForBrand(entries, brandId, options = {}) {
  ...
  // 공유 store는 main DB에 살므로 서버 태그도 'main' 고정(server-sync가 처리).
  // 브랜드 스코프 store는 실제 기록된 대상 brandId로 태깅되도록 넘긴다.
  const brandIdForSync = shared ? undefined : brandId;
  return replaceStoresInDbTransaction(db, pairs, { ...options, brandId: brandIdForSync });
}
...
export async function exportSelectedForBrand(brandId, storeNames, meta = {}, options = {}) {
  // 로그인정보·법인카드 등 민감 store가 포함될 수 있는 유일한 내보내기 경로.
  // 호출부(브랜드 백업/복원 전 자동백업)는 이미 admin을 확인하지만, 방어의 마지막 층으로 여기서도 막는다.
  await assertActiveAdmin('데이터 내보내기');
...
function finalRestoreStatus(errors) {
  const realErrors = errors.filter(error => !INFO_SKIP_STORES.has(error.store));
...
export async function importAllToBrand(data, brandId, options = {}) {
  ...
  const entries = Object.entries(data.stores).filter(([storeName, rows]) => {
    if (!ALL_STORES.includes(storeName)) {
      ...
      skipped++;
      // 구버전 백업에 담긴 제거된 store(레거시 레시피·영양 참조 등)에 실제 데이터가 있으면
      // 조용히 건너뛰지 않고 "복원되지 않았다"는 사실을 사용자에게 명확히 알린다.
      if (LEGACY_REMOVED_STORES.has(storeName) && Array.isArray(rows) && rows.length > 0) {
        errors.push({
          store: '__legacy_skipped__',
          error: `구버전 store '${storeName}' ${rows.length}건은 현재 스키마에 없어 복원되지 않았습니다. 이 데이터가 필요하면 해당 구버전 앱에서 먼저 최신 형식으로 마이그레이션한 뒤 백업하세요.`,
        });
      }
      return false;
    }
  ...
  // __shared_skipped__/__legacy_skipped__는 의도된 건너뜀(정보성)이라 실패로 보지 않는다.
  let hasRealError = errors.some(e => !INFO_SKIP_STORES.has(e.store));
  ...
  hasRealError = errors.some(e => !INFO_SKIP_STORES.has(e.store));
  if (!hasRealError && options.restoreLocalStorage && data.localStorage) {
    const localStorageErrors = [];
    // 브랜드 레지스트리는 병합(복원 이후 새로 만든 브랜드 보존), 활성 계정 키는 복원하지 않아
    // 특정 모듈만 복원해도 브랜드가 사라지거나 로그인 계정이 바뀌지 않도록 한다.
    restoreLocalStorage(data.localStorage, undefined, {
      mergeBrandMaster: true,
      skipAccountKeys: true,
