...
export function openNamed(name) {
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    request.onblocked = () => {
      ...
      // 다른 탭/창에서 동일 DB의 구 버전 연결을 닫지 않으면 영원히 블록됨.
      // 즉시 reject 해서 호출부(useCurrentRole, useDBLoad)가 viewer로 fallback하도록 한다.
      handles.delete(name);
      reject(new Error(`[DB] ${name} 업그레이드 차단됨 — 다른 창에서 DB를 닫아주세요.`));
    };
  });
...
/** 캐시된 연결을 닫고 핸들을 제거. deleteDatabase 등에서 stale 닫힌 연결이 캐시에 남지 않도록 호출. */
export function forgetHandle(name) {
