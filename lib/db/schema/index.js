...
export function createStores(idb, oldVersion = 0, upgradeTx = null) {
  ...
  // v27 마이그레이션: menu_recipe_versions 스토어 추가 (createMenuMasterStores에서 자동 처리)
  // — 레시피 저장 시점 스냅샷 이력, append-only(삭제 없음)라 기존 데이터 손실 없음.

  // v28 마이그레이션: ref_registered_overrides 스토어 추가 (createSalesStores에서 자동 처리)
  // — 판매량 보고서 "미등록" 자동 판정을 사용자가 개별로 해제할 수 있게 하는 목록. 신규
  //   store라 기존 데이터 영향 없음.

  // v29 마이그레이션: 실질적인 스키마 변경은 없다 — v28 배포 직후 개발 중 핫리로드 타이밍에
  // "DB는 v28로 기록됐지만 ref_registered_overrides store는 없는" 상태로 굳어진 브라우저가
  // 있어(IndexedDB는 기록된 버전 이상으로만 onupgradeneeded를 다시 실행한다), 버전만 한 번 더
  // 올려 createSalesStores의 idempotent 가드(contains 체크)가 다시 돌게 한다.
