 ...
 * ref_sales_categories, ref_sales_aliases, ref_excluded, ref_discontinued,
 * ref_registered_overrides, ref_event_menus
 */
...
export function createSalesStores(idb) {
  ...
  if (!idb.objectStoreNames.contains('ref_registered_overrides')) {
    const s = idb.createObjectStore('ref_registered_overrides', {
      keyPath: 'id',
      autoIncrement: true,
    });
    s.createIndex('menuName', 'menuName');
