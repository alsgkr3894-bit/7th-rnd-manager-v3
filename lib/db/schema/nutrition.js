 ...
 * nutrition_raw_values          — 베이스 영양성분값 (석쇠L/R, 씬바사삭L, per menu)
 * nutrition_allergy_links       — 메뉴별 알레르기 체크 (menuCode × allergenCode)
...
export function createNutritionStores(idb) {
  ...
  if (!idb.objectStoreNames.contains('nutrition_ingredient_values')) {
    const s = idb.createObjectStore('nutrition_ingredient_values', { keyPath: 'id', autoIncrement: true });
