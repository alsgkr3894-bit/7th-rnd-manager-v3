...
import { isSyncGuardBlocked, noteSyncGuardHeld } from './sync-guard';
...
function enqueue(operation) {
  ...
  // 로컬이 비어 있는데 서버엔 데이터가 있는 authoritative 브라우저 — 서버 행을 id
  // 충돌로 덮어쓸 위험이 있어 "서버에서 불러오기" 전까지 푸시를 보류한다(lib/db/
  // sync-guard.js). 로컬 IndexedDB 쓰기는 이미 끝난 뒤라 화면 동작은 그대로다.
