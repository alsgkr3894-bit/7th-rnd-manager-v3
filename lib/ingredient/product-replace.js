...
/**
 * photos는 배열이 아니라 {packaging,detail,actual} 슬롯 객체라 preferNew의 hasValue가
 * "슬롯이 전부 비어 있는 객체"도 값 있음으로 오판한다(normalizeIngredientPhotos가 항상
 * non-null 객체를 반환하므로). 슬롯 단위로 새 항목에 값이 있으면 새 값, 없으면 기존 값을
 * 쓴다 — 대체 대상에 사진이 없을 때 기존 항목 사진이 사라지지 않게 한다.
 */
function mergePhotos(oldRow, newRow) {
  const oldPhotos = normalizeIngredientPhotos(oldRow?.photos, oldRow?.photo);
  if (!newRow) return oldPhotos;
  const newPhotos = normalizeIngredientPhotos(newRow?.photos, newRow?.photo);
  const merged = {};
  for (const key of Object.keys(oldPhotos)) {
    merged[key] = newPhotos[key] || oldPhotos[key] || null;
  }
  return merged;
}
...
function mergeIngredientRecord(oldRow, newRow, nextCode, nextName, now) {
  ...
  const photos = mergePhotos(oldRow, newRow);
  return {
    ...base,
