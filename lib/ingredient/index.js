/** origin 값을 [{displayName,country}] 배열로 정규화 (구버전 객체·null 모두 처리) */
function normalizeOriginArr(v) {
  if (!v) return null;
  if (Array.isArray(v)) {
    const items = v.filter(it => it.country?.trim());
    return items.length ? items : null;
  }
  return v.country?.trim() ? [{ displayName: v.displayName || '', country: v.country }] : null;
}

/** 분류명 → CSS variable 기반 칩 색상 (8색 순환, 다크모드 자동 대응) */
export function getCategoryStyle(category) {
  if (!category) return {};
  let h = 0;
  for (let i = 0; i < category.length; i++) h = (h * 31 + category.charCodeAt(i)) % 8;
  const n = h + 1;
  return { background: `var(--cat-${n}-bg)`, color: `var(--cat-${n}-text)` };
}

/** 메인 카테고리 우선순위 — 분류 칩 정렬용 */
const MAIN_CATEGORY_PRIORITY = [
  '엣지',
  '도우/밀가루',
  '토핑재료',
  '치즈류',
  '사이드',
  '소스류',
  '스파게티',
  '떡볶이',
  '시즈닝',
  '스톡',
  '피클재료',
];

/** 분류 정렬 — 메인 카테고리 우선순위 + 가나다순 */
export function sortMainCategories(cats) {
  if (!Array.isArray(cats) || cats.length <= 1) return cats || [];
  const rank = t => {
    const i = MAIN_CATEGORY_PRIORITY.indexOf(t);
    return i === -1 ? 999 : i;
  };
  return [...cats].sort((a, b) => {
    const ra = rank(a),
      rb = rank(b);
    if (ra !== rb) return ra - rb;
    return a.localeCompare(b, 'ko');
  });
}

/** 해시태그 정렬 — 가나다순 */
export function sortHashTags(tags) {
  if (!Array.isArray(tags) || tags.length <= 1) return tags || [];
  return [...tags].sort((a, b) => a.localeCompare(b, 'ko'));
}

export {
  getAllIngredients,
  getIngredientMetaMap,
  addIngredient,
  updateIngredient,
  setIngredientPriceManualConfirmed,
  setIngredientPriceChangeAcked,
  setIngredientPriceChangeAckedMany,
  upsertIngredientMeta,
  previewIngredientDelete,
  deleteIngredient,
  bulkDeleteIngredients,
  excludeIngredientByCode,
  restoreIngredientByCode,
  bulkImportIngredients,
  buildIngredientProductCodeDuplicateDiagnostics,
  getIngredientProductCodeDuplicateDiagnostics,
  repairIngredientProductCodeDuplicates,
  resetAllIngredients,
  removeCategoryFromAll,
  removeTagFromAll,
  removeManyTagsFromAll,
  renameCategoryInAll,
  renameTagInAll,
  bulkSetDiscontinued,
  bulkSetCategory,
  bulkSetOriginAllergenNone,
  getIngredientHealthSummary,
  replaceIngredientProductCode,
  previewIngredientProductReplace,
} from './store';

export { SEED_MAIN_CATEGORIES, SEED_HASH_TAGS } from './master-seed';
export {
  INGREDIENT_PHOTO_KEYS,
  INGREDIENT_PHOTO_SLOTS,
  countIngredientPhotos,
  getIngredientPhoto,
  getPrimaryIngredientPhoto,
  normalizeIngredientPhoto,
  normalizeIngredientPhotos,
} from './photos';

import { simplifyIngredientName } from '@/lib/normalize';
import { SCOPE_UNASSIGNED } from './constants';
import { getProductTypeLabel, normalizeProductType } from '@/lib/jette/product-types';
import { getPrimaryIngredientPhoto, normalizeIngredientPhotos } from './photos';
import { calcUnitPrice } from '@/lib/cost/calc-unit-price';
import { normalizeCostBaseUnit } from '@/lib/cost/unit-policy';

/** 제때 관리품목 배열 → productCode → productType 맵 (전용/범용 단일 출처) */
export function buildProductTypeMap(managed) {
  return new Map(
    (managed || [])
      .filter(p => p.productCode)
      .map(p => [p.productCode, normalizeProductType(p.productType)])
  );
}

/** productType 맵에서 productCode의 전용/범용 라벨을 구한다. 없으면 미지정. */
export function scopeLabelFor(typeMap, productCode) {
  return typeMap && typeMap.has(productCode)
    ? getProductTypeLabel(typeMap.get(productCode), true)
    : SCOPE_UNASSIGNED;
}

/**
 * price_rows + cost_ingredients 메타를 합쳐 화면용 행 반환
 *
 * @param {object[]} priceRows
 * @param {Map} metaMap  productCode → cost_ingredients 레코드
 * @param {Map} [typeMap] productCode → ref_shipment_products.productType
 *   ('exclusive'|'generic'). 제때 연동 항목의 전용/범용 단일 출처.
 */
export function mergeIngredientRows(priceRows, metaMap, typeMap = null) {
  return priceRows.map(p => {
    const meta = metaMap.get(p.productCode) || {};
    const hasRecord = metaMap.has(p.productCode);
    const category = meta.category || (Array.isArray(meta.categories) && meta.categories[0]) || '';
    const tags =
      Array.isArray(meta.tags) && meta.tags.length
        ? meta.tags
        : Array.isArray(meta.categories)
          ? meta.categories.slice(1)
          : [];
    const baseQty = meta.baseQuantity ?? null;
    const unitType = normalizeCostBaseUnit(meta.baseUnitType ?? p.salesUnit);
    const unitPrice = calcUnitPrice(p.priceWithTax, baseQty);
    return {
      id: meta.id ?? null,
      productCode: p.productCode,
      productName: p.productName,
      displayName: simplifyIngredientName(p.productName),
      ingredientName: meta.ingredientName || '',
      temperature: p.temperature,
      salesUnit: p.salesUnit,
      taxType: p.taxType,
      price: p.price,
      priceWithTax: p.priceWithTax,
      priceOverride: meta.priceOverride ?? null,
      productStatus: p.productStatus,
      // 전용/범용 단일 출처 = 제때 관리품목(productType). 없으면 미지정.
      scope: scopeLabelFor(typeMap, p.productCode),
      category,
      tags,
      manufacturer: meta.manufacturer || '',
      photos: normalizeIngredientPhotos(meta.photos, meta.photo),
      photo: getPrimaryIngredientPhoto(meta),
      discontinued: meta.discontinued === true,
      baseQuantity: baseQty,
      baseUnitType: unitType,
      pieceWeightGrams: meta.pieceWeightGrams ?? null,
      note: meta.note || '',
      unitPrice,
      jetteLinked: true,
      excluded: meta.excluded === true,
      hasRecord,
      isSeeded: meta.isSeeded === true,
      updatedAt: meta.updatedAt || null,
      origin: normalizeOriginArr(meta.origin),
      allergens: Array.isArray(meta.allergens) ? meta.allergens : [],
      originHidden: meta.originHidden === true,
      originNone: meta.originNone === true,
      allergenNone: meta.allergenNone === true,
      priceManualConfirmed: meta.priceManualConfirmed === true,
      priceChangeAckedPrice: meta.priceChangeAckedPrice ?? null,
      priceChangeAckedAt: meta.priceChangeAckedAt || null,
      replacedByProductCode: meta.replacedByProductCode || null,
      replacedFromProductCode: meta.replacedFromProductCode || null,
    };
  });
}

/**
 * 가격파일에 없는 메타(시드/수동) 행을 mergedRow 형태로 변환.
 * list·manage 페이지 공통 사용.
 */
export function buildMetaOnlyRow(m, { jetteMissing = false } = {}) {
  const baseQty = m.baseQuantity ?? null;
  const unitType = normalizeCostBaseUnit(m.baseUnitType);
  const unitPrice = calcUnitPrice(m.priceOverride, baseQty);
  const category = m.category || (Array.isArray(m.categories) && m.categories[0]) || '';
  const tags =
    Array.isArray(m.tags) && m.tags.length
      ? m.tags
      : Array.isArray(m.categories)
        ? m.categories.slice(1)
        : [];
  return {
    id: m.id,
    productCode: m.productCode || null,
    productName: m.ingredientName,
    displayName: m.ingredientName,
    ingredientName: m.ingredientName,
    temperature: m.temperature || null,
    salesUnit: null,
    taxType: m.taxType || '과세',
    price: m.priceOverride,
    priceWithTax: m.priceOverride,
    priceOverride: m.priceOverride ?? null,
    productStatus: null,
    scope: m.scope || SCOPE_UNASSIGNED, // 코드없는 항목: 지정값 또는 미지정
    category,
    tags,
    manufacturer: m.manufacturer || '',
    photos: normalizeIngredientPhotos(m.photos, m.photo),
    photo: getPrimaryIngredientPhoto(m),
    discontinued: m.discontinued === true,
    baseQuantity: baseQty,
    baseUnitType: unitType,
    pieceWeightGrams: m.pieceWeightGrams ?? null,
    note: m.note || '',
    unitPrice,
    jetteLinked: false,
    excluded: m.excluded === true,
    hasRecord: true,
    isManual: m.isManual === true,
    isSeeded: m.isSeeded === true,
    updatedAt: m.updatedAt || null,
    origin: normalizeOriginArr(m.origin),
    allergens: Array.isArray(m.allergens) ? m.allergens : [],
    originHidden: m.originHidden === true,
    originNone: m.originNone === true,
    allergenNone: m.allergenNone === true,
    priceManualConfirmed: m.priceManualConfirmed === true,
    priceChangeAckedPrice: m.priceChangeAckedPrice ?? null,
    priceChangeAckedAt: m.priceChangeAckedAt || null,
    replacedByProductCode: m.replacedByProductCode || null,
    replacedFromProductCode: m.replacedFromProductCode || null,
    // 제때 코드가 있는데(과거 연동 이력) 최신 가격파일에는 없는 행 — 목록에서 그냥 사라지면
    // "대체 연결" 버튼에 닿을 방법이 없어진다(useIngredientManageData.js가 표시해 준다).
    jetteMissing: jetteMissing === true,
  };
}

/** priceChangeAckedPrice(확인 시점 단가)와 현재 단가가 사실상 같은지 — 부동소수 오차 허용. */
function isSamePrice(a, b) {
  if (a == null || b == null) return false;
  return Math.abs(a - b) < 0.005;
}

/**
 * 식자재 이슈 추출 — 미분류 / 포장수량없음 / 단가미연동 / 단가변동
 *
 * @param {object[]} rows  - mergeIngredientRows + buildMetaOnlyRow로 만든 행
 * @param {Map<string, {priceWithTax:number}>} [prevPriceMap] - 이전 가격파일 productCode → priceWithTax (선택)
 * @returns {object[]} 이슈가 있는 행에 issues:string[]와 priceDiff 정보가 추가됨
 */
export function computeIngredientIssues(rows, prevPriceMap) {
  const list = [];
  for (const r of rows) {
    if (r.discontinued || r.excluded) continue;
    const issues = [];
    let priceDiff = null;
    if (!r.category) issues.push('uncategorized');
    if (!r.baseQuantity) issues.push('no-unit');
    if (r.hasRecord && !r.jetteLinked && !r.priceManualConfirmed) issues.push('no-price-link');
    // 코드없는(비연동) 항목인데 전용/범용이 미지정
    if (!r.jetteLinked && (!r.scope || r.scope === SCOPE_UNASSIGNED)) issues.push('no-scope');
    // 등록된(hasRecord) 품목만 원산지/알레르기 미표기를 검사한다 — 제때 신규 미등록 품목은
    // 별도 패널(제때 신규 미등록)에서 다루므로 여기서 원산지/알레르기를 강제하지 않는다.
    if (r.hasRecord) {
      if (!r.originNone && !r.originHidden && (!Array.isArray(r.origin) || r.origin.length === 0)) {
        issues.push('missing-origin');
      }
      if (!r.allergenNone && (!Array.isArray(r.allergens) || r.allergens.length === 0)) {
        issues.push('missing-allergen');
      }
    }
    if (prevPriceMap && r.productCode && r.priceWithTax != null) {
      const prev = prevPriceMap.get(r.productCode);
      if (
        prev != null &&
        prev !== r.priceWithTax &&
        !isSamePrice(r.priceChangeAckedPrice, r.priceWithTax)
      ) {
        const diff = r.priceWithTax - prev;
        const pct = (diff / prev) * 100;
        issues.push(diff > 0 ? 'price-up' : 'price-down');
        priceDiff = { oldPrice: prev, newPrice: r.priceWithTax, diff, pct };
      }
    }
    if (issues.length) list.push({ ...r, issues, priceDiff });
  }
  return list;
}
