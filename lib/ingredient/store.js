...
export async function upsertIngredientMeta({ productCode, ...patch }) {
  ...
  const record = {
    ...
    origin:   patch.origin   !== undefined ? (patch.origin   ?? null)  : (existing?.origin   ?? null),
...
function buildRecord(data) {
  ...
  return {
    ...
    // 원산지·알레르기 (기존 레코드 호환: 없으면 null/[])
    origin:   data.origin   ?? null,   // { displayName, country, region } | null
