...
function normalizePhoto(v) {
  if (!v || typeof v !== 'object' || Array.isArray(v) || !v.data) return null;
  return {
    data: String(v.data),
    name: String(v.name || ''),
  };
...
export async function upsertIngredientMeta({ productCode, ...patch }) {
  ...
  const record = {
    ...
    photo:
      patch.photo !== undefined ? normalizePhoto(patch.photo) : normalizePhoto(existing?.photo),
...
export async function bulkImportIngredients(items) {
  ...
  const records = items.map(it => {
    ...
    const record = {
      ...
      photo: normalizePhoto(it.photo ?? existing?.photo),
...
export async function seedMasterIngredients(items) {
  ...
  const records = items.map(it => {
    ...
    const base = {
      ...
      note: existing?.note || '',
      photo: normalizePhoto(existing?.photo),
      isManual: false,
      isSeeded: true,
      updatedAt: now,
    };
