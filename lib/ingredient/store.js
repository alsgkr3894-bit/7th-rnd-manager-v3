...
export async function addIngredient(data) {
  ...
  logWork('INGREDIENT_SAVE', data.ingredientName || '식자재 등록').catch(e => console.warn('[ingredient/store] logWork 실패', e));
}
...
export async function updateIngredient(id, data) {
  ...
  logWork('INGREDIENT_SAVE', data.ingredientName || existing.ingredientName || '식자재 수정', { ref: id }).catch(e => console.warn('[ingredient/store] logWork 실패', e));
}
...
export async function upsertIngredientMeta({ productCode, ...patch }) {
  ...
  const record = {
    ...
    priceOverride: patch.priceOverride != null && patch.priceOverride !== ''
    ...
    temperature:  patch.temperature  !== undefined ? ((patch.temperature || '').trim() || null) : (existing?.temperature  ?? null),
    originHidden: patch.originHidden !== undefined ? patch.originHidden === true              : (existing?.originHidden ?? false),
    ...
    origin:   patch.origin   !== undefined ? normalizeOrigin(patch.origin) : normalizeOrigin(existing?.origin),
    allergens: patch.allergens !== undefined ? (Array.isArray(patch.allergens) ? [...patch.allergens] : []) : (existing?.allergens ?? []),
...
export async function deleteIngredient(id) {
  ...
  logWork('DELETE', `식자재 삭제: ${target?.ingredientName || target?.productName || ''}`, { ref: id }).catch(e => console.warn('[ingredient/store] logWork 실패', e));
  return target ?? null;
}
