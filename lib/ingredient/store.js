...
import {
  ...
  findIngredientsByProductCode,
  assertUniqueProductCode,
  buildIngredientProductCodeDuplicateDiagnostics,
} from './product-code';

// 공개 API 호환: 외부(UI·테스트)가 store.js에서 import하던 함수를 재노출한다.
...
// compositeOf 참조 검증 → ./composite-refs, productCode 식별·중복 진단 → ./product-code
// (둘 다 상단에서 import + 공개 함수 re-export)
  ...
  const record = {
    ...
    baseQuantity:
      patch.baseQuantity != null
        ? (Number.isFinite(Number(patch.baseQuantity)) ? Number(patch.baseQuantity) : null)
    ...
    priceOverride:
      patch.priceOverride != null && patch.priceOverride !== ''
        ? (Number.isFinite(Number(patch.priceOverride)) ? Number(patch.priceOverride) : null)
