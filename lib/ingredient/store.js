...
export function buildIngredientProductCodeDuplicateDiagnostics(rows = [], nutritionValues = []) {
  ...
  const groups = [...recordsByProductCode(rows).entries()]
    ...
    .map(([key, items]) => {
      ...
      return {
        ...
        removeNames: remove.map(
          row => row.ingredientName || row.productName || row.displayName || ''
        ),
 ...
 * @returns {Promise<{removed: Array<{ingredient: object, nutritionSnapshot: object|null, cascadeErrors?: Array<object>}>, failures: Array<{id: number, message: string}>}>}
 */
export async function bulkDeleteIngredients(ids) {
  ...
  for (const id of ids) {
    try {
      ...
      console.warn('[ingredient/store] bulkDeleteIngredients 항목 삭제 실패', { id, error: err });
    }
  }
 ...
 * @param {number} id
 * @returns {Promise<{ingredient: object, nutritionSnapshot: object|null, cascadeErrors: Array<object>}|null>}
 */
export async function deleteIngredient(id) {
  ...
  // cascade 삭제 전 영양값 스냅샷 — undo 시 복원 가능하도록
  let nutritionSnapshot = null;
  if (target.productCode) {
    try {
      const nutritionStore = await import('@/lib/nutrition/values/store');
      nutritionSnapshot = await nutritionStore.getIngredientValueByCode(target.productCode);
    } catch (err) {
      cascadeErrors.push({
        step: 'nutritionSnapshot',
  ...
  if (target.productCode) {
    ...
    try {
      const nutritionStore = await import('@/lib/nutrition/values/store');
      await nutritionStore.deleteIngredientValueByCode(target.productCode);
    } catch (err) {
      cascadeErrors.push({
        step: 'nutritionIngredientValue',
        message: err?.message || String(err),
      });
    }
  }
  // 알레르기 링크 cascade — nutrition_allergy_links는 CL2 이후 사실상 빈 legacy store
  try {
  ...
  if (cascadeErrors.length > 0) {
    console.warn('[ingredient/store] deleteIngredient cascade 일부 실패', cascadeErrors);
  }

  logWork('DELETE', `식자재 삭제: ${target.ingredientName || target.productName || ''}`, {
    ref: id,
  }).catch(e => console.warn('[ingredient/store] logWork 실패', e));
  return { ingredient: target, nutritionSnapshot, cascadeErrors };
}
