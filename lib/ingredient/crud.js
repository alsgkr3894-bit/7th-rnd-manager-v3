...
/**
 * "단가 인상/인하" 이슈를 확인 처리한다 — 확인 시점의 priceWithTax를 레코드에 저장해두고,
 * computeIngredientIssues가 그 값과 현재 priceWithTax가 같으면 이슈에서 제외한다.
 * 다음 단가파일에서 또 값이 바뀌면 조건이 깨져 자동으로 다시 표시된다(영구 확인이 아님).
 * setIngredientPriceManualConfirmed와 같은 패턴 — buildRecord 화이트리스트를 거치지 않고
 * 해당 항목만 직접 patch한다.
 */
export async function setIngredientPriceChangeAcked(id, price) {
  await assertActiveAdmin('단가 변동 확인');
  if (!hasStore('cost_ingredients')) throw new Error('cost_ingredients store 없음');
  const all = await getAll('cost_ingredients');
  const existing = all.find(r => r.id === id);
  if (!existing) throw new Error('항목을 찾을 수 없습니다');
  ...
  await runTransaction(['cost_ingredients'], 'readwrite', tx => {
    store(tx).put({
      ...existing,
      priceChangeAckedPrice: Number.isFinite(Number(price)) ? Number(price) : null,
      priceChangeAckedAt: now,
      updatedAt: now,
    });
  });
  logWork('INGREDIENT_SAVE', `${existing.ingredientName || '식자재'} 단가 변동 확인`, {
    ref: id,
  }).catch(e => console.warn('[ingredient/crud] logWork 실패', e));
}

/** 여러 건을 한 번에 확인 처리 — 단일 트랜잭션. */
export async function setIngredientPriceChangeAckedMany(entries) {
  await assertActiveAdmin('단가 변동 일괄 확인');
  if (!hasStore('cost_ingredients')) throw new Error('cost_ingredients store 없음');
  const safeEntries = (Array.isArray(entries) ? entries : []).filter(
    e => e && Number.isFinite(Number(e.id))
  );
  if (safeEntries.length === 0) return { acked: 0 };
  const all = await getAll('cost_ingredients');
  ...
  await runTransaction(['cost_ingredients'], 'readwrite', tx => {
    ...
    for (const entry of safeEntries) {
      ...
      s.put({
        ...existing,
        priceChangeAckedPrice: Number.isFinite(Number(entry.price)) ? Number(entry.price) : null,
        priceChangeAckedAt: now,
        updatedAt: now,
      });
      acked++;
  ...
  logWork('INGREDIENT_SAVE', `단가 변동 일괄 확인 (${acked}건)`).catch(e =>
    console.warn('[ingredient/crud] logWork 실패', e)
  );
  return { acked };
}

export async function upsertIngredientMeta({ productCode: rawProductCode, ...patch }) {
  await assertActiveAdmin('식자재 메타 저장');
  if (!hasStore('cost_ingredients')) throw new Error('cost_ingredients store 없음');
  // buildRecord와 같은 규칙 — 저장값은 제때 코드 표기(대문자)로 통일한다.
  ...
  const record = {
    ...
    // baseQuantity/pieceWeightGrams는 "빈 값으로 지우기"가 정상 동작이다 — 폼이 필드를 비우면
    // null을 명시적으로 보낸다. `!= null`로 걸러내면 그 null이 existing 값으로 되돌아가
    // 포장수량/1개당 g을 지울 수 없게 된다. patch에 키가 아예 없을 때만(undefined) 기존값을 쓴다.
    // (Number(null) === 0은 유한값이라 곧장 Number.isFinite로 넘기면 null이 0으로 바뀐다 — 먼저 걸러낸다.)
    baseQuantity:
      patch.baseQuantity !== undefined
        ? patch.baseQuantity === null || patch.baseQuantity === ''
          ? null
    ...
    pieceWeightGrams:
      patch.pieceWeightGrams !== undefined
        ? patch.pieceWeightGrams === null || patch.pieceWeightGrams === ''
          ? null
          : Number.isFinite(Number(patch.pieceWeightGrams))
            ? Number(patch.pieceWeightGrams)
            : null
        : (existing?.pieceWeightGrams ?? null),
