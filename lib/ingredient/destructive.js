...
export async function excludeIngredientByCode(productCode) {
  ...
  if (!String(productCode ?? '').trim()) {
    // productCode 없이 호출되면 targets가 항상 비어 findIngredientsByProductCode가 빈 배열을
    // 반환하고, else 분기가 productCode: undefined인 유령 레코드를 새로 만든다 — 방지.
    throw new Error('제품코드가 없는 항목은 이 방법으로 단종 처리할 수 없습니다.');
  }
