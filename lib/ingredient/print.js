...
import { formatUnitPrice, formatNumber } from '@/lib/format';
...
export function printIngredientPdf(rows, { includePhotos = true } = {}) {
  ...
  const cardBody = safeRows
    .map(row => {
      ...
      const baseQty = row.baseQuantity != null && row.baseQuantity !== ''
        ? `${formatNumber(row.baseQuantity)}${row.baseUnitType || ''}`
        : '-';
      const jettePrice = row.priceWithTax != null ? `${formatNumber(row.priceWithTax)}원` : '-';
      return `
        <article class="ing-card">
          ...
          <div class="ing-info">
            ...
            <dl>
              ...
              <dt>포장단위</dt><dd>${esc(baseQty)}</dd>
              <dt>보관온도</dt><dd>${esc(row.temperature || '-')}</dd>
              <dt>제때가격</dt><dd>${esc(jettePrice)}</dd>
...
dl { display: grid; grid-template-columns: 60px 1fr; gap: 4px 8px; margin: 0; font-size: 11px; line-height: 1.4; }
