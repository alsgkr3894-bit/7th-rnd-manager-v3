...
export async function getNutritionDashboard() {
  const [menus, rawValues, origins, ingredients] = await Promise.all([
  ...
  const originLinkedIds = new Set(
    origins.filter(o => activeIds.has(o.ingredientId)).map(o => o.ingredientId)
  );
  const originLinked = activeIngredients.filter(
    r => r.originNone === true || originLinkedIds.has(r.id)
  ).length;
