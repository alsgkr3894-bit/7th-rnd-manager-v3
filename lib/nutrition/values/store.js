 ...
 *
 * nutrition_ingredient_values — 식자재별 100g 기준 영양성분 참조값
 ...
 *   { id, menuCode, menuName, baseMenuCode, ingredientCodes[], ingredientAmounts:{ [code]:{L,R} },
...
/** 메뉴 삭제 시 nutrition_menu_ref + nutrition_raw_values 일괄 cascade 삭제 */
export async function deleteMenuRefsByMenuCode(menuCode) {
  if (!menuCode) return;
  if (hasStore('nutrition_menu_ref')) {
    const refs = await getAll('nutrition_menu_ref');
    const targets = refs.filter(r => r.menuCode === menuCode);
    if (targets.length) {
      await runTransaction('nutrition_menu_ref', 'readwrite', tx => {
        const st = tx.objectStore('nutrition_menu_ref');
        targets.forEach(r => st.delete(r.id));
      });
    }
  }
  await deleteRawValuesByMenuCode(menuCode);
...
export async function getIngredientValueByCode(productCode) {
  if (!hasStore('nutrition_ingredient_values') || !productCode) return null;
  const rows = await getAll('nutrition_ingredient_values');
  return rows.find(r => r.productCode === productCode) ?? null;
}
