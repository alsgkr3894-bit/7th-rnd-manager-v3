...
// 중복 판정·진단 순수 로직은 ./dedup로 분리 — 공개 함수는 기존 import 경로 보존을 위해 re-export.
...
export async function getAllSetCompositions() {
  ...
  return rows
    .filter(row => !(row?.kind === 'set' && !['L', 'R'].includes(row.setSide)))
    .sort((a, b) => (a.updatedAt || '').localeCompare(b.updatedAt || ''));
}

/** nutrition_set_composition에서 setSide가 없는 잘못된 행을 삭제한다. */
