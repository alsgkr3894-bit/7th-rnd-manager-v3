...
// "(1인용)" 등은 크러스트 표기(예: "(석쇠 L)")와 같은 방식으로 괄호가 통째로 제거되면
// 일반 피자와 매칭 키가 동일해져 서로 다른 제품(예: 페페로니 vs 페페로니(1인용))이
// 하나의 코드로 합쳐지는 문제가 생긴다. 괄호를 지우기 전에 1인용 여부를 먼저 판별해
// 매칭 키에 별도 표식을 남겨 두 제품이 항상 다른 키로 구분되게 한다.
const PERSONAL_PIZZA_MARK_RE = /1인용|1인피자/;

export function normalizeImportMatchKey(value) {
  const source = normalizeName(value);
  const isPersonal = PERSONAL_PIZZA_MARK_RE.test(source);
  ...
  const key = name.replace(/[^0-9A-Za-z가-힣]+/g, '').toLowerCase();
  if (!isPersonal) return key;
  // alias 저장(cleanAliasMap)이 이미 정규화된 키를 다시 이 함수에 통과시키므로,
  // 표식이 이미 붙어 있으면 중복으로 덧붙이지 않아야 두 번 호출해도 같은 값이 나온다(idempotent).
  return key.endsWith('1인용') ? key : `${key}1인용`;
}
...
const CRUST_PATTERNS = [
  ...
  { re: /^(.*?)\s*\(\s*1인용\s*\)\s*$/, crustType: PERSONAL_PIZZA_CRUST_CODE, personal: true },
  ...
  { re: /^(.*?)\s+1인용\s*$/, crustType: PERSONAL_PIZZA_CRUST_CODE, personal: true },
];
...
function buildMenuLookups(menuMasters) {
  ...
  for (const menu of menuMasters) {
    // 마스터명에 "1인용"/"1인피자" 문구가 없어도 카테고리가 1인피자면 표식을 붙여,
    // 이름만으로는 구분 안 되는 일반/1인용 동명 메뉴가 같은 키로 섞이지 않게 한다.
    const personal = isPersonalPizzaCategory(menu.category);
    const withMark = name => (personal ? `${name} (1인용)` : name);
    const nameKeys = new Set([
      normalizeImportMatchKey(withMark(menu.menuName)),
      normalizeImportMatchKey(withMark(stripPizzaSuffix(menu.menuName))),
    ]);
