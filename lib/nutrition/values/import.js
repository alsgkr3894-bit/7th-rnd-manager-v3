...
const CRUST_PATTERNS = [
  ...
  { re: /^(.*?)\s*\(\s*씬바[샤사]삭\s*R\s*\)\s*$/i, crustType: '씬바사삭R' },
  ...
  { re: /^(.*?)\s+씬바[샤사]삭\s*R\s*$/i, crustType: '씬바사삭R' },
...
export function buildImportRows({ rawRows, menuMasters, existingKeys = {} }) {
  ...
  for (const row of safeRawRows) {
    ...
    if (isSide) {
      ...
      if (matches.length >= 1) {
        const m = matches[0];
        menuCode = getMenuCodeBase(m);
        menuName = m.menuName;
        category = normalizeNutritionCategory(m.category, '사이드');
        status = 'matched';
      } else {
        menuCode = '';
        menuName = rawName;
    ...
    if (personal) {
      ...
      // 1인피자 카테고리로 필터링
      const personal1in = matches.filter(m => isPersonalPizzaCategory(m.category));
    ...
    if (matches.length >= 1) {
      ...
      category = normalizeNutritionCategory(resolvedCategory || m.category || '', '피자');
      status = 'matched';
    } else {
      menuCode = '';
