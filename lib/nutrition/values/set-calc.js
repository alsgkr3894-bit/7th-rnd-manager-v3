  ...
  SET_SIDES.forEach(side => {
    ...
    result[side] = sideKcals.length
      ? {
          ...
          minWeight: sideWeights.length ? Math.round(Math.min(...sideWeights) + slotMinWeight) : null,
          maxWeight: sideWeights.length ? Math.round(Math.max(...sideWeights) + slotMaxWeight) : null,
        }
