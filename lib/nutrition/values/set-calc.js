...
import { combineBaseWithEdge } from './store';
import { isBeverageCategory, isPizzaCategory } from '@/lib/menu-master/category-policy';
import {
  EDGE_VARIANTS,
...
/**
 * @private rawMap에서 메뉴의 총열량 + 총중량 반환
 * (단품 → 석쇠L → 1인용피자 → 씬바샤삭L 순 폴백)
 * 1인용피자가 세트박스 슬롯에 들어갈 때를 위해 전용 슬롯을 먼저 찾고,
 * 아직 이관 전 데이터가 남아있을 경우를 대비해 옛 슬롯도 폴백으로 둔다.
 */
function menuFullData(menu, rawMap, masterByCode) {
  ...
  const raw =
    ...
    {};
  // 사이드(basis 'serving')·음료는 저장된 kcal이 100g당이 아니라 1회 제공량(1병) 총열량이다
  // — 사이드·음료 출력표(side.js/beverage.js)와 같은 규칙. 2026-09-22 전엔 여기서도 ×중량/100을
  // 해서 패밀리박스 열량이 3~4배(콜라 1.25L 550kcal → 6875kcal)로 부풀려졌다.
  const category = masterByCode?.[menuCode]?.category || menu?.category || '';
  const servingTotal =
    raw.basis === 'serving' || isBeverageCategory(category) || /^D-/.test(menuCode);
  if (!servingTotal) return rawFullData(raw);
  const w = parseFloat(raw.weight);
  const k = parseFloat(raw.kcal);
  if (isNaN(w) || w <= 0 || isNaN(k) || k <= 0) return null;
  return { kcal: Math.round(k), weight: w };
}
...
/**
 * 세트박스 슬롯의 "부분 수량" 비율.
 * 등록된 완제품(기준 수량 baseQty)이 세트박스에는 더 적은 수량(qty)으로 들어갈 때
 * 영양값을 qty/baseQty 배로 반영한다. 둘 다 유효한 양수일 때만 적용, 아니면 1(전체값).
 * @param {{ qty?: number|string, baseQty?: number|string }} slot
 * @returns {number}
 */
function slotRatio(slot) {
  const qty = parseFloat(slot?.qty);
  const base = parseFloat(slot?.baseQty);
  if (!isNaN(qty) && qty > 0 && !isNaN(base) && base > 0) return qty / base;
  return 1;
}
...
const BASE_VARIANTS = [
  ...
  { crustType: THIN_CRUST_CODE, crustLabel: THIN_CRUST_LABEL, side: 'L' },
];
 ...
 * 엣지 행은 해당 사이즈 석쇠 값과 엣지 조정값을 합산한 중량으로 총열량을 계산한다.
 */
export function getPizzaCalorieVariants(menu, rawMap, edgeMap = {}) {
  ...
  EDGE_VARIANTS.forEach(({ edgeCode, side }) => {
    ...
    const full = rawFullData(combined);
    if (!full) return;
...
export function calcSetMinMaxBySize(
  slots,
  menuRefs,
  rawMap,
  masterByCode,
  ...
  for (const slot of slots || []) {
    ...
    const fullRows = candidates.map(m => menuFullData(m, rawMap, masterByCode)).filter(Boolean);
    if (!fullRows.length) {
      return Object.fromEntries(SET_SIDES.map(side => [side, { minKcal: null, maxKcal: null }]));
    }
    const kcals = fullRows.map(row => row.kcal).filter(v => v != null && v > 0);
    const weights = fullRows.map(row => row.weight).filter(v => v != null && v > 0);
    if (!kcals.length || !weights.length) {
      return Object.fromEntries(SET_SIDES.map(side => [side, { minKcal: null, maxKcal: null }]));
    }
    const ratio = slotRatio(slot);
    slotMin += Math.min(...kcals) * ratio;
    slotMax += Math.max(...kcals) * ratio;
    slotMinWeight += Math.min(...weights) * ratio;
    slotMaxWeight += Math.max(...weights) * ratio;
  }

  const result = {};
  SET_SIDES.forEach(side => {
    const sideVariants = pizzaVariants.filter(v => v.side === side);
    const sideKcals = sideVariants.map(v => v.kcal);
    const sideWeights = sideVariants.map(v => v.weight).filter(v => v != null && v > 0);
    result[side] = sideKcals.length
      ? {
          minKcal: Math.round(Math.min(...sideKcals) + slotMin),
          maxKcal: Math.round(Math.max(...sideKcals) + slotMax),
          minWeight: sideWeights.length
            ? Math.round(Math.min(...sideWeights) + slotMinWeight)
            : null,
          maxWeight: sideWeights.length
            ? Math.round(Math.max(...sideWeights) + slotMaxWeight)
            : null,
        }
...
/**
 * 세트박스 목록을 "계산 결과" 미리보기용 평탄 행 배열로 변환한다.
 * 세트박스는 열량·중량만 최소/최대 범위로 계산되고(다른 영양소는 산출 불가),
 * 각 세트의 L/R 사이드별로 한 행씩 반환한다.
 * @returns {Array<{ setName:string, side:'L'|'R', minKcal:number|null, maxKcal:number|null, minWeight:number|null, maxWeight:number|null }>}
 */
export function buildSetPreviewRows(
  setComps,
  menuRefs,
  rawMap,
  masterByCode,
  ...
  const sets = (setComps || []).filter(
    comp => comp?.kind === 'set' && SET_SIDES.includes(asDisplayText(comp.setSide))
  );
  return sets.map(comp => {
    const side = asDisplayText(comp.setSide, 'L') === 'R' ? 'R' : 'L';
    const bySize = calcSetMinMaxBySize(
    ...
    const result = bySize[side] || { minKcal: null, maxKcal: null };
    return {
      setName: asDisplayText(comp.setName, '세트'),
      side,
      minKcal: result.minKcal ?? null,
      maxKcal: result.maxKcal ?? null,
      minWeight: result.minWeight ?? null,
...
function getCandidateMenus(slot, menuRefs, masterByCode) {
  if (slot.menuCodes?.length) {
    // 슬롯이 "출력 대표 메뉴" 연결된 변형 코드(예: 오븐 스파게티 (건면))를 가리키면 라벨
    // 컨텍스트가 그 코드를 대표 코드로 바꿔 두므로(label/context.js), 대표 코드로도 찾는다.
    const codes = new Set(
      slot.menuCodes.flatMap(code => {
        const rep = asDisplayText(masterByCode?.[code]?.outputMenuCode);
        return rep ? [code, rep] : [code];
      })
    );
    return menuRefs.filter(m => codes.has(m.menuCode));
  }
