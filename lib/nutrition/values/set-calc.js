...
export function calcSetMinMax(slots, menuRefs, rawMap, masterByCode, pizzaMenus) {
  const allPizzas = pizzaMenus || menuRefs.filter(m => {
...
export function calcHalfMinMax(pizzaMenuRefs, rawMap, edgeMap) {
  if (!pizzaMenuRefs?.length) return { minKcal: null, maxKcal: null, minWeight: null, maxWeight: null };
  ...
  const lo1 = menuTotals[0], lo2 = menuTotals[Math.min(1, n - 1)];
  const hi1 = menuTotals[n - 1], hi2 = menuTotals[Math.max(n - 2, 0)];
