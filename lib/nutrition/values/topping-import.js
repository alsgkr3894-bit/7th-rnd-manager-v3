...
const ALLERGEN_ORDER_BY_CODE = Object.fromEntries(
  ALLERGEN_SEED.map(({ allergenCode }, index) => [allergenCode, index])
);
...
export async function parseToppingExcel(buffer) {
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    for (let index = headerIndex + 1; index < sheetRows.length; index += 1) {
      ...
      const hasNutrition = Object.values(columns.values).some(
        col => parseNum(rowValue(row, col)) !== ''
      );
...
export function buildToppingImportRows({ rawRows = [], toppings = [], ingredients = [] } = {}) {
  ...
  return (Array.isArray(rawRows) ? rawRows : []).map((rawRow, index) => {
    ...
    const toppingName =
      asText(row.toppingName) ||
      asText(row.ingredientName) ||
      asText(linkedIngredient?.ingredientName);
    ...
    const ingredientName = asText(
      linkedIngredient?.ingredientName || row.ingredientName || toppingName
    );
    ...
    const hasNutrition = NUTRITION_FIELDS.some(
      field => values[field.key] !== '' && values[field.key] != null
    );
    const status = !toppingName
      ? 'invalid'
      : duplicate
        ? 'dup'
        : existingTopping
    ...
    return {
      ...
      hasNutrition,
      values: Object.fromEntries(
        NUTRITION_FIELDS.map(field => [field.key, values[field.key] ?? ''])
      ),
    };
  });
}
...
export function toToppingImportRecord(row, index = 0, now = Date.now()) {
  ...
  return {
    ...
    basis: 'serving',
    ...Object.fromEntries(
      NUTRITION_FIELDS.map(field => [field.key, values[field.key]]).filter(
        ([, value]) => value !== ''
      )
    ),
  };
