...
/**
 * 식자재 삭제 시 연결된 알레르기 링크를 함께 삭제한다.
 * ingredientId가 주 연결 기준이고, productCode는 구버전/보조 매칭용이다.
 *
 * @param {{ ingredientId?: number|string|null, productCode?: string|null }} params
 * @returns {Promise<number>} 삭제한 링크 수
 */
export async function deleteAllergenLinksByIngredient({
  ingredientId = null,
  productCode = null,
} = {}) {
  if (!hasStore(LINKS_STORE)) return 0;
  const code = String(productCode || '').trim();
  const hasIngredientId = ingredientId !== null && ingredientId !== undefined && ingredientId !== '';
  if (!hasIngredientId && !code) return 0;
