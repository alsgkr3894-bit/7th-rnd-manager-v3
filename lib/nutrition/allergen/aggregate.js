...
// ALLERGEN_SEED 배열 순서(사용자 지정 기본 표시 순서)로 정렬하기 위한 코드→순번 맵.
const codeToOrder = Object.fromEntries(
  asObjectArray(ALLERGEN_SEED).map((a, index) => [asDisplayText(a.allergenCode), index])
);
function allergenOrderOf(code) {
  return code in codeToOrder ? codeToOrder[code] : Number.MAX_SAFE_INTEGER;
}
...
function menuCodeAliases(menuCode, meta = {}) {
  const code = asDisplayText(menuCode);
  if (!code) return [];
  const aliases = [code];
  const base = getMenuCodeBase({ menuCode: code, size: meta?.size });
  if (base && base !== code) aliases.push(base);

  // 영양성분 출력은 메뉴마스터 L/R 행을 base menuCode로 묶어 쓰는 경우가 많다.
  // 구형 recipe map에는 size가 빠질 수 있어 피자형 L/R 코드도 한 번 더 보정한다.
  const lrBase = code.match(/^(.+?-\d{3})-[LR]$/i)?.[1];
  if (lrBase && lrBase !== code) aliases.push(lrBase);
...
export function buildMenuAllergenMap({ ingredients, ingredientToMenus } = {}) {
  ...
  const ingByKey = buildIngredientLookup(ingredients);
  ...
  for (const [key, menus] of asMenuMap(ingredientToMenus)) {
    ...
    for (const [menuCode] of menus) {
      const meta = menus.get(menuCode);
      for (const alias of menuCodeAliases(menuCode, meta)) {
        if (!map.has(alias)) map.set(alias, new Set());
        asStringArray(ing.allergens).forEach(code => map.get(alias).add(code));
      }
    }
  }
...
export function buildEdgeAllergenMap({ ingredients, edges } = {}) {
  ...
  for (const edge of asObjectArray(edges)) {
    ...
    for (const component of asObjectArray(edge.components)) {
      const ing = findIngredientByComponent(ingByKey, component);
      if (!ing) continue;
