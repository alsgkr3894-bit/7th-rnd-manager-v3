...
import {
  ...
  findIngredientByComponent,
  normStr,
} from '@/lib/nutrition/allergen/ingredient-lookup';
...
export function logicalMenuKey(menuCode, menuName, category) {
  ...
  if ((isPizzaCategory(cat) || isPersonalPizzaCategory(cat)) && code) {
...
export function edgeTypeForCrust(crust) {
  ...
  if (isThinCrustLabel(label)) return '씬도우';
  return null;
}
...
export function nutritionEdgeCodeFor(edgeType) {
  ...
  if (edgeType === '씬도우') return THIN_CRUST_CODE;
  return null;
}
...
export function buildMenuMatrix(
  ...
  const ingByKey = buildIngredientLookup(allergenIngredients);
  ...
  for (const edge of asObjectArray(edges)) {
    ...
    for (const c of asObjectArray(edge.components)) {
      const ing = findIngredientByComponent(ingByKey, c);
  ...
  const ensureMenuBase = (menuCode, meta = {}) => {
    const code = asDisplayText(menuCode);
    const menuName = asDisplayText(meta?.menuName);
    if (!code && !menuName) return null;
    if (isExcludedMenu(code, menuName)) return null;

    const logicalKey = logicalMenuKey(code, menuName, meta?.category);
    ...
    if (!menuBase.has(logicalKey)) {
      menuBase.set(logicalKey, {
        meta: {
          ...meta,
          menuCode: undefined,
          menuName: stripSizeSuffix(menuName) || menuName,
        },
        menuCode: logicalKey,
        menuCodes: new Set(),
        codes: new Set(),
      ...
      const entry = menuBase.get(logicalKey);
      entry.meta = {
        ...entry.meta,
        ...meta,
        menuCode: undefined,
        menuName:
          stripSizeSuffix(menuName) ||
          asDisplayText(entry.meta?.menuName) ||
          asDisplayText(meta?.menuName),
        category: asDisplayText(meta?.category) || asDisplayText(entry.meta?.category),
        size: asDisplayText(entry.meta?.size) || asDisplayText(meta?.size),
      };
    }

    const entry = menuBase.get(logicalKey);
    if (code) entry.menuCodes.add(code);
    return entry;
  };

  // 기본 도우(석쇠) 알레르기 — 엣지 옵션 행 중 '석쇠'는 엣지가 아니라 레시피 묶음의 도우에서 온다.
  const doughAllergens = new Set();
  ...
  for (const [key, menus] of baseIngredientToMenus) {
    ...
    if (isDough) for (const code of allergenCodes) doughAllergens.add(code);
    for (const [menuCode, meta] of menus) {
      const e = ensureMenuBase(menuCode, meta);
      if (!e) continue;
  ...
  // 아래 토핑 루프가 전용 행을 만드는 추가토핑은 메뉴마스터 행을 또 만들지 않는다.
  // (같은 토핑이 "빈 칸 행 + 정상 행" 두 줄로 나오던 문제)
  const toppingCoveredKeys = new Set();
  for (const topping of asObjectArray(toppings)) {
    const ing = findIngredientByComponent(ingByKey, topping);
    if (!asStringArray(ing?.allergens).length) continue;
    const code = asDisplayText(topping.toppingCode);
    if (code) toppingCoveredKeys.add(code);
    const name = normStr(topping.toppingName);
    if (name) toppingCoveredKeys.add(name);
  }

  for (const menu of asObjectArray(menuMasters)) {
    if (isExtraToppingCategory(asDisplayText(menu?.category))) {
      const code = asDisplayText(menu?.menuCode);
      const name = normStr(menu?.menuName);
      if (toppingCoveredKeys.has(code) || toppingCoveredKeys.has(name)) continue;
    }
    ensureMenuBase(menu?.menuCode, menu);
  }

  const PERSONAL_CRUST = CRUST_VARIANTS.find(v => v.key === THIN_CRUST_KEY);

  const rows = [];
  for (const [menuCode, { meta, menuCodes, codes, nonDoughCodes }] of menuBase) {
    const cat = asDisplayText(meta?.category);
    const isPizza = isPizzaCategory(cat);
    const isPersonal = isPersonalPizzaCategory(cat);

    if (!isPizza && !isPersonal) {
      // 엣지 옵션 메뉴(석쇠·치즈크러스트·골드스윗·씬바사삭)는 레시피가 없어 codes가 비어 있다.
      // 해당 크러스트의 실제 구성품 알레르기를 채운다(피자별 크러스트 행과 같은 출처).
      const edgeOption = isEdgeCategory(cat) ? edgeTypeForCrust(meta?.menuName) : null;
      const optionCodes = isEdgeCategory(cat)
        ? edgeOption
          ? edgeAllergens.get(edgeOption) || new Set()
          : doughAllergens
        : null;
      rows.push({
        ...meta,
        ...
        crust: '',
        edgeType: edgeOption,
        allergenCodes: optionCodes && !codes.size ? new Set(optionCodes) : codes,
      });
      continue;
    }

    if (isPersonal) {
      // 1인피자: 씬바샤삭 크러스트 1행만 (석쇠 도우 제외 + 씬도우 엣지 포함)
      if (PERSONAL_CRUST) {
        const merged = new Set(nonDoughCodes);
        if (PERSONAL_CRUST.edgeType) {
          for (const code of edgeAllergens.get(PERSONAL_CRUST.edgeType) || []) merged.add(code);
        }
        rows.push({
          ...meta,
          rowKey: `${menuCode}__${THIN_CRUST_KEY}`,
          menuCode,
          sourceMenuCodes: [...menuCodes],
          crust: PERSONAL_CRUST.label,
          edgeType: PERSONAL_CRUST.edgeType,
    ...
    for (const v of CRUST_VARIANTS) {
      const merged = new Set(v.key === THIN_CRUST_KEY ? nonDoughCodes : codes);
  ...
  const crustOrder = new Map(
    CRUST_VARIANTS.flatMap((v, i) => [
      [v.key, i],
      [v.label, i],
    ])
  );
...
export function buildDetailRows(detailRow, baseMapData, edges, ingredientByKey) {
  ...
  if (detailRow.kind === 'topping') {
    const ing = findIngredientByComponent(ingredientByKey, detailRow);
    if (ing) {
  ...
  for (const [ingredientKey, menus] of asMenuMap(baseMapData?.ingredientToMenus)) {
    ...
    if (!ing) continue;
    if (isThinCrustLabel(detailRow.crust) && isDoughCategory(asDisplayText(ing.category))) {
      continue;
    }
    for (const [menuCode, meta] of menus) {
  ...
  if (edgeType) {
    for (const edge of asObjectArray(edges)) {
      ...
      for (const component of asObjectArray(edge.components)) {
        const ing = findIngredientByComponent(ingredientByKey, component);
        if (!ing) continue;
        pushRow({
          ing,
          source: {
