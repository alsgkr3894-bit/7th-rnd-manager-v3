...
export function extractExcludedMenuSets(menuMasters) {
  const ex = asObjectArray(menuMasters).filter(m => m.excludeFromOrigin);
  return {
    excludedMenuCodes: new Set(ex.map(m => asDisplayText(m.menuCode)).filter(Boolean)),
    excludedMenuNames: new Set(ex.map(m => asDisplayText(m.menuName).trim()).filter(Boolean)),
  };
}
