...
const menuNames = value => (Array.isArray(value) ? value.filter(Boolean) : [...asSet(value)]);
...
function sheet1Html(rows) {
  ...
  const body = safeRows
    .map((r, index) => {
      const menuCell = spanned.has(index)
        ? ''
        : `<td${spanStart.has(index) ? ` rowspan="${spanStart.get(index)}"` : ''}>${esc(
    ...
    .join('');
  return `
    <div class="sheet-title large">원산지 표시판</div>
    <table class="sign-table">
      <colgroup><col style="width:30%"><col style="width:35%"><col style="width:35%"></colgroup>
      <thead><tr><th>표시품목</th><th>원산지</th><th>메뉴명</th></tr></thead>
      <tbody>${body}</tbody>
    </table>
...
function sheet2Html(rows) {
  ...
  return `
    ...
    <table class="fridge-table">
      ...
      <thead><tr><th>음식명</th><th>표시품목</th><th>원산지</th></tr></thead>
      <tbody>${body}</tbody>
    </table>
    <div class="notice large">${esc(NOTICE)}</div>`;
}
...
function sheet3Html(rows) {
  const body = asObjectArray(rows)
    .map(
      r => `
    <tr>
      <td style="font-weight:700;text-align:center">${esc(r.group)}</td>
      <td style="font-weight:700">${esc(r.menuName)}</td>
      <td>${esc(Array.isArray(r.parts) ? r.parts.join(', ') : '')}</td>
    </tr>`
    )
    .join('');
  return `
    ...
    <table class="delivery-table">
      <colgroup><col style="width:12%"><col style="width:24%"><col style="width:64%"></colgroup>
      <thead><tr><th>구분</th><th>메뉴명</th><th>재료명(원산지)</th></tr></thead>
      <tbody>${body}</tbody>
    </table>
...
function sheet4Html(rows) {
  const lines = buildOriginStatementLines(rows)
    .map(line => `<p class="stmt-line">${line ? esc(line) : '&nbsp;'}</p>`)
    .join('');
  return `
    <div class="sheet-title large">원산지</div>
...
<title>${esc(withDownloadDateSuffix('원산지 표시판'))}</title>
<style>
...
.stmt-box { border: 1px solid #333; border-bottom: none; white-space: pre-wrap; padding: 16px 22px;
...
@media print {
  ...
  openPrintWindow(html, { width: 1000, height: 1100 });
}
