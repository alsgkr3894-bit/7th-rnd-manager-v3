...
function sheet1Html(rows) {
  const body = asObjectArray(rows).map(r => `
    <tr>
      <td>${esc(r.displayName)}</td>
      <td>${esc(r.originCountry)}</td>
      <td>${esc([...asSet(r.menus)].join(', '))}</td>
    </tr>`).join('');
  return `
  ...
  const body = asObjectArray(rows).map(r => {
    ...
    return `
      <tr>
        ...
        <td class="multiline">${esc(items.map(it => it.displayName).join('\n'))}</td>
        <td class="multiline">${esc(items.map(it => it.country).join('\n'))}</td>
      </tr>`;
  }).join('');
  return `
...
function sheet4Html(rows) {
  const lines = asObjectArray(rows).map(r =>
