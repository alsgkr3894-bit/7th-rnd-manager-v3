...
function storeMenuLabel(menu) {
  return menu.group === '피자' ? stripPizzaWord(menu.menuName) : asDisplayText(menu.menuName);
}
...
export function buildOriginStoreSheet(origins, menuOrder = [], ingredientOverrides = {}) {
  ...
  return [...merged.values()]
    .map(entry => {
      ...
      const pizzaCommon =
        allPizzaKeys.size >= 2 && [...allPizzaKeys].every(key => entry.rows[0].menuRefs.has(key));
      const menus = pizzaCommon
        ? [PIZZA_COMMON_LABEL, ...menuRefs.filter(m => m.group !== '피자').map(storeMenuLabel)]
