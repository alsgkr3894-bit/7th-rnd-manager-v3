...
// 제목 행 + 표 영역(헤더 굵게, 전체 테두리) — xlsx-js-style만 반영한다.
function styleTable(ws, { headerRow, lastRow, cols, titleRow = 0 }) {
  applyTitleStyle(XLSX, ws, { r: titleRow });
  applyTableStyle(XLSX, ws, { r0: headerRow, r1: lastRow, c1: cols - 1 });
  setRowHeight(ws, titleRow, 28);
}
