 ...
 */
function buildEdgeGroupPreview(ctx, family, size) {
  const edgeMap = ctx?.edgeMap || {};
  const rows = family.nutritionEdgeCodes
    .map(code => edgeMap[code])
    .filter(Boolean)
    // 치즈크러스트·골드스윗은 메뉴마스터에 L/R 행이 따로 있어 그 행의 사이즈만 보여준다.
    .filter(edgeRecord => !size || getCrustSize(edgeRecord.edgeCode) === size)
    .map(edgeRecord => ({
...
export function buildMenuNutritionPreview(ctx, menu) {
  ...
  if (edgeFamily) {
    if (edgeFamily.nutritionEdgeCodes.length > 0) {
      const size = menu?.size === 'L' || menu?.size === 'R' ? menu.size : null;
      return buildEdgeGroupPreview(ctx, edgeFamily, size);
    }
