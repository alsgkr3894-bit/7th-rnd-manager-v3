...
export async function exportNutritionLabelToExcel({
  ...
  // ── 시트1: 피자 (150g 기준) ──────────────────────────────────
  const ws1Rows = [['제품 영양성분표 — 피자 (150g 기준)'], ['메뉴명', '크러스트', '사이드', ...HEADERS, '함유알레르기']];
  asObjectArray(pizzaSheet).forEach(({ menuName, rows }) => {
    asObjectArray(rows).forEach(r => {
      ws1Rows.push([
        cell(menuName), cell(r.crustLabel), cell(r.side),
        cell(r.weight), cell(r.kcal), cell(r.sugar), cell(r.protein), cell(r.satFat), cell(r.sodium),
        cell(r.allergen),
      ]);
    });
  });
  ...
  // ── 시트1b: 피자 (조각 기준) ─────────────────────────────────
  const wsSliceRows = [['제품 영양성분표 — 피자 (조각 기준)'],
    ['메뉴명', '크러스트', '사이드', '조각수', '1회제공량', '중량(g)', '열량(kcal)', '당류(g)', '단백질(g)', '포화지방(g)', '나트륨(mg)', '함유알레르기']];
  asObjectArray(pizzaSliceSheet).forEach(({ menuName, rows }) => {
    asObjectArray(rows).forEach(r => {
      wsSliceRows.push([
        cell(menuName), cell(r.crustLabel), cell(r.side), cell(r.slice), cell(r.servingLabel),
        cell(r.weight), cell(r.kcal), cell(r.sugar), cell(r.protein), cell(r.satFat), cell(r.sodium),
        cell(r.allergen),
      ]);
    });
  ...
  asObjectArray(toppingSheet).forEach(r => {
    ws2Rows.push([
      cell(r.menuName), cell(r.weight), cell(r.kcal), cell(r.sugar),
      cell(r.protein), cell(r.satFat), cell(r.sodium), cell(r.allergen),
    ]);
  });
  ...
  asObjectArray(setHalfSheet).forEach(r => {
    ws4Rows.push([
      cell(r.menuName), cell(r.weight), cell(r.minKcal), cell(r.maxKcal), cell(r.allergen),
    ]);
  });
