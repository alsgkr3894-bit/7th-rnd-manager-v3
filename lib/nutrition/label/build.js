...
import { addNutrition, buildIngredientAdditionSumForSide } from '@/lib/nutrition/values/store';
...
import { THIN_CRUST_CODE, THIN_CRUST_LABEL } from '@/lib/nutrition/crust-config';
...
function primaryRawValue(rawMap, menuCode) {
  return rawMap[`${menuCode}__석쇠L`] || rawMap[`${menuCode}__${THIN_CRUST_CODE}`] || {};
}
...
function buildToppingMasterRows(toppings, toppingAllergenMap) {
...
function buildLegacyToppingRows({ menus, rawMap, masterByCode, menuAllergenMap, menuOrder }) {
...
const DERIVED_CRUST_TYPES = ['석쇠L', '석쇠R', THIN_CRUST_CODE];

/**
 * 파생 메뉴(nutrition_pizza_composition)를 menus·rawMap에 합쳐
 * 일반 메뉴처럼 빌드될 수 있게 한다.
 *
 * - 파생 menuRef: category는 베이스 메뉴에서 복사 (그룹 정렬용)
 * - 파생 rawMap 엔트리: 베이스 영양값 + 식자재 추가분(buildIngredientAdditionSumForSide)
 * - 엣지 행은 buildPizzaSheet/buildPizzaSliceSheet의 edgeMap 로직이 처리
 *
 * @param {{ menus, rawMap, compositions, ingredientNutritionMap, masterByCode }}
 * @returns {{ menus, rawMap }}
 */
export function augmentWithDerived({
  menus,
  rawMap,
  compositions,
  ...
  for (const comp of compositions) {
    ...
    for (const ct of DERIVED_CRUST_TYPES) {
      ...
      const ingAdd = buildIngredientAdditionSumForSide(comp, ingredientNutritionMap, side);
      derivedRawMap[`${comp.menuCode}__${ct}`] = addNutrition(base, ingAdd);
    }
  }
 ...
 *
 * @returns {Array<{ menuName, menuCode, rows:[{ crustLabel, side, slice, servingLabel, weight, kcal, sugar, protein, fat, sodium, allergen }] }>}
 */
