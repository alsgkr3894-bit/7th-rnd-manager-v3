 ...
 */
export function buildPizzaSliceSheet({ menus, rawMap, edgeMap, masterByCode, menuAllergenMap, sliceCounts }) {
  return menus
    .filter(m => resolveNutritionGroup(m, masterByCode) === '피자')
    .map(menu => {
      const personal = isPersonalPizza(menu, masterByCode);
      ...
      const pushRow = (crustLabel, side, raw100, baseWeightRaw) => {
        ...
        if (isNaN(weight) || weight <= 0) {
          rows.push({ crustLabel, side, slice, servingLabel: '—', weight: '—', kcal: '—', sugar: '—', protein: '—', satFat: '—', sodium: '—', allergen });
          return;
        }
        const totalKcal = parseFloat(scaleVal(raw100.kcal, weight)) || 0;
        ...
        rows.push({
          crustLabel, side, slice,
          servingLabel: `${n}조각`,
          weight: Math.round(weight * factor * 10) / 10,
          kcal: col('kcal'), sugar: col('sugar'), protein: col('protein'), satFat: col('satFat'), sodium: col('sodium'),
          allergen,
        });
      };
