...
// mergeAllergenText는 이미 이름으로 변환된 문자열을 합치므로, 코드가 아니라
// 이름 기준 순번 맵으로 ALLERGEN_SEED의 기본 표시 순서를 따르게 한다.
const ALLERGEN_NAME_ORDER = new Map(
  ALLERGEN_SEED.map((item, index) => [asDisplayText(item.allergenName), index])
);
function allergenNameOrderOf(name) {
  return ALLERGEN_NAME_ORDER.has(name) ? ALLERGEN_NAME_ORDER.get(name) : Number.MAX_SAFE_INTEGER;
}
...
export function displayNutritionMenuName(value, fallback = '메뉴') {
  ...
  while (text && text !== previous) {
    previous = text;
    text = text.replace(/(?:\s+|\s*[\(（])(?:L|R)(?:\s*사이즈)?[\)）]?\s*$/iu, '').trim();
  }
...
function mergeAllergenText(...values) {
  ...
  return seen.size
    ? [...seen].sort((a, b) => allergenNameOrderOf(a) - allergenNameOrderOf(b)).join(', ')
    : DASH;
}
...
export function buildPosterPizzaRows(pizzaSliceSheet, pizza150Sheet = []) {
  ...
  const ensureCrustRow = (group, groupIndex, row) => {
    ...
    const menuKey =
      asDisplayText(group?.menuCode) || `${menuName}__${asDisplayText(groupIndex) || groupIndex}`;
    if (!byMenu.has(menuKey)) {
      byMenu.set(menuKey, { menuName, byCrust: new Map() });
      menuOrder.push(menuKey);
    }
    const bucket = byMenu.get(menuKey);
    const byCrust = bucket.byCrust;
    const crustLabel = nutritionValue(row?.crustLabel);
    if (!byCrust.has(crustLabel)) {
      byCrust.set(crustLabel, {
        menuCode: asDisplayText(group?.menuCode),
        menuName: bucket.menuName,
        crustLabel,
        sides: {},
        per150Sides: {},
      });
  ...
  asObjectArray(pizzaSliceSheet).forEach((group, groupIndex) => {
    asObjectArray(group?.rows).forEach(row => {
      ...
      crustRow.sides[side] = {
        ...row,
        // buildPizzaSliceSheet가 계산한 실제 한판 총중량(totalWeight)을 우선 사용해
        // 화면 표와 엑셀·포스터 값을 일치시킨다. 총중량이 없는 경우에만 1회중량×조각수로
        // 역산(반올림 오차 있음)한다.
        totalWeight: row?.totalWeight ?? inferTotalWeight(row),
      };
    });
  });
  ...
  const rows = [];
  menuOrder.forEach(menuKey => {
    const bucket = byMenu.get(menuKey);
    const crustRows = [...bucket.byCrust.values()];
