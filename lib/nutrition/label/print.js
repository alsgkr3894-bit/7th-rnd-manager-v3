  ...
  return `
    ...
    <table>
      <thead><tr>
        ...
        <th>총중량</th><th>중량단위</th><th>열량(kcal/150g)</th>
        <th>단백질(g/150g)</th><th>포화지방(g/150g)</th><th>나트륨(mg/150g)</th><th>당류(g/150g)</th>
        <th>함유알레르기</th>
      </tr></thead>
      <tbody>${body}</tbody>
    </table>`;
...
function pizzaSliceHtml(rows) {
  const body = asObjectArray(rows)
    .map(({ menuName, rows: rawRows }) => {
      ...
      <tr>
        ...
        <td class="num">${val(r.totalWeight)}</td>
        <td class="num">${val(r.kcal)}</td>
        <td class="num">${val(r.sugar)}</td>
        <td class="num">${val(r.protein)}</td>
        <td class="num">${val(r.fat)}</td>
  ...
  return `
    ...
    <table>
      <thead><tr>
        <th>메뉴명</th><th>크러스트</th><th>L/R</th>
        <th>1회중량(g)</th><th>1회조각수</th><th>총조각중량(g)</th>
...
function pizza150PosterHtml(rows150) {
  const posterRows = buildPosterPizzaRows([], rows150);
  const sideHeader = PIZZA_150_GROUPS.map(
    group =>
      `<th data-pair="${esc(group.key)}">${esc(group.label)} L</th><th data-pair="${esc(group.key)}">${esc(group.label)} R</th>`
  ).join('');
  const body = posterRows.length
    ? posterRows
        .map(
          row => `
    <tr>
      <td class="poster-menu-name">${posterValue(row.menuName)}</td>
      <td class="poster-crust-name">${posterValue(row.crustLabel)}</td>
      ${PIZZA_150_GROUPS.map(
        group => `
          <td class="poster-num">${posterValue(pair150PosterValue(row, group.key, 'L'))}</td>
          <td class="poster-num">${posterValue(pair150PosterValue(row, group.key, 'R'))}</td>`
      ).join('')}
      <td class="poster-allergen">${posterValue(pairAllergen(row))}</td>
    </tr>`
        )
        .join('')
    : `<tr><td colspan="${PIZZA_150_GROUPS.length * 2 + 3}" class="poster-empty-cell">피자 150g 기준 영양성분 데이터가 없습니다</td></tr>`;
  ...
  return `
    <section class="nutrition-poster-main-section">
      <table class="nutrition-poster-table nutrition-poster-pizza-table">
        <thead>
          <tr>
            <th rowspan="2" class="poster-pizza-label">메뉴명</th>
...
function pizzaSlicePosterHtml(sliceRows) {
  const posterRows = buildPosterPizzaRows(sliceRows, []);
  const sideHeader = PIZZA_SLICE_GROUPS.map(
    group =>
      `<th data-pair="${esc(group.key)}">${esc(group.label)} L</th><th data-pair="${esc(group.key)}">${esc(group.label)} R</th>`
  ).join('');
  const body = posterRows.length
    ...
    <tr>
      ...
      <td class="poster-allergen">${posterValue(pairAllergen(row))}</td>
        ...
        .join('')
    : `<tr><td colspan="${PIZZA_SLICE_GROUPS.length * 2 + 3}" class="poster-empty-cell">피자 조각 기준 영양성분 데이터가 없습니다</td></tr>`;
...
function simplePosterHtml(title, rows, cols = SIMPLE_COLS, options = {}) {
  ...
  const moreRow =
    hidden > 0
      ? `<tr><td colspan="${cols.length + 2}" class="poster-more-cell">외 ${hidden}건 (전체 ${allRows.length}건) — 전체 목록은 엑셀·화면 표에서 확인</td></tr>`
      : '';
  const body = safeRows.length
    ...
    <tr>
      ...
      ${cols.map(col => `<td class="poster-num">${posterValue(row[col.key])}</td>`).join('')}
  ...
  const body = safeRows.length
    ? safeRows
      ...
      <p><strong>${posterValue(row.names)}</strong><span>(${posterValue(row.breakdown)})</span></p>`
        ...
        .join('') +
      (hidden > 0
        ? `<p class="poster-more-cell">외 ${hidden}건 (전체 ${allRows.length}건) — 전체는 원산지 표시판·엑셀에서 확인</p>`
        : '')
...
function nutritionPosterHtml({
  ...
  <article class="nutrition-poster-sheet">
    ${pizza150PosterHtml(pizzaSheet)}
    ${pizzaSlicePosterHtml(pizzaSliceSheet)}
...
.poster-more-cell { text-align:center; color:#999; font-size:4.4pt !important; font-style:italic; }
