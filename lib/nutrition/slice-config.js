 ...
 * 기본값: 피자 L 8조각 · R 6조각, 1인용 피자 6조각. 예외는 메뉴별 오버라이드.
...
/**
 * 메뉴 기본 조각수 — 일반 피자 L 8조각 · R 6조각, 1인용 피자 6조각.
 * (운영자가 sliceCounts로 메뉴별 별도 지정하면 그 값이 우선한다.)
 */
export function defaultSlices(menu, masterByCode = {}) {
  if (isPersonalPizzaMenu(menu, masterByCode)) return { L: 6, R: 6 };
  return { L: 8, R: 6 };
}
