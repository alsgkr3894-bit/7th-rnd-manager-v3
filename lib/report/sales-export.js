...
export function buildSalesReportWorkbookSheets({
  ...
  const sheets = [
    {
      ...
      rows: [
        ...
        [`${compareLabel} 판매량`, safeQuantity(kpi?.previous)],
        ...(showRevenue ? [[`${compareLabel} 매출액`, safeRevenue(kpi?.previousRevenue)]] : []),
        [`${compareLabel} 대비(%)`, formatSignedPercent(kpi?.deltaPct)],
        ...(showRevenue
          ? [[`매출 ${compareLabel} 대비(%)`, formatSignedPercent(kpi?.revenueDeltaPct)]]
          : []),
  ...
  if (safeOpts.prevComp) rankHeaders.push(compareLabel, '증감', '증감(%)');
  if (safeOpts.variant) {
  ...
  for (const category of categories) {
    ...
    const rows = [
      [
        ...
        '판매량',
        ...(showRevenue ? ['매출액'] : []),
        ...(safeOpts.prevComp ? [compareLabel, '증감'] : []),
      ],
