...
export async function saveReport(data) {
  await assertActiveAdmin('보고서 저장');
  await initDB();
...
export async function toggleReportFav(id, currentFav) {
  await assertActiveAdmin('보고서 즐겨찾기 변경');
  await initDB();
  const rows = await getAll('generated_reports');
...
export async function deleteReport(id) {
  await assertActiveAdmin('보고서 삭제');
  await initDB();
  return deleteById('generated_reports', id);
}

/**
 * 삭제 후보 보고서 목록 반환 (삭제 안 함)
 * @param {number} keepDays - 보관 기간 (기본 90일)
 */
export async function findPrunableReports(keepDays = 90) {
  await initDB();
  const cutoff = new Date();
  cutoff.setDate(cutoff.getDate() - normalizeKeepDays(keepDays));
  const cutoffISO = cutoff.toISOString();
  return asObjectArray(await getAll('generated_reports')).filter(r => r.createdAt < cutoffISO);
}
...
export async function pruneOldReports(keepDays = 90) {
  await assertActiveAdmin('오래된 보고서 정리');
  await initDB();
  const cutoff = new Date();
  cutoff.setDate(cutoff.getDate() - normalizeKeepDays(keepDays));
  const cutoffISO = cutoff.toISOString();
