...
/** 단일 열 표에서 L/R 행을 구분해 보여줄 라벨 — '패밀리박스 L', '치즈크러스트 R'. */
export function costReportMenuLabel(menu) {
  const info = getCostReportMenuDisplayInfo(menu);
  return isCostReportGroupableSize(info.size) ? `${info.name} ${info.size}` : info.name;
}
