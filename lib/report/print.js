  ...
  style.textContent = `
    @media print {
      ...
      * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
      ...
      /* 전역 .chip(min-height 36px, padding 8px 12px)이 배지를 큰 알약으로 키운다 — 인쇄물에서는
         메뉴명 옆에 붙는 작은 태그 크기로 눌러 준다. */
      #${PRINT_ROOT_ID} .discontinued-badge,
      #${PRINT_ROOT_ID} .irregular-menu-badge,
      #${PRINT_ROOT_ID} .unregistered-badge {
      ...
      #${PRINT_ROOT_ID} .irregular-menu-badge {
        /* 관리자 화면에서는 print-only(화면 숨김)라 인쇄 중 다시 인라인으로 펴 준다 —
           .print-only의 인쇄 규칙은 display:block이라 칩이 줄바꿈되는 것을 막는다. */
      ...
      /* "+ 단종" 마크 버튼은 no-print라 인쇄에서 사라진다 — 아직 단종 등록 전(미등록) 항목이
         PDF에서 아무 표시도 없던 문제를 막기 위해, 화면에서는 print-only로 숨겨뒀던 "미등록"
         배지를 인쇄 중에는 고정 고대비 색으로 보여준다. */
      #${PRINT_ROOT_ID} .unregistered-badge {
        display: inline-flex !important;
        background: #f4f4f5 !important;
        color: #555555 !important;
        border: 1px dashed #8a8f98 !important;
      }
      .recipe-print-overview { display: none !important; }
      #${PRINT_ROOT_ID} .recipe-print-menu-page {
        /* .recipe-print-menu-page는 paper-section을 겸한다(RecipeMenuBlock.jsx).
      ...
      /* "메뉴당 1페이지"가 꺼진 카테고리(세트박스·사이드·추가토핑)의 메뉴 블록 — 마찬가지로
         paper-section을 겸하므로 !important가 필요하다. 다만 이 블록끼리는 페이지를
         나누지 않고 카테고리 섹션 안에서 이어붙인다(break-before 규칙 없음). */
      #${PRINT_ROOT_ID} .recipe-print-menu-block {
        break-inside: avoid !important;
        page-break-inside: avoid !important;
      }
      #${PRINT_ROOT_ID} .recipe-print-cat-section {
        break-inside: auto !important;
        overflow: visible !important;
      }
      #${PRINT_ROOT_ID} .recipe-print-cat-section + .recipe-print-cat-section {
        break-before: page;
        page-break-before: always;
      }
      /* 원가계산 보고서 PDF 뒤에 레시피를 부록으로 붙일 때(opts.recipeAppendix) — 본문과
         분리되도록 항상 새 페이지에서 시작한다. */
      #${PRINT_ROOT_ID} .recipe-print-appendix {
        break-before: page;
        page-break-before: always;
      }
      #${PRINT_ROOT_ID} .recipe-print-appendix-head {
        display: none !important;
      }
    }
  `;
