...
function getPrintableElements(elements) {
  const list =
    elements && (typeof elements[Symbol.iterator] === 'function' || typeof elements.length === 'number')
      ? elements
      : [elements];
...
export function makeReportPrintTitle(reportMeta = {}, options = {}) {
  ...
  const nameWithoutPeriod = (asDisplayText(safeReportMeta.name) || '보고서').replace(rawPeriod, '').trim();
