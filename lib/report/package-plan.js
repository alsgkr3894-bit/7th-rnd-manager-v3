...
export function getMonthlyCloseTargetPeriod(closePeriod) {
  const period = normalizeYearMonth(closePeriod);
...
export function monthLabel(period) {
  ...
  if (!target) return '';
  return `${target.year}년 ${target.month}월`;
}

export function monthRangeLabel(period) {
  const target = normalizeYearMonth(period);
  if (!target) return '';
  return `${target.year}년 ${target.month}월 1일 ~ ${target.month}월 말일`;
}

export function buildPackageItemHref(item, targetPeriod) {
  const target = normalizeYearMonth(targetPeriod);
  if (!item?.href || !target) return item?.href || '/report';
  return `${item.href}?year=${target.year}&month=${target.month}`;
}
