...
export const RECIPE_PRINT_KINDS = DETAIL_RECIPE_KINDS;

const CATEGORY_LABEL_TO_KIND = new Map(DETAIL_RECIPE_KINDS.map(meta => [meta.label, meta.id]));

/** categoryLabel('피자'/'1인피자'/...)로부터 kind('pizza'/'personal'/...)를 찾는다. 없으면 ''. */
export function recipeKindFromCategoryLabel(label) {
  return CATEGORY_LABEL_TO_KIND.get(text(label)) || '';
}

/** buildRecipePrintMenus가 그룹핑에 쓰는 것과 같은 id — 선택 상태를 이 id로 저장한다. */
export function recipePrintMenuId(row) {
  return recipeMenuGroupKey(row);
}

function normalizeReportComponent(
  component,
  index,
  ...
  for (const row of Array.isArray(recipeRows) ? recipeRows : []) {
    ...
    if (!menuMap.has(groupKey)) {
      menuMap.set(groupKey, {
        id: groupKey,
        kind: text(row?.kind) || recipeKindFromCategoryLabel(row?.categoryLabel),
