...
export const KIND_META = {
  ...
  compare: {
    ...
    icon: 'calc',
    emoji: '⚖️',
    href: '/report/sales?view=compare',
  ...
  margin: {
    id: 'margin',
    title: '원가마진표 보고서',
    sub: '원가마진표를 카테고리·엣지·사이즈별로 미리보고 출력.',
    color: '#0F766E',
    icon: 'calc',
    emoji: '📈',
    href: '/report/cost?mode=margin',
    chip: { bg: '#CCFBF1', color: '#0F766E', label: '마진표' },
    mergedInto: 'cost',
    hideInLauncher: true,
  },
};

export const REPORT_LAUNCHER_KINDS = Object.values(KIND_META).filter(kind => !kind.hideInLauncher);

/** KIND_META에서 파생 — 각 파일에서 개별 선언 없이 사용 */
export const KIND_CHIP = Object.fromEntries(Object.entries(KIND_META).map(([k, v]) => [k, v.chip]));
export const KIND_COLOR = Object.fromEntries(
  Object.entries(KIND_META).map(([k, v]) => [k, v.color])
);
export const KIND_LABEL = Object.fromEntries(
  Object.entries(KIND_META).map(([k, v]) => [k, v.chip.label])
);
export const KIND_EMOJI = Object.fromEntries(
  Object.entries(KIND_META).map(([k, v]) => [k, v.emoji])
);
