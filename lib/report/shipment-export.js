...
export function buildShipmentCategorySummaryRows({
  ...
  const rows = [];
  const sumQty = list =>
    asObjectArray(list).reduce((sum, item) => sum + safeQuantity(item.totalQuantity), 0);
  const sumAmt = list =>
    asObjectArray(list).reduce((sum, item) => sum + safeQuantity(item.totalAmount), 0);
...
export async function exportShipmentReportXlsx(params = {}) {
  ...
  XLSX.writeFile(wb, withDownloadDateSuffix(`${brandName}_${periodLabel} 제때 출고량 보고서.xlsx`));
}
