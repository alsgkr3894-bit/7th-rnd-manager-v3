...
// 석쇠는 cost_edge_dough에 별도 행이 없다(costEdgeType: null) — 도우 자체 원가는 이미 피자
// 베이스 레시피에 들어있어 "추가 원가"가 없다. 예전엔 stripName(edgeType)==='석쇠'가 절대
// 매칭되지 않아 아무 엣지나(배열 첫 번째) 골라 더하는 버그가 있었다 — 항상 0을 반환한다.
function defaultEdgeCostForSize() {
  return 0;
}
...
function costForPrice(p, ctx) {
  ...
  if (ctx.includeEdge && isPizzaCategory(p.category)) {
    baseCost += defaultEdgeCostForSize();
  }
  return baseCost;
}
