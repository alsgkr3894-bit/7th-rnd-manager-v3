...
function allDetailRecipes(detailMaps) {
  return Object.values(detailMaps || {}).flatMap(map => (map instanceof Map ? [...map.values()] : []));
}

function hasActiveDetailForMenu(menu, detailMaps) {
  const menuKeys = recipeIdentityKeys(menu);
  return allDetailRecipes(detailMaps).some(
    recipe =>
      hasDetailRecipeComponents(recipe) &&
      recipeIdentityKeys(recipe).some(key => menuKeys.includes(key))
  );
}
...
function costForPrice(p, ctx) {
  ...
  let baseCost = 0;
  let hasAuthoritativeDetail = false;
  if (map && p.menuCode) {
    const rec = map.get(p.menuCode);
    if (rec && hasDetailRecipeComponents(rec)) {
      hasAuthoritativeDetail = true;
      baseCost = detailComponentCost(rec.components);
    }
  }
  if (!hasAuthoritativeDetail && hasActiveDetailForMenu(p, ctx.detailMaps)) {
    hasAuthoritativeDetail = true;
  }
  if (!hasAuthoritativeDetail && !baseCost) {
...
function normalizeReportComponent(component, index) {
  const quantity = numOrNull(component?.quantity);
  const unitPrice = numOrNull(component?.unitPrice);
  return {
    lineNo: index + 1,
    ...
    unitPrice,
    subtotal:
      quantity != null && unitPrice != null ? Math.round(quantity * unitPrice) : null,
    note: text(component?.note),
  };
}

function normalizeLegacyComponent(component, sizeLabel, unitPriceMap, index) {
  const productCode = text(component?.productCode);
  const priceInfo = productCode ? unitPriceMap?.get(productCode) : null;
  const quantity = numOrNull(component?.quantities?.[sizeLabel]);
  const unitPrice = numOrNull(priceInfo?.unitPrice);
  return normalizeReportComponent(
    {
      productCode,
      ingredientName: component?.ingredientName || priceInfo?.ingredientName,
      quantity,
      unit: component?.unitType || priceInfo?.baseUnitType || 'g',
      unitPrice,
      note: component?.note,
    },
    index
...
function buildRecipeRow({ source, kind, categoryLabel, recipe, components, size }) {
  ...
  return {
    ...
    componentCount: safeComponents.filter(component => component.ingredientName || component.productCode)
      .length,
 ...
 * 상세 레시피 4종을 우선 출력하고, 상세 레시피가 없는 구형 cost_recipes는 폴백 포함합니다.
 */
export function buildRecipePrintRows({ detailMaps = {}, legacyRecipes = [], unitPriceMap } = {}) {
  const rows = [];
  const pendingDetailRows = [];
  const activeDetailRecipes = [];

  for (const meta of DETAIL_RECIPE_KINDS) {
    const map = detailMaps[meta.id];
    const recipes = map instanceof Map ? [...map.values()] : [];
    for (const recipe of recipes) {
      const components = (Array.isArray(recipe?.components) ? recipe.components : []).map(
      ...
      if (hasDetailRecipeComponents(recipe)) {
        rows.push(row);
        activeDetailRecipes.push(row);
      } else {
        pendingDetailRows.push(row);
      }
    }
  }

  const activeDetailKeys = buildActiveDetailRecipeKeySet(activeDetailRecipes);
  const replacedPendingKeys = new Set();

  for (const recipe of Array.isArray(legacyRecipes) ? legacyRecipes : []) {
    if (recipeMatchesKeySet(recipe, activeDetailKeys)) continue;
    const sizes =
      Array.isArray(recipe?.sizes) && recipe.sizes.length ? recipe.sizes : [{ label: '단일' }];
    for (const size of sizes) {
      const sizeLabel = text(size?.label || '단일') || '단일';
      const components = (Array.isArray(recipe?.ingredients) ? recipe.ingredients : []).map(
        (component, index) => normalizeLegacyComponent(component, sizeLabel, unitPriceMap, index)
      );
      rows.push(
        buildRecipeRow({
          source: 'legacy',
          kind: 'legacy',
          categoryLabel: recipe?.menuCategory || '기타',
          recipe,
          components,
          size: sizeLabel,
        })
    ...
    for (const key of recipeIdentityKeys(recipe)) replacedPendingKeys.add(key);
  }

  rows.push(
    ...pendingDetailRows.filter(
      row => !recipeIdentityKeys(row).some(key => replacedPendingKeys.has(key))
    )
  );
...
export function buildRecipePrintMenus(recipeRows = []) {
  ...
  for (const row of Array.isArray(recipeRows) ? recipeRows : []) {
    ...
    components.forEach((component, index) => {
      ...
      if (!item.productCode && component?.productCode) item.productCode = text(component.productCode);
  ...
  for (const catLabel of catKeys) {
    ...
    const menus = catPrices.map(p => {
      ...
      if (cost === 0 && !matchEdge(p.category)) {
        ...
        else if (!map.get(p.menuCode))
          reason = ctx.recipeByName.get(p.menuName) ? '레시피 원가 0' : '레시피 미등록';
