...
function costForPrice(p, ctx) {
  ...
  if (map && p.menuCode) {
    ...
    if (rec && hasDetailRecipeComponents(rec)) {
      baseCost = detailComponentCost(rec.components, ctx.upm);
    }
  }
 ...
 */
export function buildRecipePrintRows({ detailMaps = {}, unitPriceMap = new Map() } = {}) {
  const rows = [];
  ...
  for (const meta of DETAIL_RECIPE_KINDS) {
    ...
    for (const recipe of recipes) {
      const components = (Array.isArray(recipe?.components) ? recipe.components : []).map(
        (component, index) => normalizeReportComponent(component, index, unitPriceMap)
      );
