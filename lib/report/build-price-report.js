...
export function buildPriceReportData(baseRows, latestRows, threshold) {
  ...
  const overall = { cat: '전체', total: 0, up: 0, down: 0, newItem: 0, del: 0, sum: 0, count: 0 };
  for (const c of changes) {
    overall.total++;
    const pct = Math.abs(safeChangeRate(c.changeRate) * 100);
    if (c.changeStatus === '인상') { overall.up++; overall.sum += pct; overall.count++; }
    if (c.changeStatus === '인하') { overall.down++; overall.sum += pct; overall.count++; }
    if (c.changeStatus === '신규') overall.newItem++;
    if (c.changeStatus === '삭제') overall.del++;
  }
  return { changes, catSummary: overall.total > 0 ? [overall] : [] };
}
