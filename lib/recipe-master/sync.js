...
export function recipeStoreKindForCategory(category) {
  ...
  if (isExtraToppingCategory(category)) return 'topping';
...
export function normalizeRecipeMasterComponents(components) {
  return (Array.isArray(components) ? components : [])
    .map(component => ({
      ...
      quantity: optionalNonNegativeNumber(component?.quantity),
      unit: normalizeCostBaseUnit(component?.unit),
      unitPrice: optionalNonNegativeNumber(component?.unitPrice),
...
export function buildRecipeMasterRecipePayload(source, components = []) {
  ...
  const displayGroupKey = String(source?.displayGroupKey || '').trim();
  if (category) payload.category = category;
  if (displayGroupKey) payload.displayGroupKey = displayGroupKey;
  return payload;
}
...
export function recipeSyncTargetLabel(kind) {
  ...
  if (kind === 'topping') return '추가토핑 원가';
  return '미지원';
}
...
export function buildRecipeMasterMenuPayload(draft) {
  return {
    ...
    price: optionalNonNegativeNumber(draft?.price),
