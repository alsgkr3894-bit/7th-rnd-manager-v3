...
export function normalizeRecipeMasterComponents(components) {
  return (Array.isArray(components) ? components : [])
    .map(component => ({
      ...
      quantity:
        component?.quantity != null && component.quantity !== '' ? Number(component.quantity) : null,
      unit: String(component?.unit || 'g').trim(),
      unitPrice:
...
export function buildRecipeMasterMenuPayload(draft) {
  return {
    ...
    size:
      draft?.size && draft.size !== '단일'
        ? String(draft.size).trim()
        : null,
