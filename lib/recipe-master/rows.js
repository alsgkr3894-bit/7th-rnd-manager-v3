import { summarizeMenuRecipe } from '@/lib/menu-master/recipe-summary';
import { effectiveComponentsRawCost } from '@/lib/cost/shared/effective-cost';
...
export function buildRecipeMasterRows({ menuRows, recipeMaps, ingredientIndex, unitPriceMap }) {
  return asObjectArray(menuRows)
    .map(menu => {
      ...
      const summary = summarizeMenuRecipe(menu, recipe, unitPriceMap, { kind });
      return {
        menu,
        kind,
        recipe,
...
export function calcComponentsCost(components, unitPriceMap = new Map()) {
  return effectiveComponentsRawCost(components, unitPriceMap);
}
