 ...
 *   - ref_registered_overrides ("미등록" 자동 판정 개별 해제 — 이름 매칭 실패로
 *     잘못 미등록 처리된 등록 메뉴를 사용자가 되돌림)
 *
...
/* ============================================================
   "미등록" 자동 판정 개별 해제 (ref_registered_overrides) — menuName 필드만 씀,
   구조는 ref_discontinued와 동일
============================================================ */

/** 사용자가 "메뉴마스터에 등록된 메뉴다(미등록 아님)"로 확인한 판매명 전체 조회 */
export async function getRefRegisteredOverrides() {
  if (!hasStore('ref_registered_overrides')) return [];
  return asObjectArray(await getAll('ref_registered_overrides'));
}

/**
 * "미등록" 판정 해제 추가 — 판매량 보고서 순위표에서 자동으로 "미등록"으로 판정됐지만
 * 실제로는 메뉴마스터에 등록된 메뉴일 때(이름 매칭 실패) 사용자가 직접 해제한다.
 * @param {{ menuName: string }} input
 * @throws Error — menuName이 비어있거나 이미 등록된 경우
 */
export async function addRefRegisteredOverride({ menuName }) {
  await assertActiveAdmin('미등록 판정 해제');
  if (!menuName?.trim()) throw new Error('menuName은 필수입니다');
  const all = await getRefRegisteredOverrides();
  if (all.some(row => sameExcluded(row, menuName))) throw new Error('이미 해제된 메뉴입니다');
  await runTransaction(['ref_registered_overrides'], 'readwrite', tx => {
    tx.objectStore('ref_registered_overrides').add({
      menuName: menuName.trim(),
      createdAt: new Date().toISOString(),
    });
  });
...
/**
 * 메뉴명으로 "미등록" 판정 해제를 찾아 되돌린다(다시 미등록 판정으로 복귀) — 순위표
 * 체크박스에는 id 없이 표시명만 있으므로 UI가 id를 따로 들고 있지 않아도 되게 한다.
 * 일치하는 행이 없으면 아무 일도 하지 않는다.
 * @param {string} menuName
 * @returns {Promise<{ deleted: boolean }>}
 */
export async function deleteRefRegisteredOverrideByName(menuName) {
  await assertActiveAdmin('미등록 판정 복원');
  const all = await getRefRegisteredOverrides();
  const match = all.find(row => sameExcluded(row, menuName));
  if (!match) return { deleted: false };
  await runTransaction(['ref_registered_overrides'], 'readwrite', tx => {
    tx.objectStore('ref_registered_overrides').delete(match.id);
  });
  return { deleted: true };
}
