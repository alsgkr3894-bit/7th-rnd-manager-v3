...
/** @private inA/inB 판정 함수를 받아 두 그룹(A/B) 집계 결과를 만드는 공통 로직 */
function buildCompareCore(rows, inA, inB, opts = {}) {
...
export function buildPeriodCompare(rows, periodA, periodB, opts = {}) {
  ...
  return {
    periodA: safePeriodA,
    periodB: safePeriodB,
    ...buildCompareCore(rows, inA, inB, opts),
  };
}

/**
 * 분기·연 단위처럼 여러 달을 하나의 기간으로 묶어 두 기간(A/B)을 비교한다.
 * @param {Array} rows
 * @param {Array<{year, month}>} monthsA - 기준 기간에 속한 {year, month} 목록
 * @param {Array<{year, month}>} monthsB - 비교 기간에 속한 {year, month} 목록
 * @param {object} opts — { groupBy: 'menu'|'group', category?: string|string[]|null, topN: number }
 */
export function buildRangeCompare(rows, monthsA, monthsB, opts = {}) {
  const keyOf = ({ year, month }) => `${asFiniteNumber(year, 0)}-${asFiniteNumber(month, 0)}`;
  const setA = new Set(asObjectArray(monthsA).map(keyOf));
  const setB = new Set(asObjectArray(monthsB).map(keyOf));
  const rowKeyOf = r => `${asFiniteNumber(r.year, 0)}-${asFiniteNumber(r.month, 0)}`;
  const inA = r => setA.has(rowKeyOf(r));
  const inB = r => setB.has(rowKeyOf(r));

  return {
    monthsA: asObjectArray(monthsA),
    monthsB: asObjectArray(monthsB),
    ...buildCompareCore(rows, inA, inB, opts),
  };
}
