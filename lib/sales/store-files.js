...
export async function saveSalesUpload({ meta, classifiedRows, groupedIssues, log }) {
  await assertActiveAdmin('판매량 업로드 저장');
  ...
  try {
    await runTransaction(
      ...
      'readwrite',
      tx => {
        const fileStore = tx.objectStore('sales_files');
        const dupReq = fileStore.index('year_month').getKey([meta.year, meta.month]);
        dupReq.onerror = () => tx.abort();
        dupReq.onsuccess = () => {
          if (dupReq.result != null) {
            duplicateInTransaction = true;
            tx.abort();
            return;
          }
          const fileReq = fileStore.add(meta);
    ...
    if (duplicateInTransaction) throw new Error('DUPLICATE_MONTH');
    throw err;
  }
  return fileId;
}
...
export async function deleteSalesFile(fileId) {
  await assertActiveAdmin('판매량 업로드 삭제');
...
export async function replaceFileClassification(file, classifiedRows, groupedIssues) {
  await assertActiveAdmin('판매량 분류 재반영');
...
function _saveIssuesInTx(tx, groupedIssues, rowIds, fileId, meta, onComplete) {
  ...
  issues.forEach(it => {
    ...
    req.onerror = () => tx.abort();
    req.onsuccess = () => {
      if (++added === issues.length) onComplete();
    };
  });
}
