...
/** 다음 마이크로태스크까지 이벤트 루프를 양보해 UI 블로킹 방지 */
 ...
 */
export async function reclassifyAllFiles({ onProgress } = {}) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    const sourceRows = stored
      ...
      .map(r => ({
        ...
        quantity: r.quantity,
        revenue: r.revenue ?? r.amount ?? r.salesAmount ?? r.totalAmount ?? 0,
    ...
    // 파일 간 UI 이벤트 루프 양보 (대용량 데이터셋 블로킹 방지)
    await yieldToUI();
  }
