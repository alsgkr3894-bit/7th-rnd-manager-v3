...
export function checkDuplicateInTx(tx, storeName, normalizedMenuName, actionType, setAbortReason) {
  ...
  req.onerror = () => {
    setAbortReason({ code: 'DUPLICATE_CHECK_FAILED', message: '중복 검사 중 오류가 발생했습니다.' });
    tx.abort();
  };
}

/** 기준 데이터 저장 (ref_sales_aliases | sales_rules | ref_excluded) */
export function saveIssueBasisInTx(tx, storeName, normalizedMenuName, actionType, actionData, setAbortReason) {
  const now = new Date().toISOString();
  const record =
    actionType === 'alias'
      ? { rawName: normalizedMenuName, mappedName: actionData.outputName, enable: true, createdAt: now }
      : actionType === 'rule'
        ? {
            rawMenuName: normalizedMenuName, matchType: 'exact', pattern: normalizedMenuName,
            category: actionData.category, groupName: actionData.groupName,
            detailName: actionData.detailName || null, enable: true, createdAt: now,
          }
        : { menuName: normalizedMenuName, createdAt: now };

  const req = tx.objectStore(storeName).add(record);
  req.onerror = () => {
    setAbortReason({ code: 'BASIS_SAVE_FAILED', message: '기준 데이터 저장 중 오류가 발생했습니다.' });
    tx.abort();
  };
}
 ...
 */
export function reprocessRowsInTx(tx, rowIds, actionType, actionData, deps, setAbortReason, onComplete) {
  if (!rowIds || rowIds.length === 0) { onComplete?.(); return; }
  ...
  rowIds.forEach(rowId => {
    ...
    getReq.onsuccess = () => {
      ...
      if (!row) {
        setAbortReason({ code: 'RELATED_ROW_NOT_FOUND', message: `처리할 행을 찾을 수 없습니다. (rowId: ${rowId})` });
        tx.abort();
        return;
      }

      const updated = applyAction(row, actionType, actionData, deps);
      if (actionType === 'alias' && updated.status !== 'classified' && updated.status !== 'excluded') {
        unclassifiedAfterAlias.push(rowId);
      }

      const putReq = rowStore.put(updated);
      putReq.onerror = () => {
        setAbortReason({ code: 'RELATED_ROW_UPDATE_FAILED', message: '행 업데이트 중 오류가 발생했습니다.' });
        tx.abort();
      };
      putReq.onsuccess = () => {
        ...
        if (actionType === 'alias' && unclassifiedAfterAlias.length > 0) {
          setAbortReason({
            ...
            message: '별칭 대상이 기존 분류 규칙과 연결되지 않습니다. 분류 규칙 등록을 사용해 주세요.',
          });
          tx.abort();
          return;
        }
    ...
    getReq.onerror = () => {
      setAbortReason({ code: 'RELATED_ROW_QUERY_FAILED', message: '행 조회 중 오류가 발생했습니다.' });
      tx.abort();
    };
  });
}
...
function applyAction(row, actionType, actionData, deps) {
  ...
  if (actionType === 'alias') {
    ...
    const result = deps.classifyMenuStatus({
      ...
      normalizedMenuName, mappedMenuName, matchedRule,
    ...
    return {
      ...
      groupName: result.groupName, detailName: result.detailName,
    };
  }
  if (actionType === 'rule') {
    return {
  ...
  return {
    ...row, updatedAt: now,
    status: 'excluded', category: null, groupName: null, detailName: null,
  };
}
...
export function updateIssueStatusInTx(tx, issueId, setAbortReason) {
  ...
  getReq.onsuccess = () => {
    ...
    if (!issue) {
      setAbortReason({ code: 'ISSUE_RECHECK_NOT_FOUND', message: '이슈를 다시 확인할 수 없습니다.' });
      tx.abort();
      return;
    }
    const putReq = store.put({ ...issue, status: 'resolved', resolvedAt: new Date().toISOString() });
    putReq.onerror = () => {
      setAbortReason({ code: 'ISSUE_UPDATE_FAILED', message: '이슈 상태 변경 중 오류가 발생했습니다.' });
      tx.abort();
    };
  };
  getReq.onerror = () => {
    setAbortReason({ code: 'ISSUE_RECHECK_FAILED', message: '이슈 재조회 중 오류가 발생했습니다.' });
    tx.abort();
  };
}
