...
function extractMonthPeriod(text) {
  ...
  if (startDate.getFullYear() !== sy
      || startDate.getMonth() !== sM - 1
      || startDate.getDate() !== sd) return null;
...
export function parseSalesPeriod(rows) {
  ...
  return {
    success: false,
    reason: '파일에서 판매 기간(월 전체: YYYY-MM-01 ~ YYYY-MM-말일)을 찾을 수 없습니다. 월 일부 기간은 업로드할 수 없습니다.',
  };
}
