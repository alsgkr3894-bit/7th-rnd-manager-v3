...
export function safeSheetName(name, { suffix = '' } = {}) {
  const clean = String(name).replace(/[\\\/\?\*\[\]:]/g, '_');
  const cleanSuffix = String(suffix).replace(/[\\\/\?\*\[\]:]/g, '_');
  const base = clean.slice(0, SHEET_NAME_LIMIT - cleanSuffix.length);
  return (base + cleanSuffix).slice(0, SHEET_NAME_LIMIT);
}
...
export async function exportSingleMonthXlsx(period, detail, groupRanking) {
  ...
  const summaryRows = [
    ['카테고리', '판매량(개)', '비중(%)'],
    ...detail.categories.map(c => [c.name, c.value, Math.round((c.share ?? 0) * 1000) / 10]),
    [],
  ...
  for (const cat of detail.categories) {
    ...
    sorted.forEach((g, idx) => {
      ...
      rows.push([idx + 1, g.name, g.quantity, Math.round(share * 100) / 100]);
    });
  ...
  XLSX.writeFile(
    wb,
    withDownloadDateSuffix(`${getActiveBrand().name}_${fmtPeriodKo(period)} 메뉴 판매량.xlsx`)
  );
}
...
export async function exportCompareXlsx(periodA, periodB, compare) {
  ...
  for (const [cat, rows] of byCategory) {
    ...
    const aoa = [
      [
        '순위',
        '메뉴명',
        `기준A(${fmtPeriod(periodA)})`,
        `비교B(${fmtPeriod(periodB)})`,
        '증감',
        '증감률(%)',
        '상태',
      ],
  ...
  XLSX.writeFile(
    ...
    withDownloadDateSuffix(
      `${getActiveBrand().name}_${fmtPeriodKo(periodA)}vs${fmtPeriodKo(periodB)} 메뉴 판매량 비교.xlsx`
    )
  );
}
