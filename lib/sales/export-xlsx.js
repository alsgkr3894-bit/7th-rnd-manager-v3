...
export async function exportSingleMonthXlsx(period, detail, groupRanking) {
  ...
  XLSX.writeFile(
    wb,
    `${getActiveBrand().name}_${fmtPeriodKo(period)}_메뉴판매량_${todayStr()}.xlsx`
  );
}
...
export async function exportCompareXlsx(periodA, periodB, compare) {
  ...
  XLSX.writeFile(
    wb,
    `${getActiveBrand().name}_${fmtPeriodKo(periodA)}vs${fmtPeriodKo(periodB)}_메뉴판매량비교_${todayStr()}.xlsx`
  );
}
