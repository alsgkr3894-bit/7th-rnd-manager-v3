...
export function useSalesUpload() {
  ...
  async function handleFile(file, fileError) {
    ...
    try {
      ...
      const { classifiedRows, groupedIssues } =
        classifyAndPrepare(result.validRows, result.period.year, result.period.month, classifier);
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      import('@/lib/work-log')
        .then(m => m.logWork('UPLOAD', `판매량 업로드: ${formatPeriodKor(meta)} (${meta?.totalRows ?? preview.classifiedRows?.length ?? ''}건)`))
        .catch(() => {});
      setPreview(null);
      setStage('idle');
      await refreshHistory();
