 ...
 *   success: true,
 *   period: { year, month },
 *   headerColumns: { menuColumnIndex, quantityColumnIndex, revenueColumnIndex },
 *   revenueSummary: { hasRevenueColumn, totalRevenue, warningCount },
...
export function validateSalesFile(rows) {
  ...
  if (!headerResult) {
    return {
      success: false,
      reason:
        '필수 헤더를 찾을 수 없습니다. (허용: "메뉴명", "메뉴 명" / "판매량(개)", "판매량 (개)")',
    };
  }
  ...
  const dataResult = parseAndValidateRows(
    ...
    headerResult.revenueColumnIndex
  );
  ...
  if (dataResult.invalidRows.length > 0) {
    return {
      ...
      revenueWarningRows: dataResult.revenueWarningRows,
      summary: {
  ...
  return {
    ...
    headerColumns: {
      ...
      revenueColumnIndex: headerResult.revenueColumnIndex,
      menuColumnName: headerResult.menuColumnName,
      quantityColumnName: headerResult.quantityColumnName,
      revenueColumnName: headerResult.revenueColumnName,
    },
    validRows: dataResult.validRows,
    revenueWarningRows: dataResult.revenueWarningRows,
    revenueSummary: {
      hasRevenueColumn: headerResult.revenueColumnIndex >= 0,
      totalRevenue: dataResult.validRows.reduce((sum, row) => sum + row.revenue, 0),
      warningCount: dataResult.revenueWarningRows.length,
    },
    summary: {
      totalRows: dataResult.totalRows,
      validCount: dataResult.validRows.length,
