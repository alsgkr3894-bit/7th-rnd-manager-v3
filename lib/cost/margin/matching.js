...
export function mergeRecipeIntoDetail(d, recipesByName, toNum) {
  ...
  const detailComponentSizes =
    d.detailComponentSizes instanceof Set ? d.detailComponentSizes : new Set();
  for (const label of sizeByLabel.keys()) {
    if (detailComponentSizes.has(label)) continue;
