...
function addMetric(bucket, value) {
  if (!bucket || !Number.isFinite(value)) return;
  bucket.sum += value;
  bucket.count += 1;
}

function toMetric(bucket) {
  return {
...
export function isMarginPizzaCategory(value) {
  const category = normalizeCategory(value);
  return category === '피자' || category.startsWith('피자/');
}
...
/** 카테고리·엣지·사이즈·플랫폼 등 "체크 해제 시에만 제외" 방식 선택 상태를 판별한다 */
export const isMarginReportOptionSelected = isSelected;
...
export function filterMarginReportRows(
  ...
  for (const row of asRows(rows)) {
    ...
    const category = normalizeCategory(row.menuCategory);
    if (!isSelected(categorySelection, category)) continue;

    // "엣지별 출력"·"사이즈별 출력"은 UI 힌트대로 피자 전용 컨트롤이다(엣지=크러스트/도우
    // 종류, 사이즈=L/R). 사이드·1인피자·세트박스 등은 전부 BASE_EDGE_KEY/단일로 묶이므로,
    // 이 필터를 전 카테고리에 적용하면 "석쇠기본"이나 "단일"을 해제했을 때 카테고리
    // 체크박스는 그대로인데도 비-피자 항목 전체가 조용히 사라진다.
    const isPizza = isMarginPizzaCategory(category);
    if (isPizza && !isSelected(edgeSelection, getMarginReportEdgeKey(row))) continue;

    const rowSizes = Array.isArray(row.sizes) ? row.sizes : [];
    const selectedSizes = isPizza
      ? rowSizes.filter(size => isSelected(sizeSelection, normalizeMarginSizeGroup(size?.label)))
      : rowSizes;
...
export function buildMarginPizzaCostRateSummary(rows, activePlatform, discount) {
  const total = createMetricBucket();
  const sizeBuckets = {
    L: createMetricBucket(),
    R: createMetricBucket(),
  };
  const categoryBuckets = new Map();

  for (const row of asRows(rows)) {
    const category = normalizeCategory(row.menuCategory);
    if (!isMarginPizzaCategory(category)) continue;
    ...
    for (const size of Array.isArray(row.sizes) ? row.sizes : []) {
      ...
      addMetric(total, costRate);
      addMetric(sizeBuckets[label], costRate);
      addMetric(categoryBucket.total, costRate);
      addMetric(categoryBucket.sizes[label], costRate);
    }
  }

  return {
    total: toMetric(total),
    sizes: {
      L: toMetric(sizeBuckets.L),
      R: toMetric(sizeBuckets.R),
    },
    categories: [...categoryBuckets.values()]
      .map(bucket => ({
        category: bucket.category,
        total: toMetric(bucket.total),
        sizes: {
          L: toMetric(bucket.sizes.L),
          R: toMetric(bucket.sizes.R),
        },
      }))
      .filter(bucket => bucket.total.count > 0)
      .sort((a, b) => {
        if (a.category === '피자') return -1;
        if (b.category === '피자') return 1;
