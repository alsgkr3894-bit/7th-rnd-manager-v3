...
function platformMetaLabel(activePlatform, allPlatforms) {
...
export async function exportMarginExcel(
  ...
  const buildRows = (categoryRows, categorySizeLabels) =>
    allPlatforms
      ? buildMarginExcelRowsAllPlatforms(categoryRows, categorySizeLabels, viewMode, allPlatforms, discount)
  ...
  if (!allPlatforms) {
    const pizzaCostRateSheetRows = buildMarginPizzaCostRateSheetRows(rows, activePlatform, discount);
