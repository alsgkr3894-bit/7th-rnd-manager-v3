...
function printTitle() {
  return makeFileNameWithBrand('원가마진표', 'pdf').replace(/\.pdf$/i, '');
}
  ...
  const bodyHtml = bodyRows.length
    ? bodyRows
        .map(
          row => `<tr>${headers
            .map((_, index) => {
              const cls = index === 0 ? 'name' : index >= 2 ? 'num' : '';
...
export function buildMarginPrintHtml(rows, sizeLabels, viewMode, activePlatform, discount) {
...
<body>
  <main class="page">
    <header class="header">
      ...
      <div class="meta">플랫폼: ${esc(platformName)}<br>보기: ${esc(modeLabel)} · 할인: ${esc(discountLabel(discount))}</div>
    </header>
    <div class="summary">
      ...
      <div><span>표시 기준</span><strong>${esc(modeLabel)}</strong></div>
    </div>
    ${sectionHtml}
  </main>
...
export function printMarginPdf(rows, sizeLabels, viewMode, activePlatform, discount) {
  return openPrintWindow(buildMarginPrintHtml(rows, sizeLabels, viewMode, activePlatform, discount), {
    width: 1200,
    height: 900,
  });
}
