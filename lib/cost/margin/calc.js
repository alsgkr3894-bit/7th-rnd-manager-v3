...
/**
 * 고정 fee 한 항목의 표시용 차감액 — 사이즈별 오버라이드가 있으면 L/R로 나눠서,
 * 없으면 공통값 하나로 반환한다 (플랫폼 필터 바의 "차감:" 배지 표시용).
 * @param {{value:number, sizeOverrides?:{L?:number|string, R?:number|string}}} fee
 * @returns {{common:number}|{L:number, R:number}}
 */
export function fixedFeeDisplayAmounts(fee) {
  const base = parseFloat(fee?.value) || 0;
  const lOverride = fee?.sizeOverrides?.L;
  const rOverride = fee?.sizeOverrides?.R;
  const hasL = lOverride != null && lOverride !== '' && !isNaN(parseFloat(lOverride));
  const hasR = rOverride != null && rOverride !== '' && !isNaN(parseFloat(rOverride));
  if (!hasL && !hasR) return { common: base };
  return {
    L: hasL ? parseFloat(lOverride) : base,
    R: hasR ? parseFloat(rOverride) : base,
  };
}
