 ...
 * @returns {Promise<{ applied: number, skipped: number }>} 적용/스킵된 레코드 수
 */
export async function commitBulkPrice(matched) {
  if (!Array.isArray(matched) || matched.length === 0) return { applied: 0, skipped: 0 };
  await assertActiveAdmin('식자재 일괄 단가 업데이트');
  if (!hasStore('cost_ingredients')) throw new Error('cost_ingredients store 없음');

  const now = new Date().toISOString();

  // 기존 레코드를 병렬 조회 (순서 보장)
  const existing = await Promise.all(matched.map(item => getById('cost_ingredients', item.id)));
  ...
  if (records.length === 0) return { applied: 0, skipped };
  await bulkPut('cost_ingredients', records);

  return { applied: records.length, skipped };
}
