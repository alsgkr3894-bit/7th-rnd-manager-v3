...
function rawSum(components, unitPriceMap) {
  if (!Array.isArray(components)) return 0;
  return components.reduce((acc, c) => acc + effectiveComponentSubtotal(c, unitPriceMap), 0);
}
 ...
 */
function variantCost({ recipe, edgeIndex, size, variant, price, unitPriceMap }) {
  // 씬도우는 L만 (R 메뉴는 해당 없음)
  if (variant.onlyL && size !== 'L') {
    return { cost: null, rate: null, available: false };
  }

  const baseRaw = recipe ? rawSum(recipe.components, unitPriceMap) : 0;

  let edgeRaw = 0;
  if (variant.useEdge) {
    const edge = edgeIndex[variant.edgeType]?.[size];
    if (!edge) return { cost: null, rate: null, available: false }; // 엣지 미등록 → 계산 불가
    edgeRaw = rawSum(edge.components, unitPriceMap);
  }
 ...
 * @param {Map<string,object>} [args.unitPriceMap] 제품코드 → 최신 단위 단가
 * @returns {object[]} [{ menu, byVariant: { [key]: variantCost } }]
 */
export function buildPizzaSummary({ menus, recipeMap, edges, unitPriceMap = new Map() }) {
