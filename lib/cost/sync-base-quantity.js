 ...
 *   - price row의 salesUnit을 g/개 기준으로 환산할 수 없는 경우
 *   - 매칭되는 ingredient가 없는 price row
 *   - 이미 동일한 값인 경우 (unchanged 로 집계)
 */

import { getById, bulkPut, hasStore } from '@/lib/db';
import { normalizePurchaseQuantity } from '@/lib/cost/unit-policy';
...
export function buildSyncPlan(priceRows, ingredients) {
  ...
  for (const row of priceRows) {
    const sourceQty = parseQty(row.quantity);
    if (sourceQty === null) continue; // 수량 없음 → 스킵
    ...
    const normalized = normalizePurchaseQuantity(
      sourceQty,
      row.salesUnit || ing.baseUnitType || 'g'
    );
    if (!normalized) {
      unsupported++;
      continue;
    ...
    const newQty = normalized.quantity;
    const unit = normalized.unit;

    // 이미 동일하면 unchanged
    if (oldQty === newQty && ing.baseUnitType === unit) {
      unchanged++;
      continue;
    }
  ...
  return { changes, unchanged, unmatched, unsupported };
}
...
export async function applySyncPlan(changes) {
  ...
  await assertActiveAdmin('제때 수량 기준수량 동기화');
  if (!hasStore('cost_ingredients')) throw new Error('cost_ingredients store 없음');

  const now = new Date().toISOString();

  // 현재 레코드를 병렬 조회 (혹시 사이에 변경된 경우 대비)
  const existing = await Promise.all(changes.map(c => getById('cost_ingredients', c.id)));
