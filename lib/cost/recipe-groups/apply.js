/**
 * lib/cost/recipe-groups/apply.js — 공통묶음 그룹의 레시피 선택 후보 판별 (순수 함수)
 *
 * 레시피마다 "어떤 그룹을 선택 후보로 보여줄 수 있는가"를 계산할 때 사용한다.
 * 같은 menuCategory 를 가진 레시피가 많으므로, 카테고리별 결과를 캐싱해
 * 그룹 전체 순회 횟수를 (레시피 수)에서 (서로 다른 카테고리 수)로 줄인다.
 */

/**
 * 그룹이 특정 메뉴 카테고리에서 선택 후보가 되는지 판별.
 * defaultCategories 의 한 항목이 menuCategory 와 정확히 일치하거나
 * 'cat/' 접두사로 시작하면 후보 대상이다. 카테고리 미지정 그룹은
 * 의도치 않은 전체 메뉴 자동 적용을 막기 위해 후보에서 제외한다.
 *
...
export function groupAppliesToCategory(group, menuCategory) {
  const cats = group?.defaultCategories || [];
  if (!cats.length) return false;
  const mc = menuCategory || '';
  return cats.some(c => mc === c || mc.startsWith(c + '/'));
}

/**
 * menuCategory → 선택 후보 그룹 id Set 을 캐싱하는 리졸버를 만든다.
...
export function createDefaultGroupResolver(allGroups) {
  ...
  return menuCategory => {
    ...
    if (!set) {
      set = new Set(groups.filter(g => groupAppliesToCategory(g, key)).map(g => g.id));
      cache.set(key, set);
    }
    return set;
  };
