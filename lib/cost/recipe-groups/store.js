/**
 * lib/cost/recipe-groups/store.js — cost_recipe_groups CRUD
 *
 * 레코드 구조:
 *   id                autoIncrement PK
 *   name              묶음 이름 (예: '피자L 공통', '피자LR 공통')
 *   description       설명 (선택)
 *   sizes             사용하는 사이즈 레이블 배열 (예: ['L', 'R'])
 *   defaultCategories 자동 적용할 메뉴 카테고리 배열
 *   ingredients       [{ productCode, ingredientName, quantities:{L,R,...}, unitType }]
 *   updatedAt         ISO string
 */

import { getAll, put, deleteById, hasStore, runTransaction } from '@/lib/db';
import { normalizeCostBaseUnit } from '@/lib/cost/unit-policy';

const STORE = 'cost_recipe_groups';

export async function getAllRecipeGroups() {
  if (!hasStore(STORE)) return [];
  const rows = await getAll(STORE);
  return rows.sort((a, b) => (a.name || '').localeCompare(b.name || '', 'ko'));
}

export async function saveRecipeGroup(data) {
  if (!hasStore(STORE)) throw new Error(`${STORE} store 없음`);
  const ingredients = Array.isArray(data.ingredients)
    ? data.ingredients.map(line => ({ ...line, unitType: normalizeCostBaseUnit(line?.unitType) }))
    : [];
  const record = { ...data, ingredients, updatedAt: new Date().toISOString() };
  if (record.id == null) delete record.id;
  return await put(STORE, record);
}

export async function deleteRecipeGroup(id) {
  if (!hasStore(STORE)) throw new Error(`${STORE} store 없음`);
  await deleteById(STORE, id);
  // 연결된 레시피의 groupIds에서 해당 그룹 제거 (cascade)
  if (hasStore('cost_recipes')) {
    const allRecipes = await getAll('cost_recipes');
    const affected = allRecipes.filter(r => Array.isArray(r.groupIds) && r.groupIds.includes(id));
    for (const r of affected) {
      await runTransaction(['cost_recipes'], 'readwrite', tx => {
        tx.objectStore('cost_recipes').put({
          ...r,
          groupIds: r.groupIds.filter(gid => gid !== id),
        });
      });
    }
  }
}
