...
export async function getAllEdges() {
  ...
  return rows.sort((a, b) => {
    const ta = TYPE_ORDER[a.edgeType] ?? 99,
      tb = TYPE_ORDER[b.edgeType] ?? 99;
...
export async function upsertEdge(data) {
  await assertActiveAdmin('엣지/도우 저장');
...
export async function deleteEdge(id) {
  await assertActiveAdmin('엣지/도우 삭제');
  if (!hasStore(STORE)) throw new Error(`${STORE} store 없음`);
  await runTransaction([STORE], 'readwrite', tx => {
    s(tx).delete(id);
  });
...
export async function resetAllEdges() {
  await assertActiveAdmin('엣지/도우 전체 초기화');
  ...
  logWork('RESET', `엣지·도우 전체 초기화 (${count}건 삭제)`).catch(() => {});
...
export async function seedEdges() {
  await assertActiveAdmin('엣지/도우 시드');
  if (!hasStore(STORE)) throw new Error(`${STORE} store 없음`);
  // 7번가(main) 전용 시드 — 다른 브랜드 DB에 7번가 엣지 주입 방지
  ...
  return {
    ...
    components: Array.isArray(data.components)
      ? data.components.map(component =>
          normalizeComponent(component, { allowNegativeQuantity: true })
        )
      : [],
    note: (data.note || '').trim(),
    // 마진표 파생행 노출 여부·접미사 (미지정 시 유형 기본값)
    expandInMargin:
      data.expandInMargin != null ? !!data.expandInMargin : defaultExpandInMargin(edgeType),
    marginSuffix: normalizeMarginSuffix(edgeType, data.marginSuffix || defaultMarginSuffix(edgeType)),
