...
/** 마진표 확장 제외 기본값 — 기본 5종은 모두 마진표에서 확인 가능해야 한다. */
export const NON_EXPAND_EDGE_TYPES = [];
...
/** 마진표 파생행 메뉴코드 접미사 기본값 (치즈크러스트→C, 골드스윗→G, 씬도우→s) */
...
export function defaultMarginSuffix(edgeType) {
  ...
  return normalizeMarginSuffix(t, EDGE_MARGIN_SUFFIX[t] || t.charAt(0) || 'E');
}

/** 레거시 씬도우 suffix(T/TH/씬)를 현재 메뉴코드 suffix(s)로 정규화 */
export function normalizeMarginSuffix(edgeType, suffix) {
  const t = (edgeType || '').trim();
  const raw = String(suffix ?? '').trim();
  if (t !== '씬도우') return raw || EDGE_MARGIN_SUFFIX[t] || t.charAt(0) || 'E';
  if (!raw || raw === '씬' || raw === '씬도우') return 's';
  const lower = raw.toLowerCase();
  if (lower === 't' || lower === 'th' || lower === 's') return 's';
  return raw;
}
...
export function edgeSeedKey(edge) {
  return `${(edge?.edgeType || '').trim()}|${(edge?.size || '').trim()}`;
}

export function missingEdgeSeedRows(edges = []) {
  const existingKeys = new Set((Array.isArray(edges) ? edges : []).map(edgeSeedKey));
  return EDGE_DOUGH_SEED.filter(seed => !existingKeys.has(edgeSeedKey(seed)));
}

export function seedPlaceholderEdges(edges = []) {
  return missingEdgeSeedRows(edges).map(seed => ({
    ...seed,
    __seedPlaceholder: true,
    __key: `seed:${edgeSeedKey(seed)}`,
  }));
}
