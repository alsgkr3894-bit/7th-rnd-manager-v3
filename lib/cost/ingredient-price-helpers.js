...
export function buildIngredientUsageMap({
  ...
  // 단종 메뉴의 레시피는 남아 있어도 "지금 이 식자재를 쓰는 메뉴"가 아니다.
  const discontinuedCodes = new Set(
    menuMasters
      .filter(menu => menu?.status === 'discontinued' && menu?.menuCode)
      .map(menu => String(menu.menuCode).trim())
  );
  ...
  function addUsage(targetMap, key, menuName, category, sourceTypes) {
    if (!menuName) return;
    const menu = stripSizeLabel(menuName);
    if (!targetMap.has(key)) targetMap.set(key, new Map());
    const menuMap = targetMap.get(key);
    const prev = menuMap.get(menu);
    const sources = prev?.sources instanceof Set ? prev.sources : new Set();
    for (const type of sourceTypes) sources.add(type);
    menuMap.set(menu, { category: prev?.category || usageCategory(category), sources });
  }
  ...
  for (const [ingredientKey, menus] of ingredientToMenus) {
    ...
    for (const [menuCode, menu] of menus) {
      if (discontinuedCodes.has(String(menuCode).trim())) continue;
      const sourceTypes = Array.isArray(menu?.sources)
        ? menu.sources.map(s => s?.type).filter(Boolean)
        : [];
      if (!sourceTypes.length) sourceTypes.push('직접');
      addUsage(targetMap, key, menu?.menuName || menuCode, menu?.category, sourceTypes);
    }
  }
