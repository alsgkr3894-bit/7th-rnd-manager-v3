...
// 공통묶음(묶음관리)·엣지(엣지관리)로만 연결된 메뉴는 "제품별 사용현황"의 사용량에 넣지 않는다.
// 그 메뉴의 레시피 자체에 직접 들어있거나(직접) 파생 메뉴로 연결된 경우만 실사용으로 센다.
const DIRECT_USAGE_SOURCE_TYPES = new Set(['직접', '파생메뉴']);

function hasDirectUsage(menu) {
  return Array.isArray(menu?.sources) && menu.sources.some(s => DIRECT_USAGE_SOURCE_TYPES.has(s?.type));
}
...
export function buildIngredientUsageMap({
  ...
  for (const [ingredientKey, menus] of ingredientToMenus) {
    ...
    for (const [menuCode, menu] of menus) {
      if (!hasDirectUsage(menu)) continue;
