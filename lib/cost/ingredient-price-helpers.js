...
export function sumCompositePrice(compositeOf, priceLookup) {
  if (!Array.isArray(compositeOf) || compositeOf.length === 0) return null;
  const sum = compositeOf.reduce((acc, code) => {
    const pr = priceLookup.get(code);
    const val = pr?.priceWithTax ?? 0;
