...
function isUsageCategory(category) {
  return (
    isPizzaCategory(category, { includePersonal: false }) ||
    isPersonalPizzaCategory(category) ||
    isSideCategory(category)
  );
}
...
export function buildIngredientUsageMap({ allMeta, pizzaRecs, personalRecs, sideRecs, oldRecs }) {
  ...
  const activeDetailKeys = buildActiveDetailRecipeKeySet([
    ...(Array.isArray(pizzaRecs) ? pizzaRecs : []),
    ...(Array.isArray(personalRecs) ? personalRecs : []),
    ...(Array.isArray(sideRecs) ? sideRecs : []),
  ]);
  ...
  for (const r of oldRecs) {
    if (recipeMatchesKeySet(r, activeDetailKeys)) continue;
    const topCategory = (r.menuCategory || '').split('/')[0];
    if (!isUsageCategory(topCategory)) continue;
