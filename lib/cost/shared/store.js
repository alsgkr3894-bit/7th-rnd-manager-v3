...
export function normalizeComponent(c, options = {}) {
  const allowNegativeQuantity = !!options.allowNegativeQuantity;
  return {
    productCode: (c.productCode || '').trim() || null,
    ingredientName: (c.ingredientName || '').trim(),
    quantity: allowNegativeQuantity ? optionalNumber(c.quantity) : optionalNonNegativeNumber(c.quantity),
    unit: normalizeCostBaseUnit(c.unit),
    unitPrice: optionalNonNegativeNumber(c.unitPrice),
    note: (c.note || '').trim(),
  };
}
