...
import {
  buildEffectiveRecipeComponents,
  normalizeRecipeGroupIds,
} from '@/lib/cost/recipe-groups/effective';
...
export function normalizeCategory(cat) {
  ...
  if (isExtraToppingCategory(cat)) return MENU_CATEGORY.EXTRA_TOPPING;
...
export function detailStoreFor(rawCat, maps) {
  ...
  if (isExtraToppingCategory(c)) return maps.topping;
...
export function buildRows(menuPrices, detailMaps, unitPriceMap = new Map(), recipeGroups = []) {
  const rows = [];
      ...
      const rec =
        detailRecipes.find(
          recipe =>
            ...
            normalizeRecipeGroupIds(recipe?.selectedRecipeGroupIds).length > 0
        ) || detailRecipes[0];
      if (rec) {
        const entry = entries.find(e => e.menuCode === rec.menuCode) || firstEntry;
        const effectiveComponents = buildEffectiveRecipeComponents(
          {
            menuCode: rec.menuCode || entry?.menuCode,
            menuName,
            category,
            size: rec.size || entry?.size,
          },
          rec,
          recipeGroups
        );
        if (hasDetailRecipeComponents(rec) || effectiveComponents.length > 0) {
          cost = detailComponentCost(effectiveComponents, unitPriceMap);
        }
      }
    }
