 ...
 */
function link(ingredientToMenus, menuToIngredients, key, menuCode, menuName, category) {
  if (!key || !menuCode) return;

  if (!ingredientToMenus.has(key)) ingredientToMenus.set(key, new Map());
  ingredientToMenus.get(key).set(menuCode, { menuName: menuName || '', category: category || '' });
 ...
 * }}
 */
export function buildIngredientMenuMap({ menuMasters = [], detailRecipes = [], oldRecipes = [], groups = [], edges = [] }) {
  ...
  const expandEdges = edges.filter(e =>
    e.expandInMargin == null || e.expandInMargin === true
  );
