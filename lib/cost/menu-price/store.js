...
export async function addMenuPrice(data) {
  ...
  await syncMenuMasterFromPrices(allAfter).catch(e => console.error('[menu-master sync]', e));
  return { menuCode };
}
...
export async function replaceAllMenuPrices(items) {
  ...
  await syncMenuMasterFromPrices(records).catch(e => console.error('[menu-master sync]', e));
