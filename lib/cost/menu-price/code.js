 ...
 * P-ONE-001  → { category: '1인피자',            subCategory: '씬도우 전용' }
...
export const PIZZA_SUB_CATEGORY_OPTIONS = Object.freeze([
  { code: 'PS', label: '프리미엄 스페셜', category: '피자/프리미엄 스페셜' },
  { code: 'PR', label: '프리미엄', category: '피자/프리미엄' },
  { code: 'OR', label: '오리지널', category: '피자/오리지널' },
  { code: 'HH', label: '하프앤하프', category: '피자/하프앤하프' },
]);

export function getPizzaSubCategoryOption(code) {
  const key = String(code || '')
    .trim()
    .toUpperCase();
  return PIZZA_SUB_CATEGORY_OPTIONS.find(option => option.code === key) || null;
}

export function getMenuSubCategoryFromCode(code) {
  if (!code) return null;
  const parts = String(code).trim().toUpperCase().split('-');
  if (parts[0] !== 'P') return null;
  if (parts[1] === 'ONE') {
    return { code: 'ONE', label: '씬도우 전용', category: '1인피자', subCategory: '씬도우 전용' };
  }
  const option = getPizzaSubCategoryOption(parts[1]);
...
export function updateMenuCodeSubCategory(code, subCode) {
  const parts = String(code || '')
    .trim()
    .toUpperCase()
    .split('-')
    .filter(Boolean);
  const nextSub = String(subCode || '')
    .trim()
    .toUpperCase();
  if (parts[0] !== 'P' || !getPizzaSubCategoryOption(nextSub)) return String(code || '');
  const nextParts = parts.length >= 4 ? parts : ['P', nextSub, '001', 'L'];
  nextParts[1] = nextSub;
...
export function parseCategoryFromCode(code) {
  ...
  // 추가토핑: T-CHZ-001
  if (parts[0] === 'T') {
    return { category: '추가토핑', subCategory: parts[1] || '' };
  }
  ...
  if (parts[0] === 'P') {
    ...
    const subMeta = getPizzaSubCategoryOption(sub);
    if (subMeta) return { category: subMeta.category, subCategory: subMeta.label };
...
export function generateMenuCode({ category, size, menuName, menuCode }, existingRecords) {
  ...
  if (category === '피자') {
    const sameMenu = records.find(
      r => r.category === '피자' && r.menuName === menuName && r.menuCode
    );
    if (sameMenu) {
  ...
  // 1인피자 — 사이즈 접미사 없음 (P-ONE-001 …)
  if (category === '1인피자') {
    const base = maxBaseForPrefix(prefix, codes) + 1;
    return `${prefix}-${pad3(base)}`;
  }

  // 세트박스
  if (category === '세트박스') {
    const sameMenu = records.find(
      r => r.category === '세트박스' && r.menuName === menuName && r.menuCode
    );
    let base = sameMenu ? parseMenuCode(sameMenu.menuCode)?.base || 0 : 0;
    if (!base) base = maxBaseForPrefix(prefix, codes) + 1;
    return size && size !== '단일' ? `${prefix}-${pad3(base)}-${size}` : `${prefix}-${pad3(base)}`;
  }
