...
export async function getRecentActivities(limit = 8) {
  // menu_dev_notes는 브랜드 공유(main DB) store라 safeAll(활성 브랜드 DB)이 아니라
  // getAllNotesCached(항상 main DB + 활성 브랜드 필터)로 읽어야 한다.
  const [logs, notes] = await Promise.all([safeAll('upload_log'), getAllNotesCached()]);
  ...
  return events.sort((a, b) => (b.when > a.when ? 1 : b.when < a.when ? -1 : 0)).slice(0, limit);
}
