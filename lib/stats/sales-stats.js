...
import { safeAll, pickLatestYearMonth, shiftMonth, monthLabel, buildSparkline } from './_helpers';
...
function isStatRow(r, year, month) {
  return r && Number(r.year) === year && Number(r.month) === month && r.status === 'classified';
}

function statQuantity(row) {
  return asFiniteNumber(row.quantity, 0) ?? 0;
}

function statCategory(row) {
  return asDisplayText(row.category, '미분류') || '미분류';
}

function statMenuName(row, byGroup) {
...
function sumQuantityIn(rows, year, month) {
  return rows.filter(r => isStatRow(r, year, month)).reduce((s, r) => s + statQuantity(r), 0);
}
...
function sumByCategoryIn(rows, year, month) {
  ...
  for (const r of rows) {
    ...
    const cat = statCategory(r);
    map.set(cat, (map.get(cat) || 0) + statQuantity(r));
  }
...
export async function getTopMenus(
  ...
  const rows = await safeAll('sales_rows');
  const { year, month } = anchor || pickLatestYearMonth(rows);
  const categoryLabel = asDisplayText(category);
  const scoped = categoryLabel ? rows.filter(r => statCategory(r) === categoryLabel) : rows;

  const map = new Map();
  for (const r of scoped) {
    if (!isStatRow(r, year, month)) continue;
    const key = statMenuName(r, byGroup);
    map.set(key, (map.get(key) || 0) + statQuantity(r));
  }

  // 0건은 워스트에 포함하지 않음 (실제 판매 기록 있는 메뉴만)
  const items = Array.from(map, ([name, quantity]) => ({ name, quantity })).filter(
    m => m.quantity > 0
  );
 ...
 */
export async function getTopMenusWithTrend(
  limit = 5,
  category = null,
  byGroup = false,
  order = 'desc',
  ...
  for (const r of scoped) {
    ...
    e[idx] += statQuantity(r);
  }

  const items = Array.from(map, ([name, spark]) => ({ name, quantity: spark[5], spark })).filter(
    m => m.quantity > 0
  );
