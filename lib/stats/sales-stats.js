...
import { safeAll, pickLatestYearMonth, shiftMonth, monthLabel, pickMenuName } from './_helpers';
...
export async function getSalesKpi() {
  ...
  const { year, month } = pickLatestYearMonth(rows);
...
export async function getSalesTrend(period = 'month') {
  ...
  if (period === 'year') {
    const { year } = pickLatestYearMonth(rows);
    const labels = [];
    const thisYear = [];
