...
import { safeAll, currentYearMonth, shiftMonth, buildSparkline } from './_helpers';
...
import { getAllNotesCached } from '@/lib/note/store';
...
function toNoteInputTime(note) {
  return toValidTime(note?.testDate) ?? toValidTime(note?.createdAt);
}

function isTimeInMonth(time, year, month) {
...
export async function getNoteKpi() {
  // menu_dev_notes는 브랜드 공유(main DB) store — safeAll(활성 브랜드 DB)이 아니라
  // getAllNotesCached(항상 main DB + 활성 브랜드 필터 + TTL 캐시)로 읽어야 한다.
  const notes = await getAllNotesCached();
  ...
  const sparkline = buildSparkline(
    year,
    month,
    (y, m) => notes.filter(n => isTimeInMonth(toNoteInputTime(n), y, m)).length
  );
...
export async function getNoteDetailStats() {
  ...
  const byStatus = {},
    byCategory = {};
  for (const n of notes) {
    const status = normalizeNoteStatus(n.status) || '미지정';
  ...
  for (const n of notes) {
    const noteTime = toNoteInputTime(n);
    if (noteTime == null) continue;
    const d = new Date(noteTime);
 ...
 *
 * 계산 흐름:
 * 1. menu_recipes + cost_ingredients + 가격 파일로 unitPriceMap 빌드
 * 2. cost_selling_prices 에서 menuCode 또는 (menuName, size) 판매가를 찾음
 * 3. canonical 레시피 구성품 원가를 최신 유효 단가 기준으로 계산
...
export async function getCostRateKpi() {
  ...
  const rate =
    Math.round((items.reduce((sum, item) => sum + item.costRate, 0) / items.length) * 10) / 10;
