...
export async function getPipelineStats() {
  ...
  const buckets = {
    ...
    testing:   { key: 'testing',   label: '테스트',   dotColor: 'var(--accent)',           status: ['테스트중', '재테스트'] },
    reporting: { key: 'reporting', label: '보고예정', dotColor: 'var(--color-reporting)',  status: ['보고예정'] },
  ...
  const columns = Object.values(buckets).map(col => {
    ...
    matched.sort((a, b) => (b.createdAt > a.createdAt ? 1 : -1));
    return {
      key: col.key,
      label: col.label,
  ...
  const done = released.filter(n => n.createdAt && new Date(n.createdAt).getTime() >= qStart).length;
