...
function costStatusOf(items, warnPct, critPct) {
  const safeItems = asObjectArray(items);
  const alertCount = safeItems.filter(
    item => (asFiniteNumber(item.costRate, 0) ?? 0) > critPct
  ).length;
  const cautionCount = safeItems.filter(item => {
    const costRate = asFiniteNumber(item.costRate, 0) ?? 0;
    return costRate > warnPct && costRate <= critPct;
  }).length;
...
function salesHealth({ freshness, openIssueCount, canEdit = true }) {
  ...
  return {
    ...
    status,
    href:
      openIssueCount > 0 ? '/menu-sales/unmatched' : getRoleSafeHref('/menu-sales/upload', canEdit),
    metric:
...
function costHealth({ costAlertData, ingredientHealth, warnPct, critPct }) {
  const items = asObjectArray(costAlertData?.items);
  const alertCount = items.filter(item => (asFiniteNumber(item.costRate, 0) ?? 0) > critPct).length;
  const cautionCount = items.filter(item => {
    const costRate = asFiniteNumber(item.costRate, 0) ?? 0;
    return costRate > warnPct && costRate <= critPct;
  }).length;
  const noPriceCount = asFiniteNumber(ingredientHealth?.noPriceCount, 0) ?? 0;
  const hasCostAlert = alertCount > 0;
  const hasNoPriceAlert = noPriceCount > 0;
  const status = hasCostAlert
    ? 'bad'
    : hasNoPriceAlert
      ? 'warn'
      : costStatusOf(items, warnPct, critPct);
  const href = hasCostAlert
    ? '/cost/margin'
    : hasNoPriceAlert
      ? '/ingredient/manage?catFilter=__no_price__'
      : '/ingredient/manage?view=price';
  const metric = hasCostAlert
    ? `경보 ${alertCount}건`
    : hasNoPriceAlert
      ? `단가 없음 ${noPriceCount}개`
      : `주의 ${cautionCount}건`;
  const detail = hasCostAlert
    ? `원가율 ${critPct}% 초과 메뉴를 확인하세요`
    : hasNoPriceAlert
      ? `단가 미입력 식자재 ${noPriceCount}개를 보정하세요`
      : '원가율과 식자재 단가 상태를 확인합니다';
  return { id: 'cost', label: '원가 · 식자재', status, href, metric, detail };
}
...
function noteHealth({ todos, pipeline }) {
  ...
  return {
    ...
    detail: todoCount > 0 ? '마감 임박 항목이 있습니다' : '오늘 처리할 긴급 항목이 없습니다',
  };
}
...
export function buildModuleHealth({
  ...
  issues = [],
  costAlertData = null,
  ingredientHealth = null,
  todos = [],
  pipeline = null,
  isMain = true,
  canEdit = true,
  // 홈 원가율 경보 위젯·원가마진표·원가계산보고서와 같은 기준(v3:margin-cost-warn/-crit) —
  // 기본값(30/40)은 그 훅들의 기본값과 동일.
  warnPct = 30,
  critPct = 40,
} = {}) {
  ...
  const modules = [
    salesHealth({ freshness, openIssueCount, canEdit }),
    isMain ? jetteHealth({ freshness }) : null,
    costHealth({ costAlertData, ingredientHealth, warnPct, critPct }),
