...
function costHealth({ costAlertData, ingredientHealth }) {
  ...
  const status = hasCostAlert ? 'bad' : hasNoPriceAlert ? 'warn' : costStatusOf(items);
...
export function buildModuleHealth({
  ...
  const modules = [
    ...
    costHealth({ costAlertData, ingredientHealth }),
