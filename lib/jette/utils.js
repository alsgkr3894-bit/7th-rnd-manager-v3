 ...
 * @returns {{ exclusive: number, generic: number }}
 */
export function getProductTypeCounts(rows, productTypeLookup) {
  ...
  const get = type =>
    safeRows.filter(r => {
      const product = lookup.get(asDisplayText(r.productCode));
      return product && normalizeProductType(product.productType) === type;
    }).length;
  return {
