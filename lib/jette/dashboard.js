...
import { normalizeProductType } from './product-types.js';
...
async function getShipmentSummary() {
  ...
  const sameMonth = files.filter(f => {
    ...
    return (
      period?.year === latestPeriod.year && period?.month === latestPeriod.month && f.id != null
    );
  });
  ...
  const typeCounts = aggRows.reduce(
    (acc, r) => {
      const productType = normalizeProductType(asDisplayText(r.productType));
      if (productType in acc) acc[productType]++;
      return acc;
    },
    { exclusive: 0, generic: 0 }
  );
