...
function normalizeBrand(value, fallback = {}) {
  ...
  const base = {
    ...
    parent: cleanText(value?.parent, fallback.parent || ''),
...
function normalizeBrands(input) {
  ...
  if (!defaultBrand) {
    defaultBrand =
      visibleDefaults.find(brand => brand.id === DEFAULT_BRAND_ID) ||
      visibleDefaults[0] ||
      brands[0];
  }
...
function emitBrandMasterChanged() {
  if (typeof window === 'undefined' || typeof window.dispatchEvent !== 'function') return;
  try {
    const event =
      typeof CustomEvent === 'function'
        ? new CustomEvent(BRAND_MASTER_EVENT)
        : { type: BRAND_MASTER_EVENT };
    window.dispatchEvent(event);
  } catch {
    // 저장은 이미 성공했다. 이벤트 전파 실패가 브랜드 설정 저장을 되돌리면 더 위험하다.
  }
}
...
function writeBrands(brands) {
  ...
  try {
    ...
    throw new Error(
      `브랜드 설정을 저장하지 못했습니다. 브라우저 저장소 상태를 확인해 주세요.${detail}`
    );
  }
  emitBrandMasterChanged();
  return normalized;
}
...
/**
 * 백업의 브랜드 마스터를 현재 레지스트리에 병합한다(덮어쓰기 아님).
 * 복원 시 백업 이후 새로 만든 브랜드가 사라지는 문제를 막기 위해 id 기준 합집합을 만들고,
 * 같은 id가 양쪽에 있으면 updatedAt이 더 최신인 쪽을 유지한다.
 * @param {string} rawValue - 백업 localStorage의 BRAND_MASTER_KEY 문자열(JSON)
 */
export function mergeBrandMasterFromBackup(rawValue) {
  let incoming = null;
  try {
    const parsed = JSON.parse(rawValue || 'null');
    incoming = Array.isArray(parsed?.brands) ? parsed.brands : null;
  } catch {
    incoming = null;
  }
  const current = getBrands();
  ...
  return writeBrands(Array.from(byId.values()));
}
...
// 권한 가드 정책: 아래 upsertBrand/setBrandHidden/setDefaultBrandId는 sync·localStorage
// 메타 편집(비파괴)이라 실행함수 레이어 assertActiveAdmin 가드를 두지 않는다.
// UI(useCurrentRole)에서만 viewer를 차단한다. 자세한 사유는 docs/SECURITY_POLICY.md §2-4-1.
...
export function setBrandHidden(id, hidden) {
  ...
  if (target.id === DEFAULT_BRAND_ID && hidden)
    throw new Error('기본 7번가피자는 숨길 수 없습니다.');
  if (target.isDefault && hidden) throw new Error('기본 브랜드는 숨길 수 없습니다.');
  const now = nowIso();
  return writeBrands(
    brands.map(brand =>
      brand.id === brandId ? { ...brand, hidden: Boolean(hidden), updatedAt: now } : brand
    )
  );
}
