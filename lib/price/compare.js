...
import { dedupePriceRowsByProductCode } from './duplicates';
...
export function comparePriceLists(baseRows, latestRows) {
  const safeBaseRows = dedupePriceRowsByProductCode(asObjectArray(baseRows)).rows;
  const safeLatestRows = dedupePriceRowsByProductCode(asObjectArray(latestRows)).rows;
...
/**
 * comparePriceLists() 결과를 상태별 건수 + 실제 변동 목록(변동없음 제외)으로 요약한다.
 * @param {object[]} diffRows - comparePriceLists() 반환값
 * @returns {{ up: number, down: number, added: number, removed: number, unchanged: number,
 *   total: number, changed: object[] }}
 */
export function summarizePriceChanges(diffRows) {
  const rows = asObjectArray(diffRows);
  const counts = { up: 0, down: 0, added: 0, removed: 0, unchanged: 0 };
  const changed = [];
  ...
  // comparePriceLists가 이미 상태 우선·변동률 내림차순으로 정렬해뒀으므로 그 순서를 그대로 보존한다.
  return { ...counts, total: rows.length, changed };
}
