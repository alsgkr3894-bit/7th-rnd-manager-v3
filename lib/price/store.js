...
export async function getPriceFiles() {
  ...
  return files.sort((a, b) => asDisplayText(b.updateDate).localeCompare(asDisplayText(a.updateDate)));
}
...
export async function getPriceRowsByFileId(fileId) {
  ...
  return asObjectArray(await getByIndex('price_rows', 'fileId', fileId));
}
