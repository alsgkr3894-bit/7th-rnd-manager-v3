...
export function useJettePrice() {
  ...
  async function autoRegisterNewProducts(rows) {
    ...
    for (const candidate of candidates) {
      try {
        ...
        failed++;
        console.error('[jette-price] 신규 제품 자동등록 실패:', err);
      }
    }
  ...
  async function handleFile(file, customDate) {
    if (!file) return;
    if (file.size > UPLOAD_MAX_MB.jette * 1024 * 1024) {
      showToast(`파일이 너무 큽니다 (최대 ${UPLOAD_MAX_MB.jette}MB)`, 'error');
      return;
    }
    setBusy(true);
    try {
      ...
      const autoRegisterText = autoRegisterResult.added
        ? ` · 신규 ${autoRegisterResult.added}개 자동등록`
