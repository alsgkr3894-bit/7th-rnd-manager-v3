...
import { logPriceUpload } from '@/lib/change-log';
import {
  parsePriceRows,
  savePriceUpload,
  getPriceFiles,
...
/**
 * 방금 업로드된 단가파일을 그 직전 파일과 비교해 인상/인하/신규/삭제 건수와
 * 원가 변동 메뉴 수를 자동 일지 한 줄로 남긴다. 파일이 1개뿐이면(비교 불가) 생략.
 */
async function logPriceChangeToWorkLog() {
  const files = await getPriceFiles();
  if (files.length < 2) return;
  const [latestFile, prevFile] = files;
  const [prevRows, latestRows] = await Promise.all([
    getPriceRowsByFileId(prevFile.id),
    getPriceRowsByFileId(latestFile.id),
  ]);
  const summary = summarizePriceChanges(comparePriceLists(prevRows, latestRows));
  const { getMenuCostChanges } = await import('@/lib/stats/menu-cost-change-stats');
  const costChanges = await getMenuCostChanges().catch(() => null);
  const costText = costChanges?.total ? ` / 원가 변동 메뉴 ${costChanges.total}개` : '';
  const { logWork } = await import('@/lib/work-log');
  await logWork(
    'PRICE_CHANGE',
    `단가 변동: 인상 ${summary.up}·인하 ${summary.down}·신규 ${summary.added}·삭제 ${summary.removed}${costText}`
  );
}
...
export function useJettePrice() {
  ...
  async function handleFile(file, customDate) {
    ...
    try {
      ...
      logPriceUpload(file.name, rowsToSave.length);
      import('@/lib/work-log')
        .then(m =>
          m.logWork(
            'UPLOAD',
        ...
        .catch(() => {});
      await refreshFiles(true);
      // 단가 변동·원가 변동 요약을 자동 일지에 남긴다 — 파일이 1개뿐(비교 불가)이면 생략,
      // 실패해도 업로드 자체의 성공 토스트에는 영향 없도록 전체를 감싼다.
      logPriceChangeToWorkLog().catch(() => {});
    } catch (err) {
