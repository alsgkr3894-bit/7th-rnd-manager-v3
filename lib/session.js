...
export function ensureSession() {
  ...
  const isNewSession = !getSessionItem(SESSION_FLAG);
  ...
  if (isNewSession) {
    ...
    setSessionItem(SESSION_FLAG, '1');
    setLocalItem(KEY_LAST_LOGIN, now);
  }
...
export function getLastLogin() {
  return getLocalItem(KEY_LAST_LOGIN);
}

/** 캐시된 마지막 IP (있으면 즉시 반환, 없으면 null) */
export function getCachedIP() {
  try {
    const raw = getLocalItem(KEY_LAST_IP);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== 'object' || typeof parsed.ip !== 'string') return null;
...
export async function fetchClientIP() {
  ...
  try {
    ...
    if (!ip) throw new Error('ipify: ip 없음');

    const entry = { ip, at: new Date().toISOString() };
    setLocalItem(KEY_LAST_IP, JSON.stringify(entry));
    return entry;
  } catch (err) {
    console.warn('[session] IP 조회 실패:', err?.message || String(err));
    return null;
  } finally {
    clearTimeout(timer);
  }
...
export async function fetchAppJson(path, options = {}) {
  ...
  if (!response.ok || payload?.ok === false) {
    ...
    // 호출부가 "영구 거절(4xx)"과 "일시 장애(5xx/네트워크)"를 구분할 수 있게 상태코드를 싣는다.
