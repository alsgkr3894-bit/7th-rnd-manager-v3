...
import { isAuthoritativeClient } from '@/lib/db/sync-mode';
import {
  ACCOUNT_ROLES,
  ACTIVE_ACCOUNT_KEY,
...
export async function getActiveRole() {
  // LAN 뷰어 브라우저는 계정 설정과 무관하게 항상 viewer다.
  // 계정이 0개면 admin으로 떨어지는 아래 기본값 때문에, 이 가드가 없으면
  // 새 PC로 접속한 사람이 자동으로 관리자가 되어 서버 데이터를 덮어쓸 수 있다.
  if (!isAuthoritativeClient()) return 'viewer';
  try {
