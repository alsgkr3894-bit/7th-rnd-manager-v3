...
export function withDownloadDateSuffix(fileName, date = new Date()) {
  ...
  const extension = hasExtension ? `.${sanitizeExtension(raw.slice(dotIndex + 1))}` : '';
  const baseName = sanitizeFileNamePart(
    rawBaseName
      .replace(/(?:[_\s-])\d{8}(?:_\d{6})?$/, '')
      .replace(/(?:[_\s-])\d{4}-\d{2}-\d{2}(?:[_\s-]\d{6})?$/, '')
  );
  if (baseName.endsWith(`_${stamp}`)) return `${baseName}${extension}`;
  return `${baseName}_${stamp}${extension}`;
}

/** 날짜 포함 파일명 생성: "한글업무명_20260611.ext" */
export function makeFileName(prefix, ext, date = new Date()) {
  const safePrefix = sanitizeFileNamePart(prefix || '내보내기', '내보내기');
  const safeExt = sanitizeExtension(ext || 'txt');
  return `${safePrefix}_${downloadDateStamp(date)}.${safeExt}`;
}

/** 브랜드명 포함 파일명 생성: "브랜드명_업무명_20260611.ext" */
export function makeFileNameWithBrand(prefix, ext, date = new Date()) {
  const brand = getActiveBrand();
  const brandName = brand?.name ? String(brand.name).trim() : '';
  const safePrefix = String(prefix || '내보내기').trim() || '내보내기';
  const fullPrefix = brandName ? `${brandName}_${safePrefix}` : safePrefix;
  return makeFileName(fullPrefix, ext, date);
}
...
export function readFileAsText(file, allowedExt) {
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      resolve(typeof text === 'string' && text.charCodeAt(0) === 0xfeff ? text.slice(1) : text);
    };
...
export function rowsToCsv(rows) {
  const esc = v => {
    if (v == null) return '';
    const s = String(v);
    // CSV 수식 인젝션 방지: 문자열 셀이 수식 트리거 문자(= + - @ tab CR)로 시작하면
    // 작은따옴표를 접두해 텍스트로 강제. 단, 전체가 유효 숫자(-5, +3.5)면 그대로 둬 숫자 셀 보존.
    // (+1+1 처럼 숫자로 시작하지만 수식인 경우는 Number()가 NaN이므로 살균된다)
    const isFormulaRisk =
      typeof v === 'string' && /^[=+\-@\t\r]/.test(s) && !Number.isFinite(Number(s));
    const safe = isFormulaRisk ? `'${s}` : s;
...
export function downloadCsv(rows, fileName) {
  ...
  triggerDownload(blob, fileName || makeFileName('엑셀내보내기', 'csv'));
}

/** 이미 만들어진 CSV 문자열 → 파일 다운로드 (BOM 없으면 prepend) */
...
/**
 * 업로드 실패 행 배열 → CSV 다운로드
 * rows 요소의 key를 헤더로, 값을 셀로 변환한다.
 * @param {object[]} rows
 * @param {string} [fileName]
 */
export function downloadFailedRows(rows, fileName) {
  if (!rows || rows.length === 0) return;
  const safeRows = rows.filter(row => row && typeof row === 'object');
  const headers = [...new Set(safeRows.flatMap(row => Object.keys(row)))];
  if (headers.length === 0) return;
  const csvRows = [headers, ...safeRows.map(r => headers.map(h => r[h] ?? ''))];
  downloadCsv(csvRows, fileName || makeFileName('오류행', 'csv'));
}
