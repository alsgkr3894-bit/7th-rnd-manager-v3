import { getAll, getById, getByIndex, deleteById, runTransaction, hasStore } from '@/lib/db';
...
export async function deleteNote(id) {
  if (!hasStore(STORE)) throw new Error(`${STORE} store를 찾을 수 없습니다`);
  // Delete parent and fetch children efficiently via parentId index (added in DB v11)
  await runTransaction([STORE], 'readwrite', tx => { storeOf(tx).delete(id); });
  const children = await getByIndex(STORE, 'parentId', id);
  for (const c of children) await deleteById(STORE, c.id);
