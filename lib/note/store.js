...
import { clampNoteRating, normalizeNoteMenuCode } from '@/lib/note/evaluation';
...
export async function getNoteById(id) {
  ...
  return normalized && belongsToActiveBrand(normalized) ? normalized : null;
}
...
export async function addNote(data) {
  ...
  if (!data.title?.trim() && !data.menuName?.trim() && !data.menuCode?.trim()) {
...
export async function updateNoteChainStatus(id, status, targetPatch = {}) {
  await assertActiveAdmin('노트 상태 변경');
  await initSharedDB();
  if (!hasStore(STORE)) throw new Error(`${STORE} store를 찾을 수 없습니다`);

  const safeStatus = normalizeNoteStatus(status);
  ...
  const root = current || byId.get(id);
  const chainMap = new Map();
  [root, ...collectDescendantNotes(all, root.id)].filter(Boolean).forEach(note => {
    chainMap.set(note.id, note);
  });
  const chain = [...chainMap.values()];
  const chainIds = chain.map(note => note.id);
  const now = new Date().toISOString();

  await runTransaction([STORE], 'readwrite', tx => {
    const os = storeOf(tx);
    for (const note of chain) {
      const patch = note.id === id ? targetPatch : {};
      os.put({ ...note, ...patch, status: safeStatus, updatedAt: now });
    }
  });

  invalidateNotesCache();
  logWork('NOTE_STATUS', `노트 상태 변경: ${safeStatus}`, {
    ref: id,
    count: chainIds.length,
  }).catch(err => {
    console.warn('[updateNoteChainStatus] work log failed', err);
  });
  return chainIds;
}

/**
 * 노트 유형(메뉴개발/메뉴개선)을 같은 메뉴 차수 체인 전체에 적용.
 * updateNoteChainStatus와 동일한 체인 탐색 로직을 공유한다.
 */
export async function updateNoteChainType(id, noteType) {
  await assertActiveAdmin('노트 유형 변경');
  await initSharedDB();
  if (!hasStore(STORE)) throw new Error(`${STORE} store를 찾을 수 없습니다`);

  const safeType = MENU_DEVELOPMENT_NOTE_TYPES.includes(noteType)
    ? noteType
  ...
  await runTransaction([STORE], 'readwrite', tx => {
    const os = storeOf(tx);
    for (const note of chain) {
      os.put({ ...note, noteType: safeType, updatedAt: now });
    }
  });

  invalidateNotesCache();
  logWork('NOTE_UPDATE', `노트 유형 변경: ${safeType}`, {
    ref: id,
    count: chainIds.length,
  }).catch(err => {
    console.warn('[updateNoteChainType] work log failed', err);
  });
  return chainIds;
}
...
export async function getNotesInChain(noteId) {
  await initSharedDB();
  if (!hasStore(STORE)) return [];
  const all = asObjectArray(await getAll(STORE))
    .map(normalizeNoteRecord)
    .filter(belongsToActiveBrand);
...
function buildRecord(data) {
  return {
    ...
    menuCode: normalizeNoteMenuCode(data.menuCode),
    menuName: (data.menuName || '').trim(),
    category: (data.category || '').trim(),
    noteType: normalizeNoteType(data.noteType),
    status: normalizeNoteStatus(data.status),
    testContent: (data.testContent || '').trim(),
    testDate: (data.testDate || '').trim(),
    testRound: data.testRound == null ? '' : String(data.testRound).trim(),
    materials: (data.materials || '').trim(),
    tasteEval: (data.tasteEval || '').trim(),
    tasteRating: clampNoteRating(data.tasteRating),
    textureRating: clampNoteRating(data.textureRating),
    appearanceRating: clampNoteRating(data.appearanceRating),
