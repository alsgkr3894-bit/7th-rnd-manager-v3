...
/** 배지 전용 — 전체 노트 없이 status 인덱스로만 출시예정/레거시 보고예정 건수를 셈. */
export async function getReportingNoteCount() {
  await initSharedDB();
  if (!hasStore(STORE)) return 0;
  const [releaseReadyRows, legacyReportingRows] = await Promise.all([
    getByIndex(STORE, 'status', '출시예정'),
    getByIndex(STORE, 'status', '보고예정'),
  ]);
  return [...asObjectArray(releaseReadyRows), ...asObjectArray(legacyReportingRows)].filter(
    belongsToActiveBrand
  ).length;
}
