...
function noteCard(note, index, effectiveStatus) {
  ...
  const noteSections = groups.length
    ? groups
        .map(([category, rows]) => {
          const cards = rows
            .map(note => {
              const html = noteCard(note, runningIndex, effectiveStatusById.get(note?.id));
              runningIndex += 1;
              return html;
            })
            .join('');
...
<style>
  * { box-sizing: border-box; }
  ...
  img { width: 100%; max-height: 240px; object-fit: contain; display: block; border: 1px solid #e5e7eb; border-radius: 4px; }
...
<body>
  <main class="page">
    ...
    <section class="summary-grid">
      ...
      <div class="summary-card"><span>제목 수</span><strong>${summary.menuCount.toLocaleString('ko-KR')}</strong></div>
