...
export function sampleToUnifiedRecord(sample = {}) {
  ...
  const body = textList([sample.description, sample.result]);
  ...
  return {
    ...
    testContent: body || cleanText(sample.nextAction),
    tasteEval: cleanText(sample.result),
    managerEval: textList([sample.company, sample.tester]).replace(/\n/g, ' / '),
    materials: sampleNames || groupName,
    issues: recordType === SAMPLE_RECORD_TYPES.ISSUE ? body : '',
