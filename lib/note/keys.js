export const KEYS = {
  ...
  NOTE_TYPE_FILTER: 'v3:note-type-filter',
  ...
  SIDEBAR_COLLAPSED: 'v3:sidebar-collapsed',
  PALETTE_RECENT: 'v3:palette-recent',
  PALETTE_FAVORITES: 'v3:palette-favorites',
  PROFILE: 'v3:profile',
...
export function setNoteFrom(id) {
  _ssSet(KEYS.NOTE_FROM, String(id));
}
...
/** 식자재 이슈 및 테스트 /샘플기록 작성 화면으로 이동 시 원본 노트 데이터 전달 */
export function setSampleFromNote(data) {
  try {
    _ssSet(KEYS.SAMPLE_FROM_NOTE, JSON.stringify(data));
  } catch {}
}
...
/** 식자재 이슈 및 테스트 /샘플기록 작성 화면으로 이동 시 원본 샘플 ID 전달 */
export function setSampleFrom(id) {
  _ssSet(KEYS.SAMPLE_FROM, String(id));
}
export function consumeSampleFrom() {
  const v = _ssGet(KEYS.SAMPLE_FROM);
  _ssRemove(KEYS.SAMPLE_FROM);
  return v;
}
