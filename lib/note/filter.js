...
function buildNoteStatusGroups(notes) {
  ...
  list.forEach((note, index) => {
    ...
    const key = isChained ? `chain:${rootId || parentId || id || index}` : `note:${id || index}`;
...
export function buildNoteSearchIndex(notes) {
  ...
  for (const n of asObjectArray(notes)) {
    ...
    [
      searchText(n.noteType),
      searchText(n.recordType),
      searchText(n.ingredientGroupName),
      searchText(n.sampleNames),
      searchText(n.company),
      searchText(n.tester),
      searchText(n.description),
      searchText(n.result),
    ].forEach(value => {
...
export function countNotesByType(notes, types = NOTE_UNIFIED_TYPES) {
  const counts = { [NOTE_UNIFIED_TYPE_ALL]: 0 };
  for (const type of types) counts[type] = 0;

  for (const note of asObjectArray(notes)) {
    const type = normalizeUnifiedNoteType(note?.noteType);
    counts[NOTE_UNIFIED_TYPE_ALL] += 1;
...
export function filterSortNotes(
  ...
  const safeTypeFilter = normalizeUnifiedTypeFilter(typeFilter);
  ...
  if (safeTypeFilter !== NOTE_UNIFIED_TYPE_ALL) {
    list = list.filter(n => normalizeUnifiedNoteType(n?.noteType) === safeTypeFilter);
  }
