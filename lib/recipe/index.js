import { resolveCompositePrice } from '@/lib/cost/composite-price';
import { calcUnitPrice } from '@/lib/cost/calc-unit-price';
import { normalizeCostBaseUnit } from '@/lib/cost/unit-policy';

export const MENU_CATEGORIES = [
  '피자',
  '피자/프리미엄 스페셜',
  '피자/프리미엄',
  '피자/오리지널',
  '1인피자',
  '사이드',
  '세트박스',
  '피자/하프앤하프',
  '소스',
  '음료',
  '엣지',
  '기타',
];

/**
 * 식자재 마스터 + 최신 가격 파일로 단가 맵 생성.
 *
 * compositeOf 배열이 있는 품목은 구성 코드별 세금포함가를 합산하며,
 * 일부 코드 가격이 누락된 경우 priceOverride 를 폴백으로 사용합니다.
 *
 * @param {Array<{
 *   productCode: string,
 *   ingredientName?: string,
 *   baseQuantity?: number,
 *   baseUnitType?: string,
 *   compositeOf?: string[],
 *   priceOverride?: number|null
 * }>} allMeta - 식자재 마스터 배열
 * @param {Map<string, { priceWithTax: number }>} priceRowMap
 *   - productCode → 최신 가격행 (가격 파일에서 파싱)
 * @returns {Map<string, {
 *   ingredientName: string,
 *   priceWithTax: number|null,
 *   baseQuantity: number|undefined,
 *   baseUnitType: string,
 *   unitPrice: number|null
 * }>} productCode → 단가 정보 맵
 */
export function buildUnitPriceMap(allMeta, priceRowMap) {
  const map = new Map();
  for (const m of allMeta) {
    const key = m.productCode || (m.id != null ? String(m.id) : null);
    if (!key) continue;
    const baseQty = m.baseQuantity;

    let priceWithTax = null;
    if (m.productCode) {
      if (Array.isArray(m.compositeOf) && m.compositeOf.length > 0) {
        const resolved = resolveCompositePrice(m.compositeOf, priceRowMap, { mode: 'strict' });
        priceWithTax = resolved.priceWithTax ?? m.priceOverride ?? null;
      } else {
        const pr = priceRowMap.get(m.productCode);
        priceWithTax = pr?.priceWithTax ?? m.priceOverride ?? null;
      }
    } else {
      priceWithTax = m.priceOverride ?? null;
    }

    const unitPrice = calcUnitPrice(priceWithTax, baseQty);

    map.set(key, {
      ingredientName: m.ingredientName || '',
      priceWithTax,
      baseQuantity: baseQty,
      baseUnitType: normalizeCostBaseUnit(m.baseUnitType),
      unitPrice,
    });
  }
  return map;
}

/**
 * 특정 사이즈의 레시피 원가 계산
 * @param {string} sizeLabel - 'L' | 'R' | ... (없으면 첫 번째 사이즈 사용)
 */
export function calcRecipeCost(recipe, unitPriceMap, sizeLabel) {
  if (!recipe?.ingredients?.length) return 0;
  const label = sizeLabel ?? recipe.sizes?.[0]?.label ?? null;
  return recipe.ingredients.reduce((acc, line) => {
    const info = unitPriceMap.get(line.productCode);
    if (!info?.unitPrice) return acc;
    const qty = label != null ? (line.quantities?.[label] ?? 0) : 0;
    if (!qty) return acc;
    return acc + info.unitPrice * qty;
  }, 0);
}

/** 사이즈별 원가 맵 반환: { L: 1200, R: 900 } */
export function calcCostBySizes(recipe, unitPriceMap) {
  const result = {};
  for (const s of recipe?.sizes || []) {
    result[s.label] = calcRecipeCost(recipe, unitPriceMap, s.label);
  }
  return result;
}

/**
 * 원가율 계산 (0 ~ 100 %).
 *
 * sellingPrice 가 0 이하이거나 없으면 null 을 반환합니다.
 * 반환값은 마진율이 아닌 원가율(cost rate)입니다.
 *
 * @param {number} cost         - 원가 (원)
 * @param {number} sellingPrice - 판매가 (원, VAT 포함 여부는 호출측이 결정)
 * @returns {number|null} 원가율(%) — sellingPrice 가 유효하지 않으면 null
 */
export function calcMarginRate(cost, sellingPrice) {
  if (!sellingPrice || sellingPrice <= 0) return null;
  return (cost / sellingPrice) * 100;
}
