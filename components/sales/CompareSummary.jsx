...
export function CompareSummary({ compare }) {
  ...
  const safeTotalPct = totalPct == null
    ? null
    : Number.isFinite(Number(totalPct))
      ? Number(totalPct)
      : null;
    ...
    <div className="hero-row" style={{marginTop:16}}>
      ...
      <div className="card kpi-card">
        <div>
          ...
          <div className="value num">{formatNumber(safeTotalA)}<span className="unit">개</span></div>
          <div className="trend">
            {safeTotalPct == null ? (
              ...
              <span className={safeTotalPct >= 0 ? 'up' : 'down'}>
                ...
                {' '}{safeTotalPct >= 0 ? '+' : ''}{safeTotalPct.toFixed(1)}%
                <span style={{color:'var(--text-4)', marginLeft:6}}>
                  ({safeTotalDiff >= 0 ? '+' : ''}{formatNumber(safeTotalDiff)}개)
                </span>
              </span>
            )}
          </div>
        ...
        <div style={{fontSize:12, color:'var(--text-3)', marginTop:8}}>
          {formatNumber(safeTotalB)}개 → <b style={{color:'var(--text-1)'}}>{formatNumber(safeTotalA)}개</b>
        </div>
      </div>
  ...
  return (
    <div className="card kpi-card">
      <div>
        ...
        <div className="value num" style={{color: safeColor}}>{formatNumber(safeCount)}<span className="unit">개</span></div>
        <div className="trend">
      ...
      <div style={{marginTop:12, display:'flex', flexDirection:'column', gap:6}}>
        {safeItems.length === 0 ? (
          ...
          const value = Number.isFinite(Number(useFieldB ? m.b : m.a)) ? Number(useFieldB ? m.b : m.a) : 0;
