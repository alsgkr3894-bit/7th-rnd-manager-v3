                    ...
                    return editingId === excludedId && hasExcludedId ? (
                      <tr key={key}>
                        <td colSpan={2} style={{padding:8}}>
                          <RowForm form={form} setForm={setForm} onCancel={() => setEditingId(null)} onSubmit={() => handleUpdate(excludedId)} busy={busy} submitLabel="저장"/>
                        </td>
                      </tr>
                    ) : (
                      <tr key={key}>
                        <td className="cell-name"><div className="menu-name">{menuName}</div></td>
                        <td style={{textAlign:'right'}}>
                          {pendingDeleteId === excludedId && hasExcludedId ? (
                            ...
                            <>
                              ...
                              {' '}
                              <button className="btn sm" style={{color:'var(--negative)'}} onClick={() => requestDelete(excludedId)}>삭제</button>
                            </>
                          ) : null}
                        </td>
                      </tr>
...
function RowForm({ form, setForm, onCancel, onSubmit, busy, submitLabel = '추가' }) {
  ...
  return (
    <div style={{display:'grid', gridTemplateColumns:'1fr auto auto', gap:8, marginBottom:8}}>
      <input value={menuName} onChange={e => updateForm({ ...safeForm, menuName: e.target.value })} placeholder="제외할 메뉴명 (정규화 후)" style={inputStyle}/>
