    ...
    <div className="card" style={{display:'flex', alignItems:'center', gap:14, padding:'12px 18px'}}>
      <div style={{flex:1, minWidth:0}}>
        <div style={{display:'flex', alignItems:'center', gap:8, flexWrap:'wrap'}}>
          ...
          {row.productCode && <span style={{fontSize:11, color:'var(--text-3)'}}>{row.productCode}</span>}
        </div>
        {row.priceDiff && (
          <div style={{fontSize:12, color:'var(--text-2)', marginTop:3}}>{fmtPriceDiff(row.priceDiff)}</div>
        )}
      ...
      <button className="btn sm" onClick={() => handleEdit(row)}><Icon.edit style={{width:13,height:13}}/> 수정</button>
    </div>
  );
}
...
export function IssuesView({ issueRows, onEdit }) {
  ...
  const filtered = filter === 'all' ? safeIssueRows : safeIssueRows.filter(r => hasIssue(r, filter));
