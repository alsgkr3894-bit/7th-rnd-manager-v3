...
export const ManageRow = memo(function ManageRow({ r: rawRow = {}, deletePending, onEdit, onCopy, onDeleteStart, onDeleteCancel, onDeleteConfirm, onRestore, batchMode, isSelected, onToggleSelect }) {
  ...
  const unitLabel = Number.isFinite(baseQuantity) && baseQuantity > 0 && baseUnitType
  ...
  return (
    <tr
      ...
      onClick={batchMode ? (deletable && toggleSelect ? () => toggleSelect(r.id) : undefined) : handleEdit}
    >
      {batchMode && (
      ...
      <td style={{ fontWeight: 600, fontSize: 13 }}>
        <span title={productName && productName !== name ? `원본: ${productName}` : undefined}>{name}</span>
        {r.discontinued && (
        ...
        {allergenCount > 0 && (
          <span style={{ marginLeft: 4, fontSize: 9, fontWeight: 700, padding: '1px 5px', borderRadius: 3, background: 'var(--warn-soft)', color: 'var(--warn)' }}>알레르기 {allergenCount}</span>
        )}
      </td>
      ...
      <td className="num right" style={{ fontWeight: 600, fontSize: 12 }}>
        {priceWithTax != null ? <>{formatNumber(priceWithTax)}<span className="unit">원</span></> : '-'}
      </td>
      <td>
        {category
          ? <span className="chip" style={{ ...getCategoryStyle(category), padding: '2px 8px', fontSize: 11 }}>{category}</span>
      ...
      <td style={{ textAlign: 'center' }} onClick={e => e.stopPropagation()}>
        {r.excluded ? (
          <button className="btn sm" style={{ fontSize: 11 }} onClick={handleRestore}>복원</button>
        ) : deletePending ? (
          <span style={{ display: 'flex', gap: 3 }}>
            ...
            <button className="btn sm" style={{ fontSize: 11 }} onClick={handleDeleteCancel}>취소</button>
          </span>
        ) : (
          <span style={{ display: 'inline-flex', gap: 3 }}>
            {handleCopy && (
              <button className="btn sm" aria-label="복사해서 추가" title="이 항목을 복사해 새 식자재 추가" onClick={handleCopy} style={{ color: 'var(--text-3)' }}>
                <Icon.copy style={{ width: 13, height: 13 }} />
              </button>
            )}
            <button className="btn sm" aria-label="삭제" onClick={handleDeleteStart} style={{ color: 'var(--text-3)' }}>
