...
export function ShipmentSummary({ aggRows, managedCount }) {
  ...
  const baseLabel = isAll
    ...
    : selectedTypes.size === 0
      ? '카테고리 미선택'
      : TYPE_OPTIONS.filter(o => selectedTypes.has(o.value))
          .map(o => o.label)
          .join(' + ');
  ...
  return (
    <>
      ...
      <div
        style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 16, alignItems: 'center' }}
      >
  ...
  return (
    <div className="card" style={{ padding: '16px 20px' }}>
      ...
      >
        {safeValue}
        {safeUnit && (
          <span style={{ fontSize: 13, fontWeight: 600, opacity: 0.5, marginLeft: 3 }}>
            {safeUnit}
          </span>
        )}
      </div>
