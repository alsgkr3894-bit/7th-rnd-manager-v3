...
export function CommonManageView({ tab = 'groups' }) {
  ...
  async function handleDeleteGroup(id) {
    if (!id) return;
    if (!confirm('이 묶음 및 연결된 레시피 참조가 삭제됩니다. 되돌릴 수 없습니다. 계속할까요?'))
      return;
    try {
            ...
            <div className="card" style={{ minHeight: 200, display: 'grid', placeItems: 'center' }}>
              <div style={{ textAlign: 'center', color: 'var(--text-3)' }}>
                ...
                <div style={{ fontSize: 13 }}>
                  {isMain ? (
                    <>
                      <b>마스터 시드</b>로 5종 (치즈크러스트 L/R · 골드스윗크러스트 L/R · 씬도우 L)
                      일괄 등록
                    </>
                  ) : (
                    <>
          ...
          {deletePending && (
            ...
            >
              ...
              <button
                className="btn"
                style={{ background: 'var(--negative)', color: '#fff', border: 'none' }}
                onClick={() => handleDeleteEdge(deletePending)}
              >
                삭제
              </button>
              <button className="btn" onClick={() => setDeletePending(null)}>
                취소
              </button>
            </div>
          )}
