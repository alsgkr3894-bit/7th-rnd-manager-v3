...
export function CostDetailView({
  ...
  const costByCode = useMemo(() => new Map(summaryRows.map(r => [r.menuCode, r])), [summaryRows]);
          ...
          {!loading && menus.length === 0 && (
            <div className="card" style={{ minHeight: 200, display: 'grid', placeItems: 'center' }}>
              <div style={{ textAlign: 'center', color: 'var(--text-3)' }}>
                <Icon.doc style={{ width: 32, height: 32, marginBottom: 12, opacity: 0.4 }} />
            ...
            >
              <span>
                ...
                {filteredRows.length < menus.length && (
                  <span style={{ color: 'var(--text-4)', marginLeft: 4 }}>(필터 적용 중)</span>
                )}
              </span>
              <span>
                {footerLabel} {formatNumber(filteredStats.totalCost)}원
              </span>
            </div>
          )}
        </>
