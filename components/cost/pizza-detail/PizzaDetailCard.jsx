...
export function PizzaDetailCard({ menu, recipe, onEdit }) {
  ...
  const costRate = (menu.price && baseCost > 0)
    ? (baseCost / menu.price * 100)
    : null;
    ...
    <div className="card" style={{padding:'14px 18px', display:'flex', alignItems:'center', gap:14}}>
      <div style={{flex:1, minWidth:0}}>
        <div style={{display:'flex', alignItems:'center', gap:8, flexWrap:'wrap'}}>
          <span style={{fontWeight:700, fontSize:15}}>{menu.menuName} {menu.size}</span>
          <span style={{
      ...
      <div style={{textAlign:'right', minWidth:140}}>
        ...
        <div style={{fontSize:17, fontWeight:800, color: baseCost > 0 ? 'var(--text-1)' : 'var(--text-4)', lineHeight:1.2}}>
          {baseCost > 0 ? <>{formatNumber(baseCost)}<span style={{fontSize:12, fontWeight:600, marginLeft:2}}>원</span></> : '—'}
        </div>
        {costRate != null && (
