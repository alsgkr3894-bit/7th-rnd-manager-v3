        ...
        {filtered.map((ing, i) => (
          <label key={ing.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 24px', cursor: 'pointer', background: selected.has(ing.id) ? 'var(--accent-soft)' : 'transparent', animation: 'row-in 200ms ease both', animationDelay: `${i * 30}ms` }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-3)' }}>{ing.productCode} {ing.baseUnitType && `· ${ing.baseUnitType}`}</div>
            </div>
          </label>
        ))}
        {filtered.length === 0 && <div style={{ textAlign: 'center', padding: 32, color: 'var(--text-3)', fontSize: 13 }}>검색 결과 없음</div>}
      </div>
