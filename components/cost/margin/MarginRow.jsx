...
export const MC = (pct, mode, warn, crit) =>
  mode === 'margin' ? MC_MARGIN(pct, warn, crit) : MC_COST(pct, warn, crit);
    ...
    <tr style={r.hidden ? { opacity: 0.5 } : undefined}>
      ...
      {sizeLabels.map(l => {
        ...
        if (hasAdjustment) {
          if (!s?.sellingPrice) {
            netCell = (
              <td key={l + '_n'} style={{ textAlign: 'right', color: 'var(--text-3)' }}>
                —
              </td>
            );
          } else {
            ...
            netCell = (
              <td
                key={l + '_n'}
                className="mt-num"
                style={{ textAlign: 'right', fontSize: 12, color: 'var(--text-2)' }}
              >
            ...
            <td style={{ textAlign: 'right' }}>
              {display != null ? (
                <span
                  ...
                  style={{
                    color: rateColor,
                    background: `color-mix(in srgb, ${rateColor} 14%, transparent)`,
                  }}
                >
                  {display.toFixed(1)}%
                </span>
          ...
          {r.menuCode && onToggleHide && (
            <button
              ...
              style={{
                padding: '2px 6px',
                fontSize: 10,
                color: r.hidden ? 'var(--accent)' : 'var(--text-3)',
              }}
