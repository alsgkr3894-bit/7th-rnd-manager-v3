    ...
    <div className="stat-row" style={{ marginTop: 8 }}>
      <div className="stat-card">
        <div className="stat-label">
          평균 {viewMode === 'margin' ? '마진율' : '원가율'}
          {hasAdjustment ? ' ⟳' : ''}
        </div>
        <div
          key={stats.avg}
          className="stat-value count-landed"
          style={{ color: MC(viewMode === 'margin' ? 100 - stats.avg : stats.avg, viewMode) }}
        >
          {viewMode === 'margin' ? (100 - stats.avg).toFixed(1) : stats.avg.toFixed(1)}
      ...
      {viewMode === 'cost' ? (
        <>
          <div className="stat-card">
            ...
            <div
              key={stats.lowCostCount}
              className="stat-value count-landed"
              style={{ color: 'var(--positive, #10b981)' }}
            >
              {stats.lowCostCount}
          ...
          <div className="stat-card">
            ...
            <div
              key={stats.highCostCount}
              className="stat-value count-landed"
              style={{ color: stats.highCostCount > 0 ? 'var(--negative, #ef4444)' : undefined }}
            >
              {stats.highCostCount}
              <span className="unit">개</span>
            </div>
        ...
        <>
          <div className="stat-card">
            ...
            <div
              key={stats.goodMarginCount ?? stats.lowCostCount}
              className="stat-value count-landed"
              style={{ color: 'var(--positive, #10b981)' }}
            >
              {stats.goodMarginCount ?? stats.lowCostCount}
              <span className="unit">개</span>
            </div>
          </div>
          <div className="stat-card">
            ...
            <div
              key={stats.badMarginCount ?? stats.highCostCount}
              className="stat-value count-landed"
              style={{
                color:
                  (stats.badMarginCount ?? stats.highCostCount) > 0
                    ? 'var(--negative, #ef4444)'
                    : undefined,
              }}
            >
              {stats.badMarginCount ?? stats.highCostCount}
              <span className="unit">개</span>
            </div>
          </div>
        </>
