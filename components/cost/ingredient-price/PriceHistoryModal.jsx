  ...
  return (
    <span style={{display:'inline-flex', alignItems:'center', gap:4}}>
      ...
      <span style={{fontWeight:700, fontSize:13, color}}>
        {formatNumber(newPrice)}원
      </span>
      {diff != null && (
        <span style={{fontSize:11, color, fontWeight:500}}>
          ({sign}{formatNumber(diff)}원)
        </span>
      )}
    </span>
  );
...
export function PriceHistoryModal({ ingredientId, ingredientName, onClose }) {
  ...
  const subtitle = (
    <span>식자재: <span style={{fontWeight:600, color:'var(--text-1)'}}>{ingredientName}</span></span>
  );
            ...
            <li key={h.id ?? i} style={{
              ...
              <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', gap:8}}>
                ...
                <span style={{
                  ...
                  background: 'var(--surface-3)', color: 'var(--text-3)', flexShrink: 0,
                }}>
                  {SOURCE_LABEL[h.source] ?? h.source}
                </span>
              </div>
              <div style={{display:'flex', gap:8, alignItems:'center'}}>
                <span style={{fontSize:11, color:'var(--text-3)'}}>{formatDate(h.changedAt)}</span>
                <span style={{fontSize:11, color:'var(--text-4)'}}>({formatRelative(h.changedAt)})</span>
              </div>
            </li>
          ))}
        </ul>
