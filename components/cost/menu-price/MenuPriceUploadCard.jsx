      ...
      {preview && (
        <div style={{
          ...
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', gap:12, flexWrap:'wrap'}}>
            ...
            <div style={{display:'flex', gap:6}}>
              <button className="btn sm" onClick={() => setPreview(null)} disabled={committing}>취소</button>
              <button className="btn sm primary" onClick={handleCommit}
          ...
          {preview.failed.length > 0 && (
            <div style={{marginTop:10, fontSize:12, color:'var(--warn)'}}>
              ...
              {preview.failed.slice(0, 5).map((f, i) => (
                <div key={i}>· {f.rowIndex}행 — {f.reason}{f.menuName ? ` (${f.menuName})` : ''}</div>
              ))}
              {preview.failed.length > 5 && (
                <div style={{color:'var(--text-3)', marginTop:2}}>… 외 {preview.failed.length - 5}건</div>
              )}
            </div>
          )}
        </div>
