...
export function BulkPriceModal({ onClose, onDone }) {
  ...
  const [prices, setPrices] = useState(() => {
    ...
    CODE_GROUPS.forEach(g => {
      init[g.sub] = {};
      g.sizes.forEach(s => {
        init[g.sub][s] = String(DEFAULT_PRICE_MAP[g.sub]?.[s] ?? '');
      });
    });
    return init;
  });
  ...
  return (
    ...
    >
          ...
          {g.sizes.includes('R') || g.sizes.includes('단품') ? (
            ...
            >
              <input
                ...
                onChange={e =>
                  setPrice(g.sub, g.sizes.includes('R') ? 'R' : '단품', e.target.value)
                }
                placeholder="0"
              />
              <span style={{ fontSize: 12, color: 'var(--text-4)', flexShrink: 0 }}>원</span>
      ...
      {error && (
        <div style={{ marginTop: 12, fontSize: 12, color: 'var(--negative)' }}>{error}</div>
      )}
