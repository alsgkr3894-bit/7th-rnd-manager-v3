...
export function SampleStatsWidget({ samples, router }) {
  ...
  const stats = useMemo(() => {
    ...
    const avg = rated.length > 0 ? rated.reduce((a, s) => a + toFiniteNumber(s.rating), 0) / rated.length : 0;
    const withPhoto = list.filter(s => asObjectArray(s.photos).some(p => asDisplayText(p.data))).length;
    const recent = [...list].sort((a, b) => toTime(b.createdAt) - toTime(a.createdAt)).slice(0, 3);
    const ratingDist = [1, 2, 3, 4, 5].map(r => ({ r, count: list.filter(s => toFiniteNumber(s.rating) === r).length }));
    ...
    <div className="card">
      ...
      <div style={{display:'flex', flexDirection:'column', gap:6}}>
        {recent.map((s, index) => {
          ...
          return (
            <div key={s.id ?? index} className="widget-row"
            ...
            >
              {photoData ? (
                <img src={photoData} alt={`${menuName || title} 샘플 사진`}
              ...
              <div style={{flex:1, minWidth:0}}>
                <div title={title} style={{fontSize:13, fontWeight:600, color:'var(--text-1)',
      ...
      {safeItems.length === 0 ? (
        ...
        <div className="rank-list">
          {safeItems.map((r, index) => {
            ...
            return (
              <button key={r.rank ?? (name || index)} className="rank-row"
                onClick={() => router?.push?.(`/menu-sales/rank-compare?menu=${encodeURIComponent(name)}`)}
              ...
              >
                ...
                {spark.some(v => Number(v) > 0) && (
                  <div className="rank-mini">
                    <Sparkline data={spark} fill={false} width={56} height={22}
    ...
    <div className="card">
      ...
      <div style={{display:'flex', flexDirection:'column', gap:6}}>
        {safeNotes.slice(0, 5).map((n, index) => {
          ...
          return (
            <div key={n.id ?? index} className="widget-row"
              onClick={() => href && router?.push?.(href)}
              style={{
                display:'flex', alignItems:'center', gap:10,
                padding:'8px 12px', borderRadius:8, cursor:'pointer',
  ...
  return (
    <div className="card">
      <div className="card-header">
        ...
        <button className="link accent" onClick={goMargin}>전체 →</button>
      </div>
          ...
          {alerts.map((item, i) => {
            ...
            return (
              <button key={i} className="widget-row"
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div title={menuName} style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-1)',
                    overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {menuName}
                  </div>
                  <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 1 }}>
                    {size} · 원가 {formatNumber(item.cost)}원 / 판매가 {formatNumber(item.sellingPrice)}원
                  </div>
                </div>
                <span style={{
