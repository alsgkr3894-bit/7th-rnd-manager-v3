...
export const HomeKpiRow = memo(function HomeKpiRow({ salesKpi, costKpi, noteKpi, salesCount, noteCount }) {
  ...
  const hasSalesPeriod = salesYear && Number.isInteger(salesMonthNumber) && salesMonthNumber >= 1 && salesMonthNumber <= 12;
  ...
  const momLabels = hasSalesPeriod
    ? Array.from({ length: 6 }, (_, i) => { let m = salesMonthNumber - (5 - i); while (m < 1) m += 12; return `${m}월`; })
    : [];
    ...
    <div className="hero-row motion-stagger">
      ...
      >
        <div>
          <div className="label">
            {hasSalesPeriod
              ? `${salesYear}년 ${salesMonthNumber}월 판매량`
              : '최근 판매량'}
          </div>
          <div className={salesPopped ? 'value num count-landed' : 'value num'}>{formatNumber(safeSalesCount)}<span className="unit">개</span></div>
          <div className="trend">
            {!hasDeltaPct ? (
              ...
              <>
                <span className={deltaPct > 0 ? 'up' : 'down'}>
                  ...
                  {' '}{deltaPct > 0 ? '+' : ''}{formatPercent(deltaPct)}
                </span>
                <span style={{color:'var(--text-4)'}}>전월 대비</span>
              </>
            )}
      ...
      <div className="card kpi-card">
        <div>
          ...
          <div className="value num" style={{color: safeCostRate == null ? 'var(--text-4)' : undefined}}>
            {safeCostRate == null ? '—' : safeCostRate.toFixed(1)}<span className="unit">%</span>
          </div>
          <div className="trend">
      ...
      >
        <div>
          ...
          <div className={notePopped ? 'value num count-landed' : 'value num'}>{safeNoteCount}<span className="unit">건</span></div>
          <div className="trend">
