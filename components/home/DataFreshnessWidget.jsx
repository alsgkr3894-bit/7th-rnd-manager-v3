  ...
  const uploadRows = [
    ...
    isMain
      ? {
          ...
          desc: '제때 제품 출고 파일',
          href: '/jette/shipment',
          status: freshness?.shipment,
        }
      : null,
  ].filter(Boolean);
  const backup = backupMeta(backupReminder);
  const needsAttention =
    uploadRows.filter(row => statusMeta(row.status).label !== '최신').length +
      ...
      <div className="card-header">
        <div>
          ...
          <div className="card-sub">
            {needsAttention > 0 ? `${needsAttention}개 항목 확인 필요` : '핵심 데이터 최신 상태'} ·{' '}
            {targetText(freshness?.target)}
          </div>
        </div>
