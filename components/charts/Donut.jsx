...
export function Donut({ items, size = 140, thickness = 14, onSegmentHover }) {
  ...
  const safeThickness = normalizeChartDimension(thickness, 14, {
    min: 1,
    max: Math.max(1, safeSize / 2 - 1),
  });
  ...
  const r = (safeSize - safeThickness) / 2,
    c = 2 * Math.PI * r;
      ...
      {safeItems.map((it, i) => {
        ...
        return (
          <circle
            ...
            onMouseEnter={() => {
              setHoveredIdx(i);
              notifySegmentHover(i);
            }}
            onMouseLeave={() => {
              setHoveredIdx(null);
              notifySegmentHover(null);
            }}
          />
        );
      })}
