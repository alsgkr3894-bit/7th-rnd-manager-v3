...
export const SampleListRow = React.memo(function SampleListRow({ sample = {}, onClick, onEdit, onCopy, onDelete }) {
  ...
  return (
    <tr onClick={click} style={{ cursor: 'pointer' }}>
      ...
      <td>{category ? <span className="chip" style={{ background: 'var(--surface-2)', color: 'var(--text-2)' }}>{category}</span> : '—'}</td>
      ...
      <td style={{ whiteSpace: 'nowrap', color: rating > 0 ? (RATING_COLOR[rating] || 'var(--text-2)') : 'var(--text-4)' }}>
        {rating > 0 ? '★'.repeat(Math.min(5, rating)) : '—'}
      </td>
      <td style={{ textAlign: 'right', whiteSpace: 'nowrap', color: 'var(--text-2)' }}>
        {hasPrice ? `${price.toLocaleString('ko-KR')}원${rec.priceTaxType === 'excl' ? '(별도)' : ''}` : '—'}
      </td>
      <td style={{ textAlign: 'right', whiteSpace: 'nowrap' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
          <button className="btn sm" onClick={edit}>수정</button>
          <button className="btn sm" onClick={copy}>복사</button>
