...
export default function SalesReportControls({
  ...
  const safeAvailMonthsByYear = availMonthsByYear && typeof availMonthsByYear === 'object'
    ? availMonthsByYear
    : {};
    ...
    <>
      ...
      <OptGroup label="집계 기간">
        ...
        <div className="opt-period-row">
          <select className="period-select num" value={safeYear} onChange={e => {
          ...
          {periodMode === 'month' && (
            <select className="period-select num" value={safeMonth} onChange={e => onMonth?.(parseInt(e.target.value, 10))}>
        ...
        {viewMode === 'compare' && (
          <div style={{marginTop:8}}>
            ...
            <div className="opt-period-row">
              <select className="period-select num" value={safeCmpYear} onChange={e => {
                const y = parseInt(e.target.value, 10);
                onCmpYear?.(y);
              ...
              <select className="period-select num" value={safeCmpMonth} onChange={e => onCmpMonth?.(parseInt(e.target.value, 10))}>
                {monthsFor(safeCmpYear, safeCmpMonth).map(m => <option key={m} value={m}>{m}월</option>)}
              </select>
            </div>
          </div>
        )}
      ...
      <OptGroup label="포함 섹션">
        <Check label="요약 (총 판매량·전월 대비)"      value={safeOpts.summary}    onChange={v=>handleUpd('summary',v)}/>
        <Check label="카테고리별 판매 비중"             value={safeOpts.catShare}   onChange={v=>handleUpd('catShare',v)}/>
        <Check label="피자 전월 대비 상승/하락 TOP 5"   value={safeOpts.pizzaMover} onChange={v=>handleUpd('pizzaMover',v)}/>
        <Check label="메뉴 순위표 (전체)"               value={safeOpts.rankTable}  onChange={v=>handleUpd('rankTable',v)}/>
        <Check label="카테고리별 비중 그래프"            value={safeOpts.catBar}    onChange={v=>handleUpd('catBar',v)}/>
        <Check label="규격별 세부 (L/R/기타)"           value={safeOpts.variant}   onChange={v=>handleUpd('variant',v)} hint="순위표 아래 확장"/>
        <Check label="전월 대비 증감 컬럼"              value={safeOpts.prevComp}  onChange={v=>handleUpd('prevComp',v)}/>
        <Check label="품목 제외 리스트 (마지막 페이지)"  value={safeOpts.excluded}  onChange={v=>handleUpd('excluded',v)}/>
      </OptGroup>

      <OptGroup label="문서 형식">
        <Check label="PDF"                value={safeDocFormat.pdf}   onChange={v=>handleUpdFmt('pdf',v)}/>
        <Check label="Excel (시트별 정리)" value={safeDocFormat.excel} onChange={v=>handleUpdFmt('excel',v)}/>
      </OptGroup>
    </>
  );
}
