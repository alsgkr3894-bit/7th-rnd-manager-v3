...
function formatDate(value, options) {
  ...
  const safeValue = typeof value === 'string' || typeof value === 'number' || value instanceof Date
    ? value
    : asDisplayText(value);
...
function ReportCover({ report }) {
  ...
  const createdAt = formatDate(report.createdAt, { year:'numeric', month:'long', day:'numeric' });
  return (
    <div style={{textAlign:'center', padding:'40px 0 32px', borderBottom:'1px solid var(--border)'}}>
      ...
      {author && <div style={{fontSize:12, color:'var(--text-3)', marginTop:4}}>작성자: {author}</div>}
    </div>
  );
}
...
function ReportOptionsPage({ report }) {
  ...
  const kindOpts = () => {
    ...
    if (kind === 'compare') return (<>
      ...
      <OptionRow label="기간 A" value={opts.yearA && opts.monthA ? `${asDisplayText(opts.yearA)}년 ${asDisplayText(opts.monthA)}월` : '—'}/>
    ...
    if (kind === 'cost') {
      ...
      const catLabel = Object.entries(cats).filter(([,v])=>v).map(([k])=>
...
function ReportSummaryPage({ report }) {
  ...
  return (
    <div style={{padding:'16px 0'}}>
      ...
      <OptionRow label="보고서 ID" value={reportId ? `RPT-${reportId.padStart(4,'0')}` : '—'}/>
      <OptionRow label="총 페이지" value={`${pages}쪽`}/>
      <OptionRow label="조회수" value={`${views}회`}/>
      <OptionRow label="공유 링크" value={links > 0 ? `${links}개 활성` : '없음'}/>
...
export function ReportPreviewModal({ report, onClose, onShare, printOnOpen = false }) {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === 'Escape' && typeof onCloseRef.current === 'function') onCloseRef.current();
    };
  ...
  const kind        = asDisplayText(safeReport.kind);
  const color       = KIND_COLOR[kind] || '#888';
  const name        = asDisplayText(safeReport.name, '보고서');
  const period      = asDisplayText(safeReport.period, '—');
  const author      = asDisplayText(safeReport.author, '—');
    ...
    <div className="modal-scrim">
      <div className="preview-shell" onClick={e => e.stopPropagation()}
        ...
        <div className="preview-meta">
          <button className="modal-close" onClick={handleClose} style={{marginBottom:24}}>
          ...
          <div style={{marginBottom:20}}>
            ...
            <div id="preview-modal-title" style={{fontSize:15, fontWeight:700, marginBottom:4, lineHeight:1.4}}>{name}</div>
          ...
          <div style={{display:'flex', flexDirection:'column', gap:6}}>
            <button className="btn sm primary" onClick={() => { if (typeof onShare === 'function') onShare(safeReport); handleClose(); }}>
        ...
        <div className="preview-body">
          <ReportPaper report={safeReport} pageIndex={currentPage - 1} />
        ...
        <div ref={printSourceRef} aria-hidden="true" style={{display:'none'}}>
          {PAGE_COMPONENTS.map((_, index) => (
            <ReportPaper key={index} report={safeReport} pageIndex={index} />
          ))}
        </div>
      </div>
    </div>
